...
### 1.5 Greitas profilio atpažinimas (70.5)

**Tikslas:** Po kelionės fokuso pasirinkimo duoti lengvą, vizualų „kur aš dažniausiai dirbu su duomenimis?“ momentą. Tai nėra šakų filtravimo mechanizmas – jis tik padeda dalyviui atpažinti savo analitiko profilį ir susieti modulį su kasdieniu darbu.

**Skaidrės tipas:** `intro-action-pie` (branduolys, be `pathBranch`).

**Klausimas:** „Koks tavo analitiko profilis šiandien?“

**Segmentai / kortelės:**

| Profilis               | Reikšmė | Kortelės paaiškinimas                           |
| ---------------------- | ------- | ----------------------------------------------- |
| Sprendimų analitikas   | 30      | Vadovybei reikia aiškių išvadų ir veiksmų       |
| Duomenų tvarkytojas    | 24      | Pirma sutvarkai šaltinius, lenteles ir kokybę   |
| Rinkos tyrėjas         | 18      | Ieškai tendencijų, šaltinių ir palyginimų       |
| Vizualizacijų kūrėjas  | 16      | Grafikai, dashboard'ai ir istorija iš skaičių   |
| Automatizavimo kūrėjas | 12      | Nori srautų, agentų ir pasikartojančių žingsnių |

**Atskleidimo įžvalgos:** kiekvienas pasirinkimas parodo, į ką modulyje atkreipti dėmesį: sprendimų filtrai / duomenų paruošimas / tyrimų promptai / vizualizacijos / pipeline ir agentai.

**Footer:** toliau – kelio apžvalga (7.1).
...
| **Struktūruoti**        | Excel, SQL, ERP. SQL tipui – PostgreSQL arba MySQL; lankščiai schemai – MongoDB; prototipui – SQLite. Detalus įrankių žemėlapis – skaidrėje „Duomenų struktūros kūrimas (DB)“. | „Normalizuok lentelę pagal 3NF principus.“                                        |
...
> **Promptų branda (M7P):** ne visi CopyButton reikalauja pilno META/INPUT/OUTPUT. Klasės F (flagship) / S (etapas: Duomenys→Padaryk→Formatas) / R (rolės sėkla) / L (lab) – žr. [`docs/development/M7_PROMPT_MATURITY.md`](development/M7_PROMPT_MATURITY.md). Kelio variantai – vienas skeletas + domeno slotai ([`M7_JOURNEY_COPY_REGISTRY.md`](development/M7_JOURNEY_COPY_REGISTRY.md)).
...
**Įgyvendinta JSON (2026-06-30):** skaidrė 98 nebėra roadmap stubas. Ji tapo optional „gilesnės praktikos“ skaidre su modelių, realių duomenų, automatizavimo ir validacijos blokais; 3 CopyButton promptai ir collapsible apibendrinimas. `pathBranch: technika` ir `optional: true` palikti.
...
| **7.4-w1**   | **Savitikra: Duomenų analizės pipeline**                | Warm-up-quiz po 7.3–7.4: 3 klausimai apie 6 žingsnių pipeline, EDA vietą ir publikavimo / sprendimo etapą. Tikslas – atskirti 6 žingsnių pipeline nuo 8 žingsnių MASTER.                                                     | M8             |
...
| **7.4a-w1**  | **Savitikra: 4 analizės tipai**                         | Warm-up-quiz po 7.4a: 3 klausimai, kurie vadovo klausimus „kas / kodėl / kas bus / ką daryti“ susieja su teisingu analizės tipu. Klausimai turi būti situaciniai, ne vien atminties testas.                                  | M8             |
...
| **7.4d**     | **Sprendimų filtrai**                                   | 5 grupės, kaip analizės rezultatą paversti veiksmu: Tinka/Netinka, Kas svarbiausia, Testuok/Investuok/Atmesk, Greiti laimėjimai + nuorodos į SWOT (7.4c), 80/20 ir „Kodėl?“ (7.4a), Riziką/Naudą (7.4c, 7.26). 4 CopyButton. | OUTPUT, M9     |
...
| **7.11**     | **Duomenų struktūros kūrimas (DB)**                     | Įrankių matrica (6) + CopyButton: e-commerce DB struktūra, ER, KPI; optional CopyButton įrankio pasirinkimui ir lentelių metodika (5.9).                                                                                     | INPUT          |
...
| **7.16b-w1** | **Savitikra: Duomenų paruošimas**                       | Warm-up-quiz po 7.16a–7.16b: 3 klausimai apie 5 žingsnių paruošimą, metaduomenis ir valymo checklist prieš analizę. Klausimai aiškiai atskiria 5 paruošimo žingsnius nuo 6 pipeline ir 8 MASTER.                             | M8             |
...
| **7.27**     | **Modulio 7 santrauka**                                 | Apžvalga; branduolys baigtas, vizualizacija pažymėta kaip optional šaka; `firstAction24h` – per 24–48 val. paleisti MASTER PROMPTĄ su realia darbo tema; CTA į M8 testą.                                                     | —              |
...
### 8.1b Warm-up savitikros – M7 branduolio ritmas

**Tikslas:** Ilgame M7 branduolyje kas kelis konceptus įterpti trumpą neįskaitinę savitikrą. Tai mažina `content-block` monotoniją ir ruošia M8 testui, bet nekopijuoja M8 klausimų pažodžiui.

| Skaidrė   | Vieta                                   | Tikrinamas modelis                    | Klausimų principas                                                                                                         |
| --------- | --------------------------------------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **73.5**  | Po 7.3–7.4 / prieš 71.1                 | **6 žingsnių pipeline**               | Kiek etapų, kur yra EDA, koks paskutinis publikavimo / sprendimo etapas.                                                   |
| **731.5** | Po 7.4a / prieš sentimentų analizę      | **4 analizės tipai**                  | „Kas įvyko?“, „Kodėl?“, „Kas gali įvykti?“, „Kaip veikti?“ – situaciniai klausimai.                                        |
| **891.5** | Po 7.16a–7.16b / prieš patikimumo bloką | **5 žingsnių paruošimas ir workflow** | Valymas, metaduomenys, teisinga seka prieš analizę.                                                                        |
...
**Kam tai?** 4 analizės tipai (7.4a) atsako į klausimą „Kas įvyko / Kodėl / Kas gali įvykti / Kaip veikti?“. Bet „Kaip veikti?“ dažnai nutrūksta pusiaukelėje – gauni ilgą sąrašą pastebėjimų, ne sprendimą. Ši skaidrė duoda **penkis trumpus filtrus**, kurie priverčia rezultatą baigtis konkrečiu veiksmu: taisyti, prioritetą nustatyti, atmesti, testuoti arba plėsti. Skaidrė **core** (branduolys) – matoma visiems, nepriklausomai nuo pasirinkto fokuso (skaidrė 70).

**Penkios grupės (pasirink pagal situaciją):**

| Grupė                               | Klausimas, į kurį atsakai                                                   | Metodas(ai)                                   | Ar tai naujas šioje skaidrėje?                            |
| ----------------------------------- | --------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------- |
| **A. Tinka / Netinka**              | Priimti ar atmesti?                                                         | OK / Fail patikra                             | Taip – naujas CopyButton (3a)                             |
| **B. Kas svarbiausia?**             | Ką daryti pirmiausia?                                                       | Būtina / Svarbu / Norima / Ne dabar (MoSCoW)  | Taip – naujas CopyButton (3b); ICE balas – žr. „Detaliau“ |
| **C. Testuok / Investuok / Atmesk** | Ką testuoti, kur investuoti, ką nutraukti?                                  | Kiss-Marry-Kill principas                     | Taip – naujas CopyButton (3d)                             |
| **D. Greiti laimėjimai**            | Kur mažai pastangų, greitas rezultatas?                                     | Low-hanging fruit                             | Taip – naujas CopyButton (3c)                             |
| **E. Jau moki šiuos**               | Stiprybės/silpnybės? Kas duoda daugiausia? Kodėl neveikia? Rizika ir nauda? | SWOT, 80/20, „Kodėl?“ gilinimas, Rizika/Nauda | Ne – nuoroda į 7.4a, 7.4c, 7.26 (žemiau)                  |

**Daryk dabar:** Pasirink situaciją iš lentelės viršuje → nukopijuok atitinkamą promptą (3a–3d) → įklijuok į DI su savo duomenimis ar idėjų sąrašu.

**Kopijuojami promptai (4):**

_3a. Tinka / Netinka (OK / Fail patikra)_

```
Įvertink šiuos duomenis/rezultatą OK / Fail logika.

Kontekstas: [aprašyk verslą, duomenis, tikslą]
Duomenys: [įklijuok]

Atsakyk:
1. Kas aiškiai tinka (OK)?
2. Kas aiškiai netinka (Fail)?
3. Kas neaišku?
4. Kokia tikėtina kiekvieno „Fail“ priežastis?
5. Ką taisyti pirmiausia?
6. Galutinis verdiktas: Tinka / Netinka / Tinka su sąlygomis.

Būk kritiškas – nesušvelnink silpnų vietų.
```

_3b. Kas svarbiausia? (Būtina / Svarbu / Norima / Ne dabar)_

```
Suskirstyk šį sąrašą į 4 grupes: Būtina / Svarbu / Norima / Ne dabar (šis metodas kartais vadinamas MoSCoW).

Kontekstas: [tikslas / projektas]
Sąrašas: [užduotys, funkcijos, idėjos]

Kiekvienam punktui parašyk:
1. Kodėl jis priklauso tai grupei
2. Poveikis verslui
3. Rizika, jei nedarysime
4. Rekomenduojamas kitas žingsnis

Būk negailestingas apimčiai – „Ne dabar“ grupė turi būti reali.
```

_3c. Greiti laimėjimai (Low-hanging fruit)_

```
Rask greitus laimėjimus šiuose duomenyse.

Tikslas: [ką norime pagerinti]
Duomenys: [problemos, galimybės, metrikos]

Ieškok galimybių, kurios yra:
1. Mažai pastangų
2. Mažai kainuoja
3. Greitai įgyvendinamos
4. Duoda matomą rezultatą

Kiekvienai pateik: veiksmas, tikėtinas poveikis, pastangų lygis, kada galima patikrinti, rizika, kaip matuosime sėkmę.

Išrink 5 svarbiausias.
```

_3d. Testuok / Investuok / Atmesk (Kiss-Marry-Kill)_

```
Įvertink šiuos variantus principu Testuok / Investuok / Atmesk (Kiss-Marry-Kill).

Kontekstas: [tikslas, biudžetas, terminas]
Variantai: [produktai, kanalai, klientai, idėjos]

Kiekvienam variantui:
1. Testuok – vertas mažo bandymo
2. Investuok – stiprus kandidatas dideliam įsipareigojimui
3. Atmesk – silpna grąža, blaškantis arba rizikingas

Paaiškink: 1) kodėl toks priskyrimas, 2) įrodymai už/prieš, 3) kas pakeistų nuomonę, 4) kitas žingsnis, 5) kaip matuosime sėkmę.

Būk tiesus, orientuokis į verslo rezultatą.
```

**Patikra (1 min):**
• Ar pasirinkai filtrą pagal savo situaciją (tinka/netinka, prioritetas, portfelis, greiti laimėjimai)?
• Ar rezultatas baigiasi **konkrečiu veiksmu**, ne tik lentele?
• Ar žinai, kur ieškoti SWOT, 80/20 ar Rizikos/Naudos, kai jų reikia (žemiau)?

Jei bent 2 „ne“ → grįžk į lentelę viršuje, pasirink teisingą grupę.
...
- **Tikslas (aspiracinis blueprint):** Įsitikinti, kad dalyvis įsisavino strateginį pamatą, pipeline, operacines sąvokas, duomenų tipus ir rinkimo strategiją, verslo duomenų domenus, DI vaidmenį, promptų tipus (rolė, DB, ryšiai, vizualizacijos, prognozė, tyrimų sistema, 5 žingsnių algoritmas, EDA, lentelės, screenshot, schema, Deming, MASTER PROMPTAS), **vizualizaciją ir data storytelling (DA_4)** – ciklas, psichologija, Geštaltas, verslo promptai, super promptas, alternatyvos.
- **Produktas (MVP gate):** `modules.json` – **9 graded** klausimai (daugiausia scenario + MCQ) + warm-up (4, įsk. M9 fork) + 2 bonus (801/802). Tai readiness check prieš M9, ne pilnas M7 finalinis egzaminas. DA_4 gylis / metadata tipai / true-false – **backlog** (P2), ne dabartinis DoD.
- **Formatas (MVP):** MCQ + **scenario**; remediation nuorodos į M7 skaidres. True/false – backlog.
- **Slenksčiai:** ≥70 % – rekomenduojama eiti į Modulį 9 (projektas); &lt;70 % – peržiūrėti 7.x skaidres.
- **Lukštas:** Path Test Contract – `GOLDEN_STANDARD.md` §3.4a1 (M8 išimtis: 4 warm-up).
...
**Kontekstas:** ROI, segmentacija, kurios temos veikė, ką nutraukti / plėsti / testuoti. „Nutraukti / plėsti / testuoti“ – tas pats principas kaip skaidrės 7.4d Testuok / Investuok / Atmesk (Kiss-Marry-Kill).
...
**Kontekstas:** Rizika ir nauda – ta pati logika kaip skaidrėje 7.4d („Jau moki šiuos“ blokas).
...
| **Sprendimų filtras**                              | Trumpas metodas, kuris analizės rezultatą paverčia veiksmu (taisyti, prioritetą nustatyti, atmesti, testuoti, plėsti) – ne tik aprašymu.                           |
| **OK / Fail patikra**                              | Greitas kokybės vartai: kas aiškiai tinka, kas aiškiai netinka, kas neaišku, galutinis verdiktas (Tinka / Netinka / Tinka su sąlygomis).                           |
