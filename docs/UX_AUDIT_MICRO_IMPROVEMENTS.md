...
- **Pavargsta skaityti:** Ilgos content-block skaidrės su keliais sekcijų blokais (TL;DR → Daryk dabar → Copy → Patikra → Optional) be tarpų tarp „kvėpavimo“ – viena blokų siena. Mažas body (`text-sm`) ilgose pastraipose. Lentelėse (LENTELIU_STANDARTAS) – jei per daug eilučių, skenuojamumas krenta.
...
3. Content-block skaidrėse – **TL;DR (accent) → Daryk dabar (brand) → Copy → Patikra → Optional** – tarpai `space-y-6`; vienas „Kopijuoti“ mygtukas.
...
*Šis dokumentas – UX auditas micro patobulinimams. Implementaciją atlieka CODING_AGENT pagal UI_UX_AGENT gaires ir GOLDEN_STANDARD.*
