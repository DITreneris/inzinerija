...
> **Atnaujinta:** 2026-08-06 (corporate12 Supabase Phase 1 memo)  
...
> **Horizon B (M1–12):** training repo ready (`build:corporate12`, magic-link tier **12**). Cutover = pin **1.5.0** + `build:corporate12` + Supabase `highest_plan=12` → `access_tier=12` (Stripe €199 = Phase 2) — žr. §7a + memo 06.  
...
> **Pin decision (CAV-B1):** learning freeze tag **v1.4.9** (M1–9) vs app **1.5.0** corporate12 cutover. Default production stay on `build:production` M1–9 until Horizon B execute; do not pin corporate18 (Wave D3 / CAV-C2 parked).  
...
2. **Pasirink pin target prieš deploy:**
   ...
   - **1.5.0** — **corporate12 cutover** (M1–12 + magic-link tier 12; M16–18 authoring in full SOT; magic-link prod ≤15).
...
1. Build log: `VITE_MAX_BUILD_MODULE=12` / `build:corporate12`; submodule = **1.5.0**.
...
| Reikalavimas        | Detalė                                                                                                 |
| ------------------- | ------------------------------------------------------------------------------------------------------ |
| **Pin**             | Submodule **1.5.0** — [MARKETING_SUBMODULE_PIN_CORPORATE12.md](MARKETING_SUBMODULE_PIN_CORPORATE12.md) |
