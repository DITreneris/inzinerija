...
| **Validacija**                            | ✅                       | `validate-schema.mjs` – full, core (`*-m1-m6`, `*-m1-m9`), EN overlays, glossary, tools. `sot_index.json` – `validate-sot-index.mjs`. Release gate M1–9: `npm run audit:release-preflight`; M10–12: `npm run audit:m1012`.                                                                 |
...
- **Padengimas:** modulesLoader, glossaryLoader, questionPoolSelector, certificatePdf, handout PDF (M1/M5/M6/M79), accessTier, mvp.gating, gate.smoke, a11y smoke, ModuleCompleteScreen, QuizPage, ecosystem URLs, diagram registry guards, EN audit tests (`m46EnLanguageAudit`, `m79EnLanguageAudit`, `m1012EnLanguageAudit`).
