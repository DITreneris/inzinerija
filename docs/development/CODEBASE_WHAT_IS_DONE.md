...
> **Tikslas:** Viena vieta – kas dabar veikia, kokie duomenys, kokybė, apimtis. Atnaujinta: 2026-07-26 (release **1.4.8**: M9 quest, M7 embeds, prompt maturity, tools/glossary gates).
...
| ------------------------------------------------ | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Moduliai 1–6**                                 | ✅ Pilnai                               | Teorija (M1), testas (M2), praktika (M3), pažangus (M4–M6). Full SOT: `modules.json`; MVP profilis: `modules-m1-m6.json`; EN: `modules-en.json` (M1–M3), `modules-en-m4-m6.json` (M4–M6). **M1–3 copyable freeze (M1P, 2026-07-26):** `M1_M3_PROMPT_MATURITY.md`.                                                                                                                                                                              |
...
| **Moduliai 10–12**                               | ✅ Turinys full kataloge                | Agentų kelias: taksonomija (10.45), workflow šablonai (10.48), **10.26 žmogaus kontrolės lab** (`m10_human_control_simulator`, Pattern `interactive-control-lab`, Shell: Ne; W1.1 risk strip + lab CopyButton taisyklė → 10.64), M11 testas, M12 capstone; EN partial overlay `modules-en-m10-m12.json`.                                                                                                                                       |
...
| **Žodynėlis**                                    | ✅                                      | `glossary.json` / `glossary-en.json` + search UI. Apibrėžimai **visada skaitomi** (GOLDEN §3.4d). Path-step `unlockedBy` = kelio atlygio metadata. M7–9 pack (2026-07-26): filtras 7/8/9 ne tuščias; `MASTER PROMPTAS` ≠ `Master promptas`.                                                                                                                                                                                                    |
...
| **PDF atmintinės**                               | ✅ M1, M4, M5, M6, M7–9, M10–12, M13–15 | Bendras maketas: `handoutPdfKit.ts`. M1/M4: value-only atmintinės. M5/M6: `m5/m6Handout*`. M7–9: `m79HandoutContent*.json`; M10–12: `m1012HandoutContent*.json`; M13–15: `m1315HandoutContent*.json` (kelio bundle su funnel CTA). Entry point routing – `completionArtifacts.json`; pakartotinis atsisiuntimas – `ModulesPage` „Mano medžiaga“ (**po** paskutinio `maxAccessible` modulio, prieš tier-lock; 2026-07-23).                      |
...
| **Schemas / diagramos**                          | ✅ Lokalizuotos                         | LlmArch, Schema3, DiPrezentacijosWorkflow, StrukturuotasProcesas, RagDuomenuRuosimas, RlProcess, TurinioWorkflow, ContextEngineeringPipeline (M4 sk. 45: B+1 + Prompt placeholder slots), M7–M12 React schemos (`m12_multi_agent_schema`, M10 `m10_*`) – tekstai per locale getterius arba i18n. **Lab (no Shell):** `m10_human_control_simulator` (ChoiceControl + risk strip + CopyButton artefaktas). Legacy `AgentOrchestrator` nebewired. |
...
| **Testai**                                       | ✅ 129 failai, 822 testai               | Vitest + RTL; unit, component, integration, a11y smoke, gate tier 9, handout PDF (M1/M4/M5/M6/M79/M1012/M1315), tier 4/5 eligibility, diagram / LMS polish guards, M10–15 EN/data-contract audit guard'ai, journey / M9 quest / manipulation-contrast; + M7 path map / prompt-tool / audit remediacija (2026-07-26 Unreleased). `@ tag 1.4.8` freeze = 126/781.                                                                                |
...
| **ModulesPage katalogas**                        | ✅                                      | Track chrome (brand/sky/fuchsia/rose); bazės subsection M4–M6; „Mano medžiaga“ po `maxAccessible` (GOLDEN §8.4).                                                                                                                                                                                                                                                                                                                               |
...
- **129 testų failai, 822 testai** (unit + component + integration; HEAD Unreleased 2026-07-26; `@ tag 1.4.8` = 126/781).
...
- **Learning QA:** M7–9 browser smoke CQ-M79-1/2 ✅; lieka open `TODO.md` §1.1–§1.2 — Portal 48h, PDF rankinė, M1012-2 C1–C6, DIAG-1.
- **E2E (Playwright):** gate smoke jau dengiamas Vitest; Playwright – later.
- **Monitoring (PostHog/GA4):** marketing handoff – `TODO.md` §1.4 MON-4 (ne šio repo P0).
