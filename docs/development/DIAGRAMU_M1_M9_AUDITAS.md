...
> **Docs Lean DL-3:** Gyvas inventorius = [`TEACHING_ELEMENTS_REGISTRY.md`](TEACHING_ELEMENTS_REGISTRY.md) + [`teaching-elements-overlay.json`](teaching-elements-overlay.json). Šis failas = **kokybės rubrika + istorinis M1–9 smoke** – slide ID / eilutės gali atsilikti. Orchestrator neprivalo krauti šio failo kaip primary SOT; M7–12 detales → [`DIAGRAMU_M7_M12_REGISTRY.md`](DIAGRAMU_M7_M12_REGISTRY.md).
...
| M4 / 54           | Promptų režimo lab          | `m4_prompt_mode_simulator` -> `M4PromptModeSimulatorBlock`                                                                                                 | `interactive-control-lab` (Shell Ne) | Pilnas (model + LT/EN + W1.1 strip) | Axis strip (duomenys × metodika); 3×4; artefaktas lab CopyButton → RAG / M6. Ne SVG.          |
...
| M4 / RAG ruosimas | RAG ruosimo magistrale      | `rag_duomenu_ruosimas` -> `RagDuomenuRuosimasBlock`                                                                                                        | `linear-process` (Shell Ne)          | Maturity 3 (2026-07-27)             | `InteractiveDiagramShell` chrome + brand-active tokens; HTML timeline + prompt/Copy kortelės. |
...
| M7 / 73           | DA pipeline 6               | `m7_da_pipeline` (`/da_pipeline_6.svg` alias) -> `M7DaPipelineBlock` per registry                                                                          | `linear-process`                     | Migruota i React                    | Live slide **73** (ne curriculum „70“); legacy SVG alias paliktas.                            |
...
| M7 / 89           | Duomenu paruosimas          | `m7_data_prep_workflow` -> `M7DataPrepWorkflowBlock`                                                                                                       | `linear-process`                     | A prioriteto refaktorius            | Live slide **89**; dubliuoja M9 8 zingsniu schema; reikia bendro patterno.                    |
| M7 / 92           | BI schema                   | `m7_bi_schema` (`/da_bi_schema_4.svg` alias) -> `M7BiSchemaBlock` per registry                                                                             | `linear-process`                     | Migruota i React                    | Live slide **92**; legacy SVG alias paliktas.                                                 |
...
| P3          | `RagDuomenuRuosimasBlock.tsx`                                                                      | **Done 2026-07-27:** `InteractiveDiagramShell` chrome + brand-active tokens (maturity 2→3, Shell=Ne). TE: `diagram:rag_duomenu_ruosimas`.                                                               | Prompt + CopyButton kortelės UX išlaikytos; ne SVG spine.                                               |
