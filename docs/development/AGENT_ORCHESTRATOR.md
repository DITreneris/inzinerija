...
| **Turinio tiesa (Moduliai 7–9)**                                                           | `docs/turinio_pletra_moduliai_7_8_9.md`                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Turinio tiesa (Moduliai 10–12)**                                                         | `docs/turinio_pletra_moduliai_10_11_12.md` — **gyvas turinys FREEZE** (hygiene liekana **39**; nevaryti į 0; T01–T08 Must chrome ✅; T01 I5 parked; pin **v1.6.3**). Chrome = `build:modules-en-m10-m12`, ne `generate:core-data`. Intake: `M10_M12_TESTER_INTAKE_2026-08.md`.                                                                                                                                                                                          |
| **Turinio tiesa (Moduliai 13–15)**                                                         | `docs/turinio_pletra_moduliai_13_14_15.md` (authoring). Owner walk Phase A: `docs/development/intake/M13_OWNER_WALK_INTAKE_2026-08.md` (**be JSON** kol batch). TRIM/TE freeze.                                                                                                                                                                                                                                                                                         |
| **Turinio tiesa (Moduliai 16–18)**                                                         | `docs/turinio_pletra_moduliai_16_17_18.md` + eilė `MODULIO_16_SKAIDRIU_EILES.md` + full SOT JSON; TE Must/Should/C2 done; Could won’t-now; D3 Deferred `TODO.md` §1.5                                                                                                                                                                                                                                                                                                   |
| **Turinio atpažinimas** (kur kalbama apie Modulį 1…6, skaidrės)                            | `docs/CONTENT_MODULIU_ATPAZINIMAS.md`                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Duomenų tiesa**                                                                          | `src/data/modules.json`, `src/data/modules-m1-m6.json`, `src/data/modules-m1-m9.json`, `src/data/modules-en-m10-m12.json`, `src/data/modules-en-m13-m15.json`, `src/data/modules-en-m16-m18.json`, `src/data/promptLibrary.json`, `src/data/glossary.json`, `src/data/glossary-m1-m6.json`, `src/data/glossary-m1-m9.json`, `src/data/tools.json`, `src/data/tools-en.json`, `src/data/tools-m1-m6.json`, `src/data/tools-m1-m9.json`, `src/data/hallucinationRates.ts` |
...
**Architektūra A:** `src/data/modules.json`, `src/data/glossary.json`, `src/data/tools.json` ir `src/data/tools-en.json` (exact `name` parity; gate `npm run audit:tools` / `validate:schema`) lieka full redagavimo SOT. Core profiliai per `generate:core-data` (`*-m1-m6` / `*-m1-m9` / `*-m1-m12` / `*-m1-m15`) — **be** M16+ spill. EN overlays: `modules-en-m10-m12` / `m13-m15` / `m16-m18` (merge kai kataloge max ≥10/13/16). Keitimai M10+ **nereikalauja** M1–9 core sync; M16+ → `build:modules-en-m16-m18`, ne `generate:core-data`.
...
| **B) Duomenys (JSON/duomenys)** – struktūra, validacija, sinchronas su turiniu, duomenų kokybė                                                                   | **DATA_AGENT**                 | full SOT: `modules.json`, `glossary.json`, `tools.json`+`tools-en.json`; core: `*-m1-m6`…`*-m1-m15`; EN: `modules-en-m10-m12` / `m13-m15` / `m16-m18`; taip pat `promptLibrary.json`, `hallucinationRates.ts` |
...
| **V) Vartotojo kelionė / MVP modulio analizė** – trintis, energija, onboarding, mokymosi baigtis / transferas, 5 zonų diagnostika                                | **USER_JOURNEY_AGENT**         | žr. `docs/development/USER_JOURNEY_AGENT.md`                                                                                                                                                                  |
...
2. **DATA_AGENT** – JSON struktūra ir sinchronas su turiniu; reconcile core profilį pagal build (`*-m1-m6` MVP ar `*-m1-m9` corporate); M10+ – tik full SOT + EN overlay.
...
**Jei užduotis apima vartotojo kelionės / MVP modulio analizę arba modulio patobulinimą iš vartotojo patirties:** pirmiausia **USER_JOURNEY_AGENT** (diagnozė, 5 zonos, Top 5, micro-win, 48h transfero patikra — mokymosi baigtis, ne CRO); tolesnė seka – CONTENT_AGENT → DATA_AGENT → CODING_AGENT → CODE_REVIEW_AGENT → QA_AGENT. Išvestį naudoja CONTENT/DATA/CODING. Sekos aprašas: `USER_JOURNEY_AGENT.md` (šis pipeline).
...
**Specialūs mišrūs maršrutai (sutrumpintai):** žr. `AGENTS.md` §Mišri užduotis – pvz. **M9 practice-quest-intro** (Feature Doc `M9_PRACTICE_QUEST_INTRO.md`, GOLDEN §3.4e, journey-m9, skipOptional progress 5); **M13 gen → I2V** (Epic A 13.37 enhancement be Feature Doc; Epic B 13.47 `special` + `M13_I2V_CLIP_BUILDER.md`, eilė 13.4→13.47→13.5); **M16–18 TE-M1618** (Must + Should S1–S4 + C2 ✅; Could C1/C3–C5 won’t-now; registry `DIAGRAMU_M16_M18_REGISTRY`; overlay incl. table/embed; EN via `build:modules-en-m16-m18`, ne core; corporate18 = D3 Deferred).
...
**CHECKS reikalavimas:** Draudžiama rašyti tik „patikrinau – atitinka“. Privaloma nurodyti: kurį failą atidarėte, pagal kurį dokumentą tikrinote, ir 1–2 konkrečius pavyzdžius (pvz. „pagal PAPRASTOS_KALBOS_GAIRES – pakeista X → Y“). Jei CHANGES sąraše yra **diagramos failas** (pvz. `*Diagram*.tsx`, `*ProcessStepper*`), CHECKS **privalo** įtraukti schemų vizualinės patikros rezultatus (SCHEME_AGENT.md §5) – kiekvienas kriterijus su OK arba FAIL. **Interaktyvios Shell/process schemos:** desktop enlarge („Išskleisti schemą“) = **default OFF** (SCHEME §3.11) – CHECKS: nėra false affordance mygtuko, nebent opt-in dense static. **Kai keičiama skaidrių vizualizacija ir enlarge opt-in įjungtas:** CHECKS privalo įtraukti **vartotojo kelio** patikrą – ar skaidrėje React, ar modalas rodo tą patį turinį (ne klaidinantį statinį failą); žr. `docs/development/AGENT_VERIFICATION_NE_MELUOTI.md` ir SCHEME_AGENT.md §5.5. Pilna privalomo doc įsikrovimo lentelė pagal rolę – `.cursor/skills/orchestrator/SKILL.md` (Mandatory doc loading per role).
...
**Rolė:** Duomenų sluoksnio prižiūrėtojas. Full authoring SOT: `modules.json`, `glossary.json`, `tools.json`+`tools-en.json`; core profiliai: `modules-m1-m6.json`, `modules-m1-m9.json`, `glossary-m1-m6.json`, `glossary-m1-m9.json`, `tools-m1-m6.json`, `tools-m1-m9.json` (+ EN twins); EN overlay: `modules-en-m10-m12.json`; taip pat `promptLibrary.json`, `hallucinationRates.ts`. Tipai – `src/types/modules.ts`. UI keičia CODING_AGENT.
**Pilna spec:** Periodiškumas ir trigeriai – `docs/development/DATA_AGENT_DUOMENYS_ATNAUJINIMAS.md`; įrankiai – `docs/development/DATA_AGENT_TOOLS.md` (`audit:tools`).
...
**Pilna spec / seka:** `docs/development/USER_JOURNEY_AGENT.md`.
...
- **Moduliai 16–18 (Kodo inžinerija):** Authoring + TE Must + Should S1–S4 + C2 done; Could C1/C3–C5 won’t-now (`TODO.md` §1.2g closed). Registry `DIAGRAMU_M16_M18_REGISTRY.md` + overlay prieš naujus `image`/lab keys. S1 Feature Doc `M16_DIRECTION_PICKER_LAB.md`. Corporate18 = Wave D3 Deferred. Ne painioti su M7 viz 100–106.
...
- **Ekosistema M1–6 (spin-off):** SOT – `docs/ECOSYSTEM_MAP.md`; URL kodas – `src/constants/ecosystemUrls.ts`; registras – `sot_index.json` → `ecosystem`. Mišri užduotis: CONTENT (labels + funnel) → DATA (`spinoffCta` JSON + core sync) → CODING (touchpoints + `trackSpinoffClick`) → CODE_REVIEW → QA (§5f outbound smoke). Orkestratoriaus special case: `.cursor/skills/orchestrator/SKILL.md` §Ecosystem / spin-off.
