...
- **Po M3:** neužrakina M4. **ModuleCompleteScreen** = `secondary` CTA šalia Tęsti. **Katalogas:** M3→M4 hinge juosta (`ModulesReadyCheckStrip`) grid’e prieš `base-cycle-2` — next-step brolis (white/card, brand eyebrow, `primary` CTA, ne grey `secondary`, ne `accent`, ne virš katalogo, ne po coming-soon). Tik kai M3 baigtas, M4 nebaigtas, `quizCompleted === false`. Results = eiti toliau / remediation, ne vien deepen spin-off.
...
- **Bazė M1–6:** vienas track; po M1–M3 – tylesnė **subsection** „Kontekstas ir projektas (M4–M6)“ (`trackBaseCycle2*`), ne antras track. M3→M4 ready-check juosta (`shouldShowModulesReadyCheck`) įterpiama **prieš** tą subsection — ne po coming-soon.
...
- **Home retrieval:** Home nėra antras katalogas — hero CTA = vienintelis primary; kartojimas = antrinis text-link centruotoje grupėje (`flex justify-center`, ne palaidi `inline-flex` `text-center` tėve; raktas `home:recallCta`). Ne `RetrievalDueCard` kortelė. Katalogas owns next-step.
...
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tipografija              | Ar roles iš `typographyClasses` (GOLDEN §1 / DS 0.3.3), ne raw `text-[10px]` / `font-black`? H1 vienas (ModuleView); hero išimtys = `h2`/`metric`/`heroDisplay`. `npm run audit:typography`.                                                                                                                                                                         |
...
| Exclusive choice         | Ar Trumpas/Ilgas, journey fokusas ar decision lab naudoja `ChoiceControl` (GOLDEN §3.1b), ne inline dublikatą / `CTAButton`? Decision lab gali turėti **du** radiogroup'us (scenarijus + režimas); default selected = brand; be module-accent stack. **FAIL:** antras vizualus ChoiceControl (Shell stepper = `navMode="sr-only"`). `variant: choice` **tik 10.48**. |
| Lab color exception      | `interactive-control-lab` (10.26, M4/54): ar risk spalvos **tik** lab surface (GOLDEN §3.1c)? Stake chip + tekstas (ne tik spalva)? Dark contrast OK? Strip (3 chips, be empty) / timing juosta skaitoma mobile? Path ChoiceControl be `optionTone`?                                                                                                                 |
