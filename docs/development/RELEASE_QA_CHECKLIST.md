...
- [ ] **Moduliai 7–9 EN (lean + expansion):** `modules-en-m7-m9.json` — deep-merge overlay; merge kai `maxModuleId >= 7`. Patikra: `npm run audit:en-coverage-m7-m9` (lean) ir `npm run audit:en-coverage-m7-m9 -- --full`; validacija – `validateModulesEnM79()` per `npm run validate:schema`. Rankinė: M7 slide 70 journey EN, M8 test klausimai EN, M9 workflow 93–94 + hub 99.
