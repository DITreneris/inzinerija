...
4. `quiz-en.json`, `modules-en-us-overrides.json` (en-us variantas)
...
**Skriptai:** `scripts/build-en-m10-m12.mjs`, `extract-modules-m10-m12.mjs`, `audit-en-coverage-m10-12.mjs`, `sync-m10-12-multi-agent.mjs`.
...
2. **EN M10–12 merge be `maxModuleId >= 10` guard** → MVP/test profilyje modulių skaičius išauga (6→9+), lūžta testai.
3. **`SummarySlide` custom laukai** (pvz. `useCaseCatalog`) **nerenderinami** — naudoti `sections[]` struktūrą.
4. **M10+ keitimai sync'inami į `*-m1-m6.json`** — nereikia; core profiliai baigiasi moduliu 6 arba 9.
