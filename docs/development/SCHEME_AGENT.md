...
| **Modulis 10 – trigger, 3A, žmogaus kontrolė, taksonomija, medis, spec/incident, orkestratorius** | Shell: `AgentWorkflowBlock` (10.2, LMS etalonas density=hero + agentWorkflowLayout.ts), `M10ThreeAStrategyBlock` (10.25, horizontal 100% stacked 80/15/5 + `m10ThreeAStrategyLayout.ts`), `M10TriggerFlowBlock` (10.15), `M10LearningLoopBlock` (10.49, 4 makro), `M10OrchestratorBlock` (10.482), `M10WorkflowSpecBlock` / `M10IncidentPlaybookBlock` (10.65), `M10ToolDecisionTreeBlock` (10.4, HitArea+StepNav), `M10AgentTaxonomyBlock` (10.45, Pattern `dual-taxonomy` Type Etalon W5, 8-step decision spine). **Lab (no Shell):** `M10HumanControlSimulatorBlock` (10.26, `m10_human_control_simulator`, Pattern `interactive-control-lab`) – `m10HumanControlContent.ts` + `m10HumanControlModel.ts` + `m10HumanControlLabTokens.ts` (W1.1 risk strip). M12 `m12_multi_agent_schema` – feedback edge evaluator→coordinator. Content: `m10DiagramContent.ts` + `m10*Layout.ts`. `M10SpecIncidentBlock` – legacy. |
...
**LMS polish (1A):** ambicija = mokymų platformos gestalts (ne premium SaaS redesign). Pilot etalonas M10.2 – `docs/development/LMS_DIAGRAM_POLISH_10_2.md`; Type Etalon W1–W3 + W4 residual caption tokens + **W5 dual-taxonomy** (`m10_agent_taxonomy`) – track **complete**; vykdymo taisyklės + pamokos – `.cursor/skills/scheme-agent/` (`SKILL.md` §LMS polish, `lessons.md`).
...
Naujai Modulio 10 diagramai (pvz. `m10_agent_taxonomy`):

1. **Labels LT/EN** – `m10DiagramContent.ts` (`getM10*Labels(locale)`); CONTENT_AGENT patvirtina semantiką.
2. **Geometrija** – `*Diagram.tsx` (SVG, layout konstantos; precedentai: `M10ThreeAStrategyDiagram`, `M7ThreeAgentsBlock`).
3. **Wrapper** – `*Block.tsx` + `EnlargeableDiagram` (mobileBehavior, enlargeLabel).
4. **Routing** – `diagramRenderers.tsx` (preferuojama procesinėms schemoms) arba `ContentSlides.tsx` legacy; export `shared/index.ts`.
5. **JSON** – `modules.json` — `sections[].image`; EN overlay — `modules-en-m10-m12.json` (tik `image` + caption, ne labels).
6. **Registry** – įrašas `DIAGRAMU_M7_M12_REGISTRY.md` prieš merge.
...
Kai registry **Pattern** = `interactive-control-lab` ir **Shell** = Ne (pvz. M10 `m10_human_control_simulator` / skaidrė 10.26):

1. **Registry** – Pattern = `interactive-control-lab`, Shell = Ne, render kelias = `*Block` (be `*Diagram` SVG).
2. **Routing** – tik `diagramRenderers.tsx` (ne ContentSlides special-case).
3. **UI** – `ChoiceControl` (+ optional `optionTone`) + `CopyButton`; **risk strip** (3 scenarios) + mode timing strip + 2 trade meters; be SVG / `DiagramStepHitArea` / Enlarge-as-default.
4. **Labels** – atskiras `*Content.ts` (LT/EN getters); deterministinės taisyklės – `*Model.ts` (ne layout geometrija); risk spalvos – `*LabTokens.ts` (GOLDEN §3.1c).
5. **Lab visual (W1 / W1.1)** – gradient lab shell; **risk strip** (3 scenarios, no empty cell); stake chips (brand/amber/rose); timing juosta pagal `errorTiming`; fit emerald / mismatch amber / error rose; recommendation tik po režimo; 2 trade meteriai (greitis + krūvis). Mikro-motion. Be purple glow / run-agent sim.
6. **Netaikyti** LMS shell / `density=hero` / HitArea checklists – tai ne proceso schema. Chrome/a11y → CODING + UI_UX; semantiką → CONTENT.
...
6. **Registry** – `DIAGRAMU_M13_M15_REGISTRY.md` prieš JSON.
...
- **markerUnits:** process markeriai su `stroke.flow` / `flowStrong` (≥ 3.5) **privalo** `markerUnits="userSpaceOnUse"` (`DIAGRAM_TOKENS.arrow.markerUnits`). SVG default `strokeWidth` scaling išpučia antgalį (`markerLen × stroke`) ir praryja kotą (GAP ≈ 24–32 → lieka pliki trikampiai).
