...
| **Modulis 10 – trigger, 3A, taksonomija, medis, spec/incident, orkestratorius** | Shell: `AgentWorkflowBlock` (10.2), `M10TriggerFlowBlock` (10.15), `M10LearningLoopBlock` (10.49, 4 makro), `M10OrchestratorBlock` (10.482), `M10WorkflowSpecBlock` / `M10IncidentPlaybookBlock` (10.65). Static: `M10ThreeAStrategyBlock` (10.25), `M10ToolDecisionTreeBlock` (10.4, HitArea+StepNav), `M10AgentTaxonomyBlock` (10.45). M12 `m12_multi_agent_schema` – feedback edge evaluator→coordinator. Content: `m10DiagramContent.ts` + `m10*Layout.ts`. `M10SpecIncidentBlock` – legacy.                                                                                                                    |
...
4. **Routing** – `ContentSlides.tsx` — `section.image.includes('m10_…')`; export `shared/index.ts`.
