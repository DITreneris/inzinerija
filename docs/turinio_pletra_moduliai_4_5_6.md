...
- **„Po šio modulio galėsi:“** (4 punktai, paprasta nauda – ne curriculum inventorius)
  1. Duoti DI savo šaltinius ir gauti atsakymus su nuorodomis.
  2. Struktūruoti gilesnes užklausas keliais žingsniais.
  3. Trumpinti promptus ir taupyti tokenus bei laiką.
  4. Tikrinti atsakymus – mažiau spėlionių ir haliucinacijų.
  - Tiltai į Modulį 6 – `aboutText` / pabaigos skaidrėse, ne outcome sąraše.
...
- **outcomes (4 punktai; paprasta nauda):**
  1. Duoti DI savo šaltinius ir gauti atsakymus su nuorodomis.
  2. Struktūruoti gilesnes užklausas keliais žingsniais.
  3. Trumpinti promptus ir taupyti tokenus bei laiką.
  4. Tikrinti atsakymus – mažiau spėlionių ir haliucinacijų.
...
**Kelio pasirinkimas (diferenciacija, PEDAGOGINE_ANALIZE §2.8):** Po reveal – du mygtukai **Trumpas kelias** / **Ilgas kelias** (`howToUseModule.short` / `.full`), ne curriculum ID sąrašas. **Trumpas** = Fast track ON (praleidžia `optional` skaidres); **Ilgas** = visos skaidrės. Sinchronizuojasi su header „Fast track“. Optional gilesnės temos: id **59** (atviros DB), **65** (promptų sekos) + esami bonusai (46, 46.5, 55, 61). UI neeksponuoja SOT kodų (`4.2-open`, `4.3a`).
...
| **4.1-prompts**       | **Kada metodinis, kada agentinis?** (id 54)                            | **MUST lab** `m4_prompt_mode_simulator` (`interactive-control-lab`, Shell: Ne): režimas pagal **gyvi duomenys × metodikos griežtumas** (ne brandos kopėčios iki agentų). 3 scenarijai × 4 režimai; match/mismatch; klaidos atvejis; kopijuojama taisyklė → RAG / M6. Optional collapsible šablonai. Vieta: po įrankių bloko, prieš Custom GPT.                                                                                                                                                           | Workflow, promptų tipai, įrankiai                                                                               |
...
| **4.2a-academic**     | **DI įrankiai, kurie taupo jūsų laiką informacijos paieškai**          | Sprendimo atmintinė: `toolChoiceBar` + lentelė „Kada ką?“ (6 įrankiai) + tipinė eiga lentelė; Trumpai → Daryk → Perplexity starteris → Patikra; tools kortelės collapsible. Sync M7/71.35. Verslas (ne akademinis) tonas.                                                                                                                                                                                                                                                                                | RAG (4.2), Input (šaltiniai), Quality (citatos)                                                                 |
...
| **RLHF**                   | Reinforcement Learning from Human Feedback – paskatinamasis mokymas su žmogišku grįžtamuoju ryšiu; naudojamas formuoti „geresnių“ atsakymų strategijas DI modeliuose.                                    |
...
- **Struktūra – pirmiausia orientacinis blokas, po to veiksmo modelis:** 0. **Kodėl pradedame nuo Dantės?** (terms) – Svarbu suprasti, kad DI susideda iš daugybės hierarchinių lygmenų – tai padės vėliau suvokti, kur „telpa“ šaltiniai ir kontekstas.
...
- **Seka (prioritetas: schema pirmiausia, skalės kaip iliustracija):** Trumpai → **Kaip veikia LLM?** (esama schema – kodėl kontekstas svarbus) → Apibrėžimas / Ką sudaro → Iliustracija (InstructGPT su įvadu) → Esmė / Kuo padeda → Daryk dabar (copyable) → Veiksmas / Šaltiniai.
- **Blokai (pedagogika, aiškumas, įrodymas, CTA):**  
  ...
  2. **Kaip veikia LLM?** (brand, `image: llm_autoregressive`) – **Interaktyvus blokas** (LlmAutoregressiveBlock): **8 žingsnių** state-driven diagrama (4 pirmo lygio: N Įvestis→LLM→Išvestis→Pasirinkta; 4 antro lygio: N+1 tie patys blokai), „Tu esi čia“, mygtukai 1–8 + Atgal/Pirmyn, paaiškinimai apačioje. Diagrama rodoma tiesiogiai (be „Peržiūrėti visą dydį“ modalo). Turinys: žodis po žodžio, kiekvienas priklauso nuo viso ankstesnio konteksto – todėl gerai suformuotas promptas (vaidmuo, formatas) sumažina klaidas. Vieta: iškart po Trumpai, kaip pagrindinė „kaip veikia“ vizualizacija. (Statinis `llm_autoregressive_rytas_zalgiris.svg` – atsarginė nuoroda; UI rodo React komponentą per `diagramRenderers` registry.)
...
- **1. Trumpai (accent, pirmoji sekcija):** Per 1–2 min gausi DI įvertinimą savo promptui pagal 4 kriterijus (tikslas, kontekstas, struktūra, formatas) ir 1–2 patarimus. Nukopijuok promptą žemiau ir įklijuok savo – iš karto atsakymas. (GOLDEN_STANDARD §3.2 – Trumpai viršuje.)
...
- **Pipeline diagrama (vietoj dviejų PNG):** Po sekcijos „Kodėl čia?" rodoma **viena progresinė schema** (`ContextEngineeringPipelineDiagram`). Antraštė: **„Nuo prompto prie konteksto: vienas pipeline, du keliai"**. JSON laukas: `content.pipelineDiagram = "context-engineering"`. Mazgai: Vartotojo tikslas → Prompt → **Kontekstas** → LLM → **Įrankiai / Duomenys** → Output. Brand (navy) = Promptų inžinerijos kelias; emerald = Konteksto inžinerija prideda. LT toggle: **Promptų inžinerija** / **Konteksto inžinerija**; EN: Prompt engineering / Context engineering. **UI (2026-07):** Prompt režime 3 ir 5 = dashed placeholder slotai (`+ kontekstas` / `+ įrankiai`), ne permatomi box’ai; Context režime – pilni emerald mazgai. Takeaway apačioje: **„Geri rezultatai atsiranda ne iš geresnio prompto, o iš geresnio konteksto."** Schemai **nereikia** „Peržiūrėti pilname dydyje".
...
- **Sutampa su oficialiais šaltiniais (skaidrėje arba collapsible „Šaltiniai“):**
  ...
  - **OpenAI (platform.openai.com):** struktūruotos instrukcijos, specifiškumas, pavyzdžiai, progresyvus požiūris (zero-shot → few-shot) – atitinka **Inžineriją** ir **Komunikaciją**.  
    _Pastaba:_ Oficialūs vadovai nenaudoja būtent „4 dedamųjų“ – tai pedagoginis rėmas, kuris suderinamas su jų gairėmis (inžinerija = projektavimas/iteracija; kalba = kontekstas/reikšmė; psichologija = įrėminimas; komunikacija = aiškumas, žmogaus–DI sąveika).
...
- **Praktika:** skaidrėje – kopijuojamas **vertinimo promptas** (vienas copyable), label žmogui: „Įvertinti savo promptą (5 principai) – nukopijuokite ir paleiskite“. practicalTask = tik žingsniai (evaluator-prompt-block, GOLDEN §3.2b). Optional blockVariant: terms; „Toliau“ konkretus: Skaidrė 48 (RL/RLHF). **Integruota praktika „pataisyk promptą“:** content.correctPromptPractice – intro, blogas pavyzdys, textarea (vartotojas rašo pataisytą variantą), mygtukas „Parodyti sprendimą“, paslėptas sprendimas (Kokie principai pažeidžiami?, Pataisytas variantas copyable, Kas pasikeitė?). Buvusi atskira skaidrė 49.5 sujungta į 49. Prompt library 6 skirsnis: Flagship „Prompto kokybės patikrinimas (pagal 5 principus)“ + Micro `after-eval-one-action` (vienas klausimas po įvertinimo — ką keisti darbe per 24–48 val.).
...
- **1. Įvairių stilių ir formatų tekstų kūrimas**
  - „Parašyk oficialų kvietimo tekstą į verslo renginį, kuris vyks kitą mėnesį.“ _(oficialus / formalus stilius)_
  - „Sukurkite formalią sutartį su klientu, parduodu [produktas].“ _(formalus / teisinis stilius)_
  - „Parašykite skelbimą „ieškome darbuotojo“, naudojant entuziastingą toną.“ _(entuziastingas / įtraukiantis stilius)_

- **2. Atsakymai į klientų el. laiškus**
  - „Parašykite mandagų atsakymą klientui, kuris skundžiasi dėl pavėluoto pristatymo.“ _(mandagus, atsiprašantis, problemų sprendimas)_
  - „Parašykite profesionalų atsakymą klientui, kuris teiraujasi apie jūsų paslaugų kainas ir nuolaidas.“ _(profesionalus, informatyvus)_
  - „Parašykite pagarbų atsakymą klientui, kuris domisi grąžinimo politika jūsų internetinėje parduotuvėje.“ _(pagarbus, aiškiai aprašanti politika)_

- **Pastaba UI:** Skaidrėje – antraštė „PRAKTINĖS UŽDUOTYS“; dvi kategorijos (Įvairių stilių tekstų kūrimas | Atsakymai į klientų el. laiškus); po kiekvienu punktu – CopyButton. _Vieno puslapio HTML tinklalapio kūrimo užduotis – Modulyje 6 (žr. §4)._
...
**Kada metodinis, kada agentinis?** (skaidrė id **54**, 4.1-prompts) – turinys (kopijuojamas į UI/JSON):

- **Tezė:** agentinis **nėra** brandos kopėčios virš metodinio. Režimą rink pagal **gyvi / išoriniai duomenys × metodikos / formato griežtumas**.
- **Chrome (GOLDEN §3.2):** Trumpai (`accent`) → Daryk dabar (`brand` + lab) → Patikra (`accent`). Optional collapsible „Pavyzdžių šablonai“ su 2 copyable (ne lab sienos dalis).
- **Simuliatorius (MUST):** `sections[].image` = `m4_prompt_mode_simulator`; Pattern `interactive-control-lab`; Shell: **Ne**. Axis strip (3 scenarijai, be empty cell) + `ChoiceControl` (scenarijus × režimas) → rekomendacija / neatitikimas **tik po** režimo → timing juosta → 2 trade meteriai (greitis, haliucinacijos rizika) → „Patikrink klaidos atvejį“ → **kopijuojama režimo taisyklė** (režimas, sąlyga, patikra, kitas žingsnis → RAG / M6). Failai: `M4PromptModeSimulatorBlock`, `m4PromptModeModel.ts`, `m4PromptModeContent.ts`, `m4PromptModeLabTokens.ts`.
- **3 scenarijai:** `known_swot` (metodinis) · `live_top10` (agentinis) · `gather_then_analyze` (agentinis → metodinis).
- **4 režimai:** `methodical` · `agentive` · `sequence` · `vague_ask` (niekada rekomendacija – mokomasis silpnas režimas).
- **Patikra:** Ar yra užduotis, kuriai neužtenka laisvo klausimo? Ar taisyklėje režimas + sąlyga + patikra?
- **Carry:** RAG / Deep research užrašai + Modulio 6 projekto planas.
- **Lab UI:** GOLDEN §3.1c + W1.1 (axis strip; stake = duomenų / haliucinacijos rizika, ne finansinė „kritinė“ rose).
...
- **Schema sekcija (JSON, skaidrė id 43):** `image: strukturuotas_procesas` → `StrukturuotasProcesasBlock` (3 žingsniai, interaktyvus; ne legacy `strukturuotas_procesas_3_zingsniai`).
...
- **🎯 Universalus promptas (veikia su Gamma / Prezent / Canva):**  
  _„Sukurk 8 skaidrių prezentaciją tema [TEMA]. Tikslinė auditorija: [AUDITORIJA]. Stilius: profesionalus, aiškus, be perteklinių tekstų. Įtrauk: – struktūrą (problema → sprendimas → nauda), – 2 infografikus, – 1 vizualinį akcentą, – aiškų „key takeaway“ pabaigoje. Naudok mūsų prekės ženklo spalvas: [SPALVOS].“_
...
- **Trumpai (accent):** Diagrama rodo: įvestis → DI → išvestis; DI gali paimti duomenis iš tavo dokumentų ar atminties. Kitame skyriuje išmoksite, kaip tai panaudoti (RAG).
...
- **2. Agentinė vizualizacija (brand):**  
  ...
  image: „llm_arch“, imageAlt: „DI agentinė sistema: režimai Bazinis, RAG, Įrankiai“.
...
- **0. Vienos eilutės rėmas (privaloma)**
  ...
  - Naudoti kaip Trumpai arba viršutinį takeaway skaidrėje (accent blokas).
...
- **1. Oficialūs atviri duomenų šaltiniai – 3 blokų architektūra (būtina sustruktūruoti, kad skaidrė neperdėtų ir būtų aišku, kada ką naudoti)**
  ...
  **🔵 A. ES lygmuo**
  - **Eurostat** (ec.europa.eu/eurostat) – ES statistikos: BVP, užimtumas, prekyba, demografija, regionai. API ir atsisiuntimai. _(Mini vizualas 🇪🇺 Eurostat – rekomenduojama.)_
  - **data.europa.eu** – ES atvirų duomenų portalas: įstatymai, ataskaitos, datasetai iš įstaigų ir valstybių. _(Mini vizualas 🇪🇺 data.europa.eu – rekomenduojama.)_
  - **OECD** – tarptautinė lyginamoji statistika ir rodikliai.
  - _data.gov (JAV) / World Bank Open Data_ – collapsible „Kiti tarptautiniai šaltiniai“ (Baltijos/ES auditorijai ne pirmo plano).
...
- **4. Kopijuojami mini-promptai (RAG + atviri duomenys)**
  ...
  - **Struktūruotas „atvirų šaltinių sąrašas“ (RAG orientuotas, ne generinis):**  
    _Blogas variantas:_ „Sudaryk atvirų šaltinių sąrašą Lietuvoje.“  
    _Geras variantas (copyable):_ „Sudaryk oficialių Lietuvos atvirų duomenų šaltinių sąrašą, tinkamų RAG analizei. Suskirstyk į kategorijas: statistika, teisė, verslas, finansai, viešieji pirkimai. Prie kiekvieno nurodyk: instituciją, nuorodą, kokio tipo duomenys prieinami, ar turi API.“ – tai jau metodika.
...
3 - Lentelė: **Situacija** | **Rekomenduojamas sprendimas** | **Kodėl?**. Pavyzdžiai: „Reikia bendro verslo konteksto“ → DI atmintis; „Reikia analizuoti skaičius“ → dokumentai; „Teisinis tikslumas“ → tik oficialūs šaltiniai, ne DI atmintis.
...
- **Pedagoginis įrankis (UI):** sekcija „Kada ką?“ – `toolChoiceBar` („Ką darai dabar?“) + `section.table` (`solutionMatrixStyle`: Situacija | Įrankis | Įvestis → rezultatas | Laikas). Tipinė eiga – atskira lentelė (ne collapsible). GOLDEN: Trumpai → Daryk → Starteris (Perplexity copyable) → Patikra. Tools kortelės – `toolsCollapsible`. Ta pati struktūra M7/71.35 (kontekstas: Duomenų analizės kelias).

- **Tipinė eiga (lentelė):**
  - (1) Šaltiniai (Perplexity) 5–10 min → (2) PDF (PaperGuide / Elicit) 10–15 min → (3) Citavimo patikrinimas (Scite) ~5 min → (4) Sintezė (Elicit) 10–15 min. Pilna eiga vienai temai – dažnai 30–45 min; Consensus ir Connected Papers – mokslo gyliui.

- **Pastaba UI:** content-block: whyBenefit, sections (sprendimo lentelė + eiga + Daryk/Copy/Patikra), tools collapsible apačioje. Accent: Trumpai + Patikra.
...
- **Pavadinimas:** PRAKTINĖS UŽDUOTYS.
- **Sekcijų schema (GOLDEN_STANDARD §3.2):** Trumpai (accent) → Kada to reikia? (brand) → Daryk dabar (brand) → Universalus šablonas (copyable) → 3 šablonai – 3 situacijos (terms, collapsible) → Esmė (accent).
...
- **Skaidrė „Tokenizavimas“ – encoder pavyzdys ir apibrėžimas (kopijuojamas į UI/JSON):**
  - **Kairė: GPT token encoder and decoder.**
    ...
    - _Pastaba:_ 198 dažnai = tarpas arba eilutės pabaiga. Subžodžiai (per+ro, man+zan+as) rodo, kad vienas žodis gali būti keli tokenai.
  - **Dešinė – apibrėžimas (geltonas blokas, didžiosiomis):**
    ...
    - DI SUPRANTAMA
    - KOMANDA
    - _(Šaltinis: natūralią kalbą paverčia į DI suprantamą komandą.)_
  - _Vizualizacija:_ atitinka `tokenization.png`; skaidrėje – šis turinys arba interaktyvus „encoder“ blokas (spalvoti segmentai + ID).
...
- **Skaidrė „Promptingo patarimai“ – tokenų biudžeto valdymas (kopijuojamas į UI/JSON):**
  ...
  - **7 patarimai (numeruoti, svarbūs žodžiai paryškinti oranžina), kiekvienas su ryšiu su tokenais:**
    1. **TURĖKITE STRUKTŪRĄ** – Suplanuokite savo promptus ir atsakymus, kad jie atitiktų **tokenų ribas**. _(Tokenų biudžetas: planuojant įvestį ir laukiamą išvesties ilgį neviršijate konteksto lango.)_
    2. **SKAIDYKITE** – Temas į potemes, kurios gali būti sprendžiamos pagal **tokenų biudžetą**. _(Mažesnės užduotys = mažesnis vienos žinutės apimtis = lengviau tilpti į ribas.)_
    3. **KURKITE TIKSLIUS PROMPTUS (GREITUS TIKSLINIMUS)** – **Taupysite tokenus.** _(Trumpas, aiškus prašymas = mažiau įvesties tokenų, mažiau nereikalingo atsakymo.)_
    4. **APIBENDRINKITE** – Prašykite dar kartą apibendrinti ir nurodydami svarbią informaciją tolesniuose raginimuose. _(Santraukos sumažina kontekstą – ilgame pokalbyje „sena“ istorija pakeičiama trumpa santrauka, taip taupant tokenus.)_
    5. **TIKRINKITE IR REDAGUOKITE** – Visada patikrinkite, ar segmentuotas arba apibendrintas tekstas atitinka pradinę reikšmę. _(Kokybiškas apibendrinimas = neprarandate prasmės, taupydami tokenus.)_
    6. **NAUDOKITE „SVERTO“ METODĄ** – Naudokite DI galimybes sukurti kontūrus arba juodraščius, kuriuos galima toliau tobulinti rankiniu būdu. _(Vienas trumpas promptas „sukurk planą“ vietoje ilgo iteravimo = mažiau tokenų.)_
    7. **NAUDOKITE PROJEKTUS, AGENTUS, GPT** – Valdysite tokenų ekonomiją, pvz. „srautinį perdavimą“ atliekant svarbias užduotis. _(Struktūruotos sesijos / specializuoti įrankiai padeda riboti ir matuoti sunaudojimą.)_
...
- **Skaidrė „Sisteminis svertas“ – koncepcija aiškiai, praktiškai, pritaikomai (kopijuojamas į UI/JSON):**
  ...
  - **Aiškiai – pagrindinis principas (mėlyna rombas / bullet):**
    ...
    - _Ryšys su tokenais:_ vienoda struktūra = mažiau „fluff“, greitesnis tikslinimas = taupomi tokenai ir laikas.
  ...
  - **Pritaikomai – pavyzdys (ikona: segtukas), copy-paste:**
    - **Pavyzdys:**
      ...
      - Ilgis: 100 žodžių“
    - _Pritaikymas:_ tą patį šabloną galima naudoti kitai temai (pvz. „Tema: Žalioji energetika“, „Ilgis: 150 žodžių“) – koreguoji tik reikiamus laukus.
...
- **Skaidrė „Tekstų formatavimas“ – praktiniai patarimai ir trumpi promptai (kopijuojamas į UI/JSON):**
  ...
  - **5 punktai (patarimas + trumpas promptas prie kiekvieno):**
    ...
    | **Stilius skaitomumui**                       | DI gali pasiūlyti **paryškinti**, _kursyvą_, <u>pabraukimą</u> pagrindinėms mintims ar skyriams.             | „Paryškink pagrindines mintis **pusjuodžiu**, skyrių antraštes – ##.“           |
  ...
  - **Tarpinės užduotys atlikimui – Lentelių kūrimas (3 užduotys + lygiavimas):**
    - **1. Analizė.**  
      **Promptas (kopijuojamas):** „Sukurk lentelę, skirtą lyginti Lietuvos Seimo rinkimų 2000–2025 metų duomenis. Stulpeliai: „Metai“, „Partijų skaičius“, „Rinkimų laimėtojas“ ir „Ministras pirmininkas“. Užpildyk eilutes atitinkamais duomenimis.“  
      _Tikslas:_ lyginamoji analizė su istoriniiais duomenimis; reikalauja konkretaus, struktūruoto išvedimo.
    - **2. Duomenys.**  
      **Promptas (kopijuojamas):** „Sukurk lentelę, kuri parodytų elektroninės kosmetikos parduotuvės pardavimus: prekė, gamintojas, kaina, kiekis, nuolaida, kaina su nuolaida, viso.“  
      _Tikslas:_ komercinių transakcijų suvestinė; kelios skaitinės ir kategorinės reikšmės.
    - **3. Tvarkaraštis.**  
      **Promptas (kopijuojamas):** „Sukurk [profesija] savaitės darbotvarkę su stulpeliais „Diena“, „Rytas“, „Popietė“ ir „Vakare“. Užpildyk tvarkaraštį su svarbiausiomis veiklomis [n+1] per savaitę.“  
      _Tikslas:_ laiko pagrindu struktūruota informacija; pritaikoma bet kuriai profesijai (pakeičiame [profesija] ir [n+1]).
    - **Lygiavimas (Markdown):**  
      ...
      **Pavyzdys sintaksės:**  
      ...
      `|:------|-------:|`  
      _(„Metai“ – kairėje, „Partijų skaičius“ – dešinėje.)_
...
- **Pažengusi praktika – ilgo dokumento workflow (PAVYZDŽIAI):**
  ...
  - **Žingsniai (atitinka skaidrės „PAVYZDŽIAI“ punktus):**
    1. **PATEIK STRUKTŪRĄ.**
       ...
       - _Ryšys su tokenais:_ iš anksto planuojama, kur „sunaudosime“ daugiausia tokenų.
    2. **SUGENERUOK TEMŲ IR POTEMIŲ PLANĄ.**
       ...
       - _Ryšys su tokenais:_ planas trumpesnis nei pilnas tekstas, bet leidžia kontroliuoti apimtį.
    3. **APIBENDRINK (MAX_TOKENS).**
       ...
       - _Ryšys su tokenais:_ naudojamas aiškus ribojimas (žodžiai / max_tokens), kad dalis dokumento netaptų per ilga.
    4. **SUKURK ŠABLONĄ.**
       ...
       - _Ryšys su tokenais:_ šablonas = mažai tokenų, vėliau galima pildyti po dalį.
    5. **PATEIK PROJEKTO PLANĄ PAGAL GERIAUSIAS PRAKTIKAS.**
       ...
       - _Ryšys su tokenais:_ planuojama, kada daryti santraukas ir „sutrumpinti istoriją“, kad DI nepamirštų svarbios informacijos.
    6. **VIZUALIZUOK.**
       - Promptas: „Vizualizuok šį procesą (workflow) paprasta schema: žingsniai 1–6 ir kur sunaudojami/tausojami tokenai.“
       - _Ryšys su tokenais:_ schema sustiprina supratimą, kad tokenų ekonomika = procesų valdymas.
...
- **Skaidrė „Kaip mažinti haliucinacijas? – 5 praktinės taisyklės“ (4.6) – turinys (kopijuojamas į UI/JSON):**  
  ...
  - **5 strategijos (kiekviena: antraštė + trumpas paaiškinimas / instrukcija DI):**
    1. **„Jei neaišku – klausk“ protokolas.**  
       _Instrukcija DI:_ „Jei mano užklausa neaiški ar trūksta duomenų, pateik 1–3 patikslinimo klausimus. Nepridėk detalių, kurių nenurodžiau.“
    2. **Atskirk faktus nuo spėjimų.**  
       _Instrukcija DI:_ „Atskirk FAKTUS (paremti duomenimis) ir SPĖJIMUS (prielaidos, interpretacijos). Paženk SPĖJIMAS.“
    3. **Neprigalvok duomenų.**  
       _Instrukcija DI:_ „Nekurk skaičių, datų, procentų, teisinių nuorodų ar statistikos. Jei jų reikia, pasakyk, kad informacijos trūksta.“
    4. **Jei nesi tikras – pasakyk.**  
       _Instrukcija DI:_ „Jei dėl informacijos nesi tikras, parašyk „nesu tikras“ ir nurodyk neapibrėžtumą. Nespėliok.“
    5. **Naudok tik tai, kas pateikta.**  
       _Instrukcija DI:_ „Atsakyk tik remdamasis mano pateikta informacija. Jei reikia išorinių duomenų – pirmiausia paklausk.“
...
**Skaidrė „InstructGPT: instrukcijų laikymasis“ (4.8) – turinys (archyvas; sujungta į skaidrę 4.1a id 44):**

> **Pastaba (2026-07):** Atskira skaidrė 4.8 nebėra UI; LLM schema ir InstructGPT įrodymas – skaidrėje **44** (žr. §4.1a aukščiau). Žemiau – istorinis 4.8 aprašas referencijai.
...
- **Struktūra (sekcijų tvarka):**
  ...
  2. **Kaip veikia LLM?** (brand) – interaktyvus `LlmAutoregressiveBlock` (`image: llm_autoregressive`); statinis `llm_autoregressive_rytas_zalgiris.svg` – tik atsarginė nuoroda. Body: LLM generuoja tekstą žodis po žodžio; įvestis → LLM → tikimybės kitam žodžiui → pasirinktas tokenas; kiekvienas žodis priklauso nuo viso ankstesnio konteksto – todėl gerai suformuotas promptas sumažina klaidas.
...
- **Rezultatų slenksčiai:** panašiai kaip Modulio 2 (pvz. &lt;70% → pakartoti sprintą ir peržiūrėti 510 / 47; ≥70% → rekomenduojama eiti į praktiką, Modulis 6).
...
**Skaidrės 47–516 (Wave A+B, 2026-07-24):** Vienas Apply kelias be triplikacijos.

- **47** – brief, `preCopyCheckBlock`, 8 skaidrių struktūra, collapsible: tik **Master (struktūra)** + diagrama. **Be** pilno 6 blokų copyable.
- **47.2** – **Storyline** (sprendimas + 3 argumentai + CTA; antraštės = teiginys) + copyable + tema vs teiginys lentelė.
- **47.5** – **vienintelis** 6 blokų turinio promptas + **įrankio medis** (3 keliai: Gamma / Copilot / Canva; kiti – collapsible).
- **510** – timed sprinto šablonas (`practicalTask`) + `briefCheckBlock` + **soft proof**.
- **510.5** – **QC lab** (blogas vs geras fragmentas + 6 punktų checklist + copyable).
- **515** – JIT kortelės; **511** warm-up (4); **512–514** mini testas; **516** optional Rescue bonus po results.

Eilė: žr. [`MODULIO_5_SKAIDRIU_EILES.md`](MODULIO_5_SKAIDRIU_EILES.md).

**Artefakto atsisiuntimo sprendimas (2026-07):** Skaidrė 47 – copy-first (8 struktūra + Master). Modulio 5 PDF – skaidrė **514** (`handoutDownloadLabel`).
...
**Rekomenduojama skaidrių eilė (User Journey):** practice-intro (60) → **61 Tyrimo ataskaita** (pirmas ne-optional veiksmas) → 62 COMBO → 67.8 HTML grandinė → 68 Vieno puslapio HTML → 63 SUPER PROMPTAI → **65 Refleksija** → **65.5 Duomenų tvarkymo checklist (path-step)** → **64 Duomenų tvarkymas** → optional 66/67 (Custom GPT) → **69 Projekto santrauka** (`summary`). Soft completion: **1 iš 2** projektų (Tyrimo ataskaita **arba** Custom GPT) – be hard `minScenariosToComplete`. Oficiali eilė: [`docs/MODULIO_6_SKAIDRIU_EILES.md`](MODULIO_6_SKAIDRIU_EILES.md).
...
- **Žingsniai (instrukcijos)** – prie kiekvieno žingsnio pridėti **„Kada toliau:“**:
  1. META – rolė, tikslas, auditorija. _Kada toliau: kai META blokas užpildytas (rolė + tikslas), eik į INPUT._
  2. INPUT – duomenys (šaltiniai), apribojimai (RAG stilius). _Kada toliau: kai šaltiniai ir apribojimai nurodyti, eik į OUTPUT._
...
- **Pilnas pavyzdinis promptas (CopyButton):**  
  _„META: Tavo rolė – patyręs front-end programuotojas ir UX/UI dizaineris. Tikslas – vieno puslapio .html tinklalapis. INPUT: Tema – wingfoil bendruomenė Lietuvoje. Turinys: bendruomenės pristatymas, treniruotės, kontaktai. CTA – lankytojas užpildo kontaktinę formą. OUTPUT: Pilnas HTML su įterptu CSS viename faile, be komentarų. REASONING: 1) Pristatymo blokas, 2) Treniruočių sekcija, 3) Kontaktai ir forma – logiška seka. QUALITY: Tonas modernus, aiškus, draugiškas; mobilus, lengvai skaitomas. ADVANCED: Tik validus HTML+CSS kodas, be markdown.“_
