...
import M16VsrMaturityDiagram from './M16VsrMaturityDiagram';
import {
  getM16VsrMaturityChrome,
  getM16VsrMaturityExplanations,
} from './m16M18DiagramContent';

export default function M16VsrMaturityBlock() {
  const { locale } = useLocale();
  const loc = locale === 'en' ? 'en' : 'lt';
  const explanations = getM16VsrMaturityExplanations(loc);
  const labels = getM16VsrMaturityChrome(loc);
