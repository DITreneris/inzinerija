...
import M18DiffRitualDiagram from './M18DiffRitualDiagram';
import {
  getM18DiffRitualChrome,
  getM18DiffRitualExplanations,
} from './m16M18DiagramContent';

export default function M18DiffRitualBlock() {
  const { locale } = useLocale();
  const loc = locale === 'en' ? 'en' : 'lt';
  const explanations = getM18DiffRitualExplanations(loc);
  const labels = getM18DiffRitualChrome(loc);
