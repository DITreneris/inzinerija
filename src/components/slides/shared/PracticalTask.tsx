  ...
  return (
    <div className="mt-8 p-4 sm:p-6 bg-accent-50 dark:bg-accent-900/20 rounded-2xl border-l-4 border-accent-400 relative">
      ...
      {showCompleteFeedback && (
        <div className="mb-4 flex items-center gap-2 rounded-lg bg-emerald-100 dark:bg-emerald-900/40 border border-emerald-300 dark:border-emerald-700 px-4 py-2 text-emerald-800 dark:text-emerald-200 animate-fade-in" role="status" aria-live="polite">
          <CheckCircle className="w-5 h-5 shrink-0" />
          <span className="font-medium">{locale === 'en' ? 'Well done!' : 'Gerai padirbėta!'}</span>
        </div>
      )}
        ...
        <div className="mb-4 bg-white dark:bg-gray-800 border-2 border-accent-200 dark:border-accent-800 rounded-xl p-4 relative group">
          <div className="flex items-start justify-between gap-3 mb-2">
            <p className="text-sm font-semibold text-accent-800 dark:text-accent-200">
              {task.templateLabel || (locale === 'en' ? 'Copy prompt' : 'Kopijuoti promptą')}
            </p>
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex items-center justify-center w-8 h-8 rounded-full bg-brand-500 text-white font-bold text-sm" aria-hidden>
                        {step.step}
                      </div>
                      ...
                      <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg p-3 relative group">
                        <div className="flex items-center justify-between mb-2">
                          <p className="text-xs font-semibold text-emerald-800 dark:text-emerald-200 flex items-center gap-1.5">
                            ...
                            {locale === 'en' ? 'Partial solution (copy):' : 'Tarpinis sprendimas (kopijuoti):'}
                          </p>
      ...
      {task.instructions?.steps?.length === 6 && (() => {
        ...
        return (
          <div className="mb-3" role="status" aria-label={locale === 'en' ? 'Block completion progress' : 'Blokų užpildymo progresas'}>
      ...
      {(task.inputHint || task.template) && (
        <p className="mb-2 text-sm text-gray-600 dark:text-gray-400">
          {task.inputHint ?? 'Įvesk visą promptą į lauką žemiau. Jei šablone matai [skliaustelius] – įrašyk savo žodžius vietoje jų.'}
        </p>
      )}
      ...
      <div className="relative">
        <textarea
          ...
          aria-label={locale === 'en' ? 'Task answer field' : 'Užduoties atsakymo laukas'}
        />
      ...
      {hasDraft && !showSavedFlash && (
        <div className="flex items-center gap-1.5 mt-1 text-xs text-brand-600 dark:text-brand-400">
          ...
          <span>{locale === 'en' ? 'Draft saved – you can come back anytime' : 'Juodraštis išsaugotas – gali grįžti bet kada'}</span>
        </div>
      )}
      ...
      {!isTaskCompleted ? (
        <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3 mt-4">
          <p className="text-xs text-gray-500 dark:text-gray-400 order-2 sm:order-1">
            {locale === 'en' ? 'Answer is auto-saved' : 'Atsakymas automatiškai išsaugomas'}
          </p>
          <div className="flex flex-wrap gap-2 order-1 sm:order-2">
            <button
              ...
              aria-label={locale === 'en' ? 'Copy draft' : 'Kopijuoti juodraštį'}
            >
              {copied ? (
              ...
              {copied ? (locale === 'en' ? 'Copied!' : 'Nukopijuota!') : (locale === 'en' ? 'Copy' : 'Kopijuoti')}
            </button>
            <button
            ...
            {task.allowMarkWithoutAnswer && (
              <button
                ...
                aria-label={locale === 'en' ? 'Mark task done without text' : 'Pažymėti užduotį atlikta be teksto'}
              >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-4">
          <div className="flex items-center gap-2 text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-900/30 px-4 py-3 rounded-xl animate-fade-in">
            <CheckCircle className="w-5 h-5 shrink-0" />
            <span className="font-semibold">{locale === 'en' ? 'Task completed! 🎉' : 'Užduotis atlikta! 🎉'}</span>
          </div>
          ...
          {task.feedbackPrompt && (
            <div className="mt-4 p-4 rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-900/10">
              <p className="text-xs font-semibold text-amber-800 dark:text-amber-200 mb-2">{locale === 'en' ? 'What did you get from AI?' : 'Ką gavai iš DI?'}</p>
              <p className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap mb-2">{task.feedbackPrompt}</p>
              <CopyButton text={task.feedbackPrompt} size="sm" ariaLabel={locale === 'en' ? 'Copy feedback prompt' : 'Nukopijuoti atsiliepimo promptą'} />
            </div>
          )}
          ...
          {!isEditing ? (
            <div className="flex flex-wrap gap-2">
              <button
                ...
                disabled={!answer?.trim()}
                className="btn-secondary flex items-center justify-center gap-2 min-h-[44px] disabled:opacity-50 disabled:cursor-not-allowed"
                aria-label={locale === 'en' ? 'Copy prompt' : 'Kopijuoti promptą'}
              >
                {copied ? (
                  <CheckCircle className="w-4 h-4 text-emerald-600" />
                ) : (
              ...
              <button
                ...
                className="btn-secondary flex items-center justify-center gap-2 min-h-[44px]"
                aria-label={locale === 'en' ? 'Edit prompt' : 'Redaguoti promptą'}
              >
