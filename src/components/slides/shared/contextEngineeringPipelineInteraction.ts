...
const DEFAULT_LABELS_LT: InteractivePipelineContent['labels'] = {
  ...
  availableInContextModeOnly:
    'Šis žingsnis aktyvus tik Konteksto inžinerijos režime.',
};
...
const DEFAULT_LABELS_EN: InteractivePipelineContent['labels'] = {
  ...
  availableInContextModeOnly:
    'This step is active only in Context engineering mode.',
};
...
function getDefaultContent(locale: Locale): InteractivePipelineContent {
  ...
  const modes =
    locale === 'en'
      ? {
          prompt: {
            panelTitle: 'Prompt engineering path',
            panelDescription: 'Short path: goal → prompt → model → answer.',
          },
          context: {
            panelTitle: 'Context engineering path',
            panelDescription:
              'Full path: context and tools help get a more accurate answer.',
          },
        }
      : {
          prompt: {
            panelTitle: 'Promptų inžinerijos kelias',
            panelDescription:
              'Greitas kelias: tikslas → promptas → modelis → atsakymas.',
          },
          context: {
            panelTitle: 'Konteksto inžinerijos kelias',
            panelDescription:
              'Pilnas kelias: kontekstas ir įrankiai padeda gauti tikslesnį atsakymą.',
          },
        };
  return {
    labels,
    ...
    steps: nodes.map((node) => ({
      ...
      withContext: '',
      availableInModes:
        node.mode === 'context' ? ['context'] : ['prompt', 'context'],
    })),
  };
}
...
export function resolveInteractivePipelineContent(
  content?: InteractivePipelineContent,
  locale: Locale = 'lt'
): InteractivePipelineContent {
...
export function isStepAvailableInMode(
  step: InteractivePipelineStep,
  mode: InteractivePipelineMode
): boolean {
...
export function getStepById(
  content: InteractivePipelineContent,
  stepId: string
): InteractivePipelineStep | undefined {
  return content.steps.find((step) => step.id === stepId);
}

export function getFirstAvailableStepId(
  content: InteractivePipelineContent,
  mode: InteractivePipelineMode
): string {
...
export function getModeSummaryLabel(
  mode: InteractivePipelineMode,
  locale: Locale = 'lt'
): string {
