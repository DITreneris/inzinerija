...
export default function ContextEngineeringPipelineDiagram({
  className = '',
  interactiveContent,
}: Props) {
  ...
  const content = useMemo(
    () => resolveInteractivePipelineContent(interactiveContent, locale),
    [interactiveContent, locale]
  );
  const [activeStepId, setActiveStepId] = useState<string>(() =>
    getFirstAvailableStepId(content, 'prompt')
  );
  ...
  const hoveredStep = hoveredStepId
    ? getStepById(content, hoveredStepId)
    : undefined;
  const hoveredStepAvailable = hoveredStep
    ? isStepAvailableInMode(hoveredStep, mode)
    : false;
  const hoveredStepIndex = hoveredStep
    ? pipelineNodes.findIndex((node) => node.id === hoveredStep.id)
    : -1;
  const tooltipTopPercent =
    hoveredStepIndex >= 0
      ? (NODE_Y[hoveredStepIndex] / VB_HEIGHT_INTERACTIVE) * 100
      : 0;
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="relative rounded-xl shadow-[0_8px_24px_rgba(0,0,0,0.06)] border border-black/[0.04] dark:border-gray-700 overflow-hidden">
        {hoveredStep && hoveredStepIndex >= 0 && (
          ...
          >
            ...
            {hoveredStepAvailable
              ? hoveredStep.shortHint
              : content.labels.availableInContextModeOnly}
          </div>
        )}
        <svg
        ...
        >
          ...
          {/* ═══ Layer 1: underlay paths (po fono, po blokais) ═══ */}
          <g aria-hidden="true">
            {/* Prompt → LLM L-forma (context mode) – apeina Kontekstas */}
            <path
          ...
          {/* ═══ Layer 2: INPUT punktyrinis rėmas (be teksto) ═══ */}
          {isContext && (
            <g
              style={{
          ...
          {/* ═══ Layer 3: mazgai + stubos rodyklės ═══ */}
            ...
            return (
              ...
              >
                ...
                <rect
                  ...
                  onMouseLeave={() =>
                    setHoveredStepId((prev) => (prev === node.id ? null : prev))
                  }
                ...
                {i < pipelineNodes.length - 1 &&
                  (() => {
                    ...
                    const hideForLoopMode = isContext && isLlmToTools;
                    return (
                      <line
                        x1={CX}
                        y1={y + BOX_H}
          ...
          {/* ═══ Layer 4: overlay etiketės (po mazgų ir stubų) ═══ */}
          {isContext && (
            <g aria-hidden="true">
              <text
                x={ANNOTATION_LANE_X}
      ...
      {activeStep && (
        ...
        >
          ...
          <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3 lg:p-4 space-y-2">
            <h4 className="font-bold text-brand-900 dark:text-brand-100">
              {activeStep.title}
            </h4>
            <p className="text-sm text-gray-700 dark:text-gray-300">
              <span className="font-semibold">{content.labels.hint}:</span>{' '}
      ...
      >
        ...
        <div className="space-y-0.5">
          <p
            className={`text-xs font-bold uppercase tracking-wide ${isContext ? 'text-emerald-700 dark:text-emerald-400' : 'text-amber-700 dark:text-amber-400'}`}
          >
            {isContext ? uiLabels.keyRuleLabel : uiLabels.noteLabel}
          </p>
          <p
            className={`text-sm font-medium ${isContext ? 'text-emerald-900 dark:text-emerald-100' : 'text-amber-900 dark:text-amber-100'}`}
          >
