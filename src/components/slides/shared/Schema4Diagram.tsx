...
function getNodeFill(id: string): {
  ...
  if (id === 'retrieval')
    return { fill: ACCENT, fillOpacity: 0.95, stroke: ACCENT_STROKE };
...
export default function Schema4Diagram({
  className = '',
}: {
  className?: string;
}) {
  ...
  return (
    ...
    >
      <title id={`schema4-title-${uid}`}>
        Schema 4: multimodalinė įvestis ir atmintis. Pagrindinis srautas:
        įvestis → modelis → išvestis. Atminties sluoksnis: saugykla →
        informacijos gavimas (skaito) → kontekstas į modelį; pasirinktinis
        įrašymas iš modelio į saugyklą.
      </title>
      <defs>
      ...
      <g transform={`translate(${PAD_TRANSFORM}, ${PAD_TRANSFORM})`}>
        ...
        {SCHEMA4_NODES.map((node) => {
          ...
          return (
            <g key={node.id}>
              ...
              <text
                x={node.x + node.w / 2}
                y={node.y + (line2 ? 20 : node.h / 2 + 4)}
                textAnchor="middle"
                fontFamily={FONT}
                fontSize={node.id === 'model' ? 12 : 9}
                fontWeight="700"
                fill="white"
              >
                {line1}
        ...
        {SCHEMA4_EDGES.map((edge, i) => {
          ...
          const labelY =
            edge.label === 'Įrankiai' ? fromNode.y + fromNode.h + 8 : undefined;
