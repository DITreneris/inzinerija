/**
 * M4 – methodical vs agentic prompt-mode decision lab (not a DiagramKit walkthrough).
 * GOLDEN §3.1c interactive-control-lab; W1.1 – axis strip (no empty cell).
 */
...
import {
  ...
  getPromptModeScenarios,
  getPromptModeUiLabels,
  getPromptModes,
...
export default function M4PromptModeSimulatorBlock() {
  const { locale } = useLocale();
  const loc = locale === 'en' ? 'en' : 'lt';
  const ui = getPromptModeUiLabels(loc);
  const scenarios = getPromptModeScenarios(loc);
  const modes = getPromptModes(loc);

  const [scenarioId, setScenarioId] =
    useState<PromptModeScenarioId>('live_top10');
  const [selectedMode, setSelectedMode] = useState<PromptModeId | null>(null);
  ...
  const artefact = useMemo(() => {
    ...
    return formatPromptModeArtefact(
      loc,
      buildPromptModeRuleParts(scenarioId, selectedMode)
    );
  ...
  const scenarioTone = useMemo(() => {
    const map: Partial<Record<PromptModeScenarioId, ChoiceOptionTone>> = {};
  ...
  const modeTone = useMemo(() => {
    const map: Partial<Record<PromptModeId, ChoiceOptionTone>> = {};
  ...
  const statusHint =
    selectedMode == null
      ? loc === 'en'
        ? 'Pick a prompt mode to see the recommendation and your rule.'
        : 'Pasirink promptų režimą – pamatysi rekomendaciją ir taisyklę.'
      : isMismatch
  ...
  return (
    <div className={LAB_SHELL_CLASS} role="region" aria-label={ui.regionAria}>
      ...
      <AxisStrip
        ...
        axisCaption={`${ui.matrixLiveData} × ${ui.matrixStructure}`}
        noneLabel={ui.matrixNone}
        neededLabel={ui.matrixNeeded}
          ...
          {tradeOffs ? (
            <>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                />
                <TradeMeter
                  label={ui.hallucinationRisk}
                  valueLabel={ui.tone[tradeOffs.hallucinationRisk]}
                  tone={tradeOffs.hallucinationRisk}
                />
              </div>
            </>
          ) : null}
...
function AxisStrip({
  ...
  lowerLabel: string;
  higherLabel: string;
  stakeChip: { low: string; mid: string; high: string };
  scenarios: { id: PromptModeScenarioId; label: string }[];
  activeScenarioId: PromptModeScenarioId;
  onSelectScenario: (id: PromptModeScenarioId) => void;
}) {
  const items = getAxisStripItems();
  const labelById = Object.fromEntries(
    scenarios.map((s) => [s.id, s.label])
  ) as Record<PromptModeScenarioId, string>;
    ...
    <div className="rounded-xl border border-brand-200/70 bg-white/70 p-3 dark:border-brand-800/50 dark:bg-gray-900/40 sm:p-4">
      ...
      >
        {items.map((item) => {
          ...
          const stake = scenarioToStake(id);
          const active = id === activeScenarioId;
          const liveHint = item.liveData === 'needed' ? neededLabel : noneLabel;
          const structHint =
            item.structure === 'higher' ? higherLabel : lowerLabel;
          return (
            <button
              key={id}
              type="button"
