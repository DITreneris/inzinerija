...
function resolveColors(
  ...
  if (variant === 'ranking') {
    ...
    return {
      ...
      barColor: emphasis ? 'bg-brand-600 dark:bg-brand-400' : 'bg-brand-500 dark:bg-brand-400/80',

      numColor: emphasis ? NUM_COLORS.brand : 'text-brand-700 dark:text-brand-300',
...
export default function PortalHorizontalBarRow({
  ...
  const { barColor, numColor, barHeight } = resolveColors(variant, colorKey, emphasis);
    ...
    <div className={`flex items-center gap-3 ${emphasis ? 'py-0.5' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex justify-between gap-2 mb-1">
          ...
          <span className={`font-bold shrink-0 tabular-nums ${numColor}`}>{pct}</span>
