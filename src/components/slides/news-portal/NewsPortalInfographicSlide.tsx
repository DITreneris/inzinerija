...
export function NewsPortalInfographicSlide({
  ...
  const hasSidebar = editorialLayout && (heroSidebarTeasers?.length ?? 0) > 0;
  ...
  const showSecondaryCards =
    editorialLayout && Boolean(secondaryCards && secondaryCards.length === 2);
  ...
  const renderRibbons = (placement: Parameters<typeof promoRibbonsAt>[1]) =>
    promoRibbonsAt(promoRibbons, placement).map((ribbon, idx) => (
      <NewsPortalPromoRibbon key={`${placement}-${idx}`} ribbon={ribbon} />
    ));
  ...
  return (
    <div className={`w-full max-w-6xl mx-auto ${rootSpacing}`}>
      ...
      {editorialLayout && (
        <PortalChapterBreak
          id={PORTAL_SECTION_DEPTH}
          label={chapterDepthLabel}
        />
      )}
      ...
      {editorialLayout && (
        <PortalChapterBreak
          id={PORTAL_SECTION_CLOSE}
          label={chapterCloseLabel}
        />
      )}
      ...
      <footer className={PORTAL_FOOTER.wrapper}>
        <div>
          ...
          {footerSub && <div className={PORTAL_FOOTER.sub}>{footerSub}</div>}
        </div>
...
function SecondaryCardsGrid({ cards }: { cards: NewsPortalSecondaryCard[] }) {
  ...
  return (
    <div
      className={`grid grid-cols-1 lg:grid-cols-2 ${PORTAL_SPACING.gapCards} items-stretch`}
    >
      {cards.map((card, idx) => (
        <PortalBlockShell
          key={idx}
        ...
        >
          ...
          <div className="min-w-0">
            <PortalCardHeader label={card.sectionLabel} title={card.title} />
            ...
            {card.source && (
              <p className={`mt-1.5 ${PORTAL_TEXT.mutedXs}`}>{card.source}</p>
            )}
          </div>
        </PortalBlockShell>
      ))}
