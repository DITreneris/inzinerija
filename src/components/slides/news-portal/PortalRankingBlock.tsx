...
export default function PortalRankingBlock({
  toolsAndYouth,
}: PortalRankingBlockProps) {
  const tools = toolsAndYouth.tools ?? [];

  return (
    <PortalBlockShell
      variant={PORTAL_DEPTH_CARD_VARIANT}
      footerInsight={toolsAndYouth.toolsInsight}
    >
      <PortalCardHeader
      ...
      />
      <ol
        className="space-y-3 max-w-2xl list-none m-0 p-0"
        aria-label={toolsAndYouth.toolsTitle}
      >
        {tools.map((tool, i) => (
          <li key={i}>
