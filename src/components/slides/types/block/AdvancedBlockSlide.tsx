...
export function AdvancedBlockSlide({
  slide,
  onRenderTask,
}: AdvancedBlockSlideProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Hero */}
      {c?.heroTitle && (
        <Banner variant="info" className="p-5 rounded-xl">
            ...
            <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
              <div className="flex items-center justify-between text-sm mb-2">
                <span className="text-gray-600 dark:text-gray-400">
                  {c.temperatureScaleLeft}
                </span>
                <span className="text-gray-600 dark:text-gray-400">
                  {c.temperatureScaleRight}
                </span>
              </div>
              ...
              <table className="w-full text-base border-collapse">
                <thead>
                  <tr className="bg-amber-50 dark:bg-amber-900/20">
                    {c.cheatSheetHeaders?.map((h, i) => (
                      <th
                        key={i}
                        className="px-4 py-3.5 text-left border-b border-amber-200 dark:border-amber-800 align-top"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="text-gray-700 dark:text-gray-300">
                  {c.cheatSheetRows?.map((row, i) => (
                    <tr
                      key={i}
                      className={`border-b border-gray-200 dark:border-gray-700 ${i % 2 === 1 ? 'bg-gray-50 dark:bg-gray-900/50' : ''}`}
                    >
                      {row.map((cell, j) => (
                        <td
                          key={j}
      ...
      {/* Safe default */}
      {c?.safeDefaultValue && (
        <Banner variant="success" className="p-5 rounded-xl">
          ...
          <div className="bg-white dark:bg-gray-800 p-3 rounded-lg font-mono text-sm inline-block">
            <span
              ...
              dangerouslySetInnerHTML={{ __html: c.safeDefaultValue }}
            />
          </div>
        </Banner>
      )}
                    ...
                    <div className="pr-8 text-xs">
                      ...
                      {taskLines.map((line, li) => (
                        <div
                          key={`t${li}`}
                          className={
                            li === 0 ? 'text-gray-500 dark:text-gray-400' : ''
                          }
                        >
                          {line}
                        </div>
      ...
      {c?.ruleTemperatureLabel && (
        <Banner
          variant="info"
          className="!bg-violet-50 dark:!bg-violet-900/20 !border-violet-500 flex flex-wrap items-center gap-4"
        >
