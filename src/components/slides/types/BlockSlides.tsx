...
import {
  AdvancedBlockSlide,
  type AdvancedBlockSlideProps,
} from './block/AdvancedBlockSlide';
...
export function MetaBlockSlide({
  onRenderTask,
}: {
  ...
  return (
    <div className="space-y-6">
      ...
      <TemplateBlock
        label={t('blockMetaTemplateLabel')}
        template={t('blockMetaTemplate')}
      />
      {onRenderTask()}
    </div>
  );
...
export function InputBlockSlide({
  onRenderTask,
}: {
  onRenderTask: () => JSX.Element | null;
}) {
  ...
  return (
    <div className="space-y-6">
      ...
      <TemplateBlock
        label={t('blockInputTemplateLabel')}
        template={t('blockInputTemplate')}
      />
      {onRenderTask()}
    </div>
  );
...
export function OutputBlockSlide({
  onRenderTask,
}: {
  onRenderTask: () => JSX.Element | null;
}) {
  ...
  const structureItems = [
    t('blockOutputStructure1'),
    t('blockOutputStructure2'),
    t('blockOutputStructure3'),
    t('blockOutputStructure4'),
  ];
  const reqItems = [
    t('blockOutputReq1'),
    t('blockOutputReq2'),
  ...
  const formatItems = [
    t('blockOutputFormat1'),
    t('blockOutputFormat2'),
    t('blockOutputFormat3'),
    t('blockOutputFormat4'),
    t('blockOutputFormat5'),
  ];
  const reqListItems = [
    t('blockOutputReqItem1'),
    t('blockOutputReqItem2'),
    t('blockOutputReqItem3'),
    t('blockOutputReqItem4'),
    t('blockOutputReqItem5'),
  ];
  return (
    <div className="space-y-6">
      <div className="bg-orange-50 dark:bg-orange-900/20 border-l-4 border-orange-500 p-6 rounded-xl">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border-2 border-gray-200 dark:border-gray-700">
        <h4 className="font-bold mb-4 text-gray-900 dark:text-white">
          {t('blockOutputExampleTitle')}
        </h4>
        <div className="bg-brand-50 dark:bg-brand-900/20 p-4 rounded-xl text-sm mb-3">
          <p className="text-brand-700 dark:text-brand-300 mb-2 font-semibold">
            {t('blockOutputFormatLabel')}
          </p>
          <p className="text-gray-600 dark:text-gray-400 mb-3">
            {t('blockOutputStructureLabel')}
          </p>
        ...
        <div className="bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-xl text-sm">
          <p className="text-emerald-700 dark:text-emerald-300 mb-2 font-semibold">
            {t('blockOutputRequirementsTitle')}
          </p>
      ...
      <TemplateBlock
        label={t('blockOutputTemplateLabel')}
        template={t('blockOutputTemplate')}
      />
      {onRenderTask()}
    </div>
  );
...
export function ReasoningModelsSlide({
  slide,
  onRenderTask,
}: {
  slide?: Slide;
  ...
  const t = getT('contentSlides');
  const intro =
    slide?.content && 'veiksmoIntro' in slide.content
      ? slide.content.veiksmoIntro
      : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-gray-900 to-brand-900 p-5 rounded-xl text-white">
        <p className="text-sm text-brand-200 mb-1">
          {t('blockReasoningModelsHero1')}
        </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div
          className="bg-brand-50 dark:bg-brand-900/20 p-4 rounded-xl border-2 border-brand-200 dark:border-brand-800"
          role="article"
          aria-label={t('blockReasoningModelsCotTitle')}
        >
          ...
          <span className="inline-block px-2 py-0.5 rounded-full bg-brand-200 dark:bg-brand-800 text-brand-800 dark:text-brand-200 text-xs font-medium">
            {t('blockReasoningModelsCotBadge')}
          </span>
        </div>
        <div
          className="bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-xl border-2 border-emerald-200 dark:border-emerald-800"
          role="article"
          aria-label={t('blockReasoningModelsTotTitle')}
        >
          <div className="flex items-center gap-2 mb-2">
            <span className="text-2xl" aria-hidden="true">
              🌳
          ...
          <span className="inline-block px-2 py-0.5 rounded-full bg-emerald-200 dark:bg-emerald-800 text-emerald-800 dark:text-emerald-200 text-xs font-medium">
            {t('blockReasoningModelsTotBadge')}
          </span>
        </div>
      </div>
        ...
        <summary className="cursor-pointer list-none flex items-center justify-between p-5 hover:bg-brand-50/80 dark:hover:bg-brand-900/30 transition-colors select-none min-h-[44px]">
          <div className="flex items-center gap-3">
            ...
            <h4 className="font-bold text-lg text-brand-900 dark:text-brand-100">
              {t('blockReasoningModelsCotDetailsHeading')}
            </h4>
          </div>
          <span className="text-gray-400 dark:text-gray-500 group-open:rotate-180 transition-transform text-sm shrink-0">
            ▼
          </span>
        </summary>
        ...
        <summary className="cursor-pointer list-none flex items-center justify-between p-5 hover:bg-emerald-50/80 dark:hover:bg-emerald-900/30 transition-colors select-none min-h-[44px]">
          <div className="flex items-center gap-3">
            ...
            <h4 className="font-bold text-lg text-emerald-900 dark:text-emerald-100">
              {t('blockReasoningModelsTotDetailsHeading')}
            </h4>
          </div>
          <span className="text-gray-400 dark:text-gray-500 group-open:rotate-180 transition-transform text-sm shrink-0">
            ▼
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <TemplateBlock
          label={t('blockReasoningModelsCotTemplateLabel')}
          template={cotTemplate}
        />
        <TemplateBlock
          label={t('blockReasoningModelsTotTemplateLabel')}
...
export function ReasoningBlockSlide({
  slide,
  onRenderTask,
}: {
  slide?: Slide;
  ...
  const steps = [
    {
      num: 1,
      step: t('blockReasoningStep1'),
      desc: t('blockReasoningStep1Desc'),
    },
    {
      num: 2,
      step: t('blockReasoningStep2'),
      desc: t('blockReasoningStep2Desc'),
    },
    {
      num: 3,
      step: t('blockReasoningStep3'),
      desc: t('blockReasoningStep3Desc'),
    },
    {
      num: 4,
      step: t('blockReasoningStep4'),
      desc: t('blockReasoningStep4Desc'),
    },
    {
      num: 5,
      step: t('blockReasoningStep5'),
      desc: t('blockReasoningStep5Desc'),
    },
    {
      num: 6,
      step: t('blockReasoningStep6'),
      desc: t('blockReasoningStep6Desc'),
    },
  ];
  const liteSteps = [
    t('blockReasoningLiteStep1'),
    t('blockReasoningLiteStep2'),
    t('blockReasoningLiteStep3'),
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div
          className="bg-rose-50 dark:bg-rose-900/20 p-4 rounded-xl border-2 border-rose-200 dark:border-rose-800"
          role="article"
      ...
      <div
        className="bg-brand-50 dark:bg-brand-900/20 p-5 rounded-xl border-2 border-brand-200 dark:border-brand-800"
        role="article"
        aria-label={t('blockReasoningLiteTitle')}
      >
        <div className="flex flex-wrap items-center gap-3 mb-3">
          ...
          <span className="text-xs px-2 py-0.5 rounded-full bg-brand-200 dark:bg-brand-800 text-brand-800 dark:text-brand-200 font-medium">
            {t('blockReasoningLiteBadge')}
          </span>
        </div>
        <div className="flex flex-wrap gap-2">
          {liteSteps.map((step, idx) => (
            <div
              key={idx}
              className="flex items-center gap-1.5 bg-white dark:bg-gray-800 px-3 py-2 rounded-lg"
            >
      ...
      <details className="group bg-gradient-to-r from-brand-50 to-brand-100 dark:from-brand-900/20 dark:to-brand-900/30 rounded-xl overflow-hidden">
        ...
        <div className="px-5 pb-5 space-y-2">
          {steps.map((item) => (
            <div
              key={item.num}
              className="flex gap-3 items-start bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-200 dark:border-gray-700"
            >
      ...
      <details className="group bg-white dark:bg-gray-800 rounded-xl border-2 border-gray-200 dark:border-gray-700 overflow-hidden">
        <summary className="cursor-pointer list-none flex items-center justify-between p-5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors select-none min-h-[44px]">
          <div className="flex items-center gap-3">
            ...
            <h4 className="font-bold text-lg text-gray-900 dark:text-white">
              {t('blockReasoningExamplesHeading')}
            </h4>
          </div>
          <span className="text-gray-400 dark:text-gray-500 group-open:rotate-180 transition-transform text-sm shrink-0">
            ▼
      ...
      <TemplateBlock
        label={t('blockReasoningTemplateLabel')}
...
export function QualityBlockSlide({
  slide,
  onRenderTask,
}: {
  slide?: Slide;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-rose-50 dark:bg-rose-900/20 border-2 border-rose-300 dark:border-rose-700 p-5 rounded-xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <div
            className="bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-700 p-3 rounded-lg"
            role="article"
            aria-label={t('blockQualityBadLabel')}
          >
          ...
          <div
            className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-300 dark:border-emerald-700 p-3 rounded-lg"
            role="article"
            aria-label={t('blockQualityGoodLabel')}
          >
            <div className="flex items-center gap-2 mb-1">
              <span className="text-lg">✅</span>
      ...
      <div className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700">
        <h4 className="font-bold mb-3 text-gray-900 dark:text-white text-base">
          {t('blockQualityCriteriaTitle')}
        </h4>
        <div className="space-y-1.5 text-sm">
          {criteria.map((item, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`flex items-center gap-2 p-2.5 ${colors.bg} ${colors.bgDark} rounded-lg`}
              >
                <CheckCircle
      ...
      <details className="group bg-violet-50 dark:bg-violet-900/20 rounded-xl border-2 border-violet-300 dark:border-violet-700 overflow-hidden">
        <summary className="cursor-pointer list-none flex items-center justify-between p-5 hover:bg-violet-100/50 dark:hover:bg-violet-900/30 transition-colors select-none min-h-[44px]">
          <div className="flex items-center gap-3">
            ...
            <h4 className="font-bold text-lg text-violet-900 dark:text-violet-100">
              {t('blockQualityReasoningHeading')}
            </h4>
          </div>
          <span className="text-gray-400 dark:text-gray-500 group-open:rotate-180 transition-transform text-sm shrink-0">
            ▼
        ...
        <div className="px-5 pb-5 space-y-1.5 text-sm">
          {reasoningCriteria.map((item, idx) => (
            <div
              key={idx}
              className="flex items-start gap-2 p-2.5 bg-white dark:bg-gray-800 rounded-lg"
            >
              <span className="text-violet-600 dark:text-violet-400 font-bold mt-0.5 shrink-0">
                •
              </span>
              <span className="text-gray-700 dark:text-gray-300">{item}</span>
            </div>
      ...
      <details className="group bg-white dark:bg-gray-800 rounded-xl border-2 border-gray-200 dark:border-gray-700 overflow-hidden">
        <summary className="cursor-pointer list-none flex items-center justify-between p-5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors select-none min-h-[44px]">
          <div className="flex items-center gap-3">
            ...
            <h4 className="font-bold text-lg text-gray-900 dark:text-white">
              {t('blockQualityQcTemplatesHeading')}
            </h4>
          </div>
          <span className="text-gray-400 dark:text-gray-500 group-open:rotate-180 transition-transform text-sm shrink-0">
            ▼
        ...
        <div className="px-5 pb-5 space-y-4">
          <div>
            ...
            <TemplateBlock
              label={t('blockQualityTemplateLabel')}
              template={t('blockQualityInlineTemplate')}
            />
            <div className="mt-2 p-2.5 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-700">
              <p className="text-xs text-emerald-700 dark:text-emerald-400 font-semibold mb-1">
                {t('blockQualityBusinessExample')}
              </p>
              <p className="text-xs text-gray-700 dark:text-gray-300 italic">
                {t('blockQualityInlineExample')}
              </p>
            </div>
          </div>
          <div>
            ...
            <TemplateBlock
              label={t('blockQualityTemplateLabel')}
              template={t('blockQualityPostHocTemplate')}
            />
            <div className="mt-2 p-2.5 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-700">
              <p className="text-xs text-emerald-700 dark:text-emerald-400 font-semibold mb-1">
                {t('blockQualityBusinessExample')}
      ...
      <div
        className="bg-blue-50 dark:bg-blue-900/20 p-5 rounded-xl border-2 border-blue-300 dark:border-blue-700"
        role="article"
        aria-label={t('blockQualityMicroTitle')}
      >
        ...
        <div className="flex flex-wrap gap-2">
          <span className="inline-flex items-center gap-1.5 bg-white dark:bg-gray-800 px-3 py-2 rounded-lg border border-blue-200 dark:border-blue-800 text-sm">
            ...
            {t('blockQualityMicro1')}
          </span>
          <span className="inline-flex items-center gap-1.5 bg-white dark:bg-gray-800 px-3 py-2 rounded-lg border border-blue-200 dark:border-blue-800 text-sm">
            <span className="text-blue-600 dark:text-blue-400 font-bold">
              2.
            </span>{' '}
            {t('blockQualityMicro2')}
          </span>
          <span className="inline-flex items-center gap-1.5 bg-white dark:bg-gray-800 px-3 py-2 rounded-lg border border-blue-200 dark:border-blue-800 text-sm">
            <span className="text-blue-600 dark:text-blue-400 font-bold">
              3.
            </span>{' '}
            {t('blockQualityMicro3')}
          </span>
        </div>
      </div>
...
export function AdvancedParameters2Slide({
  slide,
  onRenderTask,
}: AdvancedParameters2SlideProps) {
  useTranslation();
  const t = getT('contentSlides');
  const intro =
    slide?.content && 'veiksmoIntro' in slide.content
    ...
    <div className="space-y-6">
      ...
      <div className="bg-brand-50 dark:bg-brand-900/20 border-l-4 border-brand-500 p-6 rounded-xl">
        ...
        <p className="text-gray-700 dark:text-gray-300 mb-3">
          {t('blockAdvanced2Goal')}
        </p>
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-4 rounded-lg">
          <p className="text-sm text-red-800 dark:text-red-200 font-semibold">
            ⚠️ {t('blockAdvanced2Warning')}
          </p>
      ...
      <details
        open
        className="bg-white dark:bg-gray-800 rounded-xl border-2 border-blue-200 dark:border-blue-800 group"
      >
        <summary className="p-6 cursor-pointer list-none flex items-center justify-between select-none hover:bg-blue-50/50 dark:hover:bg-blue-900/10 rounded-xl transition-colors">
          <h4 className="font-bold text-lg text-blue-900 dark:text-blue-100">
            {t('blockAdvanced2MaxTokensTitle')}
          </h4>
          <span className="text-gray-400 dark:text-gray-500 ml-2 group-open:rotate-180 transition-transform">
            ▼
          </span>
        </summary>
            ...
            <table className="w-full text-base border-collapse">
              <thead>
                <tr className="bg-blue-50 dark:bg-blue-900/20">
                  <th className="px-4 py-3.5 text-left border-b border-blue-200 dark:border-blue-800 align-top">
                    {t('blockAdvanced2TableValue')}
                  </th>
                  <th className="px-4 py-3.5 text-left border-b border-blue-200 dark:border-blue-800 align-top">
                    {t('blockAdvanced2TableWhen')}
                  </th>
                </tr>
              </thead>
              <tbody className="text-gray-700 dark:text-gray-300">
                <tr className="border-b border-gray-200 dark:border-gray-700">
                  <td className="px-4 py-3.5 font-mono align-top leading-relaxed">
                    50–100
                  </td>
                  <td className="px-4 py-3.5 align-top leading-relaxed">
                    {t('blockAdvanced2Max50')}
                  </td>
                </tr>
      ...
      <TemplateBlock
        ...
        template={t('blockAdvanced2Template')}
      />
      {onRenderTask()}
    </div>
  );
...
export function FullExampleSlide({
  onRenderTask,
}: {
  onRenderTask: () => JSX.Element | null;
}) {
  ...
  const blocks: FullExampleBlock[] = [
    { num: 1, name: 'META', color: 'rose', content: t('blockFullExampleMeta') },
    {
      ...
      name: 'INPUT',
      color: 'orange',
      content: t('blockFullExampleInput'),
    },
    {
      num: 3,
      name: 'OUTPUT',
      color: 'amber',
      content: t('blockFullExampleOutput'),
    },
    {
      num: 4,
      name: 'REASONING',
      color: 'emerald',
      content: t('blockFullExampleReasoning'),
    },
    {
      num: 5,
      name: 'QUALITY',
      color: 'brand',
      content: t('blockFullExampleQuality'),
    },
    {
      num: 6,
      name: 'ADVANCED',
      color: 'violet',
      content: t('blockFullExampleAdvanced'),
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3 text-sm">
        {blocks.map((block) => {
          ...
          return (
            <div
              key={block.num}
              className={`${colors.bg} ${colors.bgDark} p-4 rounded-xl border-l-4 ${colors.border}`}
            >
              <p
                className={`text-xs font-bold ${colors.text} ${colors.textDark} mb-1 uppercase tracking-wider`}
              >
      ...
      <TemplateBlock
        label={t('blockFullExampleTemplateLabel')}
        template={t('blockFullExampleTemplate')}
      />
