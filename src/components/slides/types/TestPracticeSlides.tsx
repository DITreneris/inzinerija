...
import {
  getLastCategoryScores,
  setLastCategoryScores,
  getLastMaxStreak,
  setLastMaxStreak,
} from './path-test/pathTestSession';
export type { CategoryScore } from './path-test/pathTestSession';
import { TestIntroSlide } from './path-test/TestIntroSlide';
export { TestIntroSlide };
...
/* TestIntroSlide -> src/components/slides/types/path-test/TestIntroSlide.tsx */
...
export function TestResultsSlide({
  ...
  const radarData: RadarDataPoint[] = RADAR_CATEGORIES.map((cat) => ({
    ...
    value: getLastCategoryScores()[cat]?.percentage ?? 0,
  }));
  const hasRadarData = Object.keys(getLastCategoryScores()).length > 0;
    ...
    <div className="space-y-6">
      ...
      >
        ...
        {getLastMaxStreak() >= 3 && (
          <div className="inline-flex items-center gap-1.5 mt-3 px-3 py-1 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 text-sm font-medium">
            ...
            {locale === 'en'
              ? `Best streak: ${getLastMaxStreak()} in a row`
      ...
      {moduleId === 2 && (
        <CategoryBreakdownWithLinks
          categoryScores={getLastCategoryScores()}
