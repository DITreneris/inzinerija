...
export function ActionIntroSlide({ content }: ActionIntroSlideProps) {
  ...
  const ctaLabel =
    content.ctaText ||
    (hasReveal ? t('actionIntroCtaReveal') : t('actionIntroCtaDefault'));
  const toolsPreview = content.tools?.slice(0, TOOLS_PREVIEW_COUNT) ?? [];
  const toolsRest =
    (content.tools?.length ?? 0) > TOOLS_PREVIEW_COUNT
      ? content.tools!.slice(TOOLS_PREVIEW_COUNT)
      : [];
  const hasMoreTools = toolsRest.length > 0;
  const outcomesList = content.outcomes ?? [];
  const outcomesPreview = showAllOutcomes
    ? outcomesList
    : outcomesList.slice(0, OUTCOMES_PREVIEW_COUNT);
        ...
        <div className="relative z-10 flex flex-col items-center text-center gap-3 sm:gap-4 max-w-lg mx-auto">
          ...
          {!showHookOnly && (
            <>
              ...
              {content.heroSubText &&
                (() => {
                  const parts = content.heroSubText
                    .split(/\.\s+/)
                    .filter(Boolean);
                  const isTwoLineSmugis =
                    parts.length === 2 &&
                    content.heroSubText.includes('Problema');
                  if (isTwoLineSmugis) {
                    return (
              ...
              {!hasReveal && content.firstActionCTA && (
                <div
                  className="mt-2 sm:mt-3 w-full max-w-md rounded-xl bg-accent-500/20 dark:bg-accent-900/30 border border-accent-400/50 dark:border-accent-600/50 p-4 text-left"
                  role="region"
      ...
      {(hasReveal ? revealed : true) && content.outcomes?.length > 0 && (
        <>
          {revealed && content.firstActionCTA && (
            <div
              className="rounded-xl bg-accent-500/20 dark:bg-accent-900/30 border border-accent-400/50 dark:border-accent-600/50 p-4 animate-fade-in"
              role="region"
              aria-label={t('firstStepAria')}
            >
              <p className="text-xs font-semibold text-accent-700 dark:text-accent-300 uppercase tracking-wider mb-1.5">
                {t('firstStepLabel')}
              </p>
              <p className="text-sm sm:text-base text-gray-800 dark:text-white font-medium leading-snug">
                {content.firstActionCTA}
              </p>
            </div>
          )}
              ...
              {hasMoreOutcomes && (
                <button
                  ...
                  aria-label={
                    showAllOutcomes
                      ? t('hideResults')
                      : t('showAllResults', { count: outcomesList.length })
                  }
                >
                  {showAllOutcomes ? (
                ...
                <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                  ...
                  >
                    <span className="font-bold text-gray-900 dark:text-white text-sm">
                      {t('forAudienceDuration')}
                    </span>
                    <ChevronDown
                      className={`w-4 h-4 flex-shrink-0 text-gray-500 transition-transform duration-200 ${showMetaCollapsible ? 'rotate-180' : ''}`}
                      aria-hidden="true"
                    />
                  </button>
                  {showMetaCollapsible && (
                    <div
                      className="p-4 pt-0 space-y-3 border-t border-gray-200 dark:border-gray-700"
                      role="region"
                      aria-label={t('forAudienceDuration')}
                    >
                      {content.duration && (
                        <div>
                          <h4 className="font-bold text-gray-900 dark:text-white text-sm mb-1">
          ...
          {content.howToUseModule &&
            (content.howToUseModule.items?.length ||
              ...
              >
                ...
                {content.howToUseModule.items &&
                  content.howToUseModule.items.length > 0 && (
          ...
          {content.tools && content.tools.length > 0 && (
            <div className="animate-fade-in border-2 border-brand-200 dark:border-brand-800 rounded-2xl bg-gradient-to-b from-brand-50/80 to-white dark:from-brand-950/50 dark:to-gray-900 p-6 sm:p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {(showAllTools ? content.tools : toolsPreview).map(
                  (tool, idx) => (
                    <div
                      key={idx}
              ...
              {hasMoreTools && (
                <button
                  ...
                  aria-label={
                    showAllTools
                      ? t('hideExtraTools')
                      : t('showAllToolsCount', { count: content.tools!.length })
                  }
                >
                  <Wrench
                  ...
                  <span>
                    {showAllTools
                      ? t('hideExtraTools')
                      : t('showAllToolsCount', {
                          count: content.tools!.length,
                        })}
                  </span>
                  <ChevronDown
                    className={`w-4 h-4 flex-shrink-0 transition-transform duration-200 ${showAllTools ? 'rotate-180' : ''}`}
                    aria-hidden="true"
                  />
                </button>
              )}
