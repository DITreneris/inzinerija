...
describe('m79HandoutPdf', () => {
  ...
  it('writes brand title and linked ecosystem CTAs', async () => {
    ...
    const linkedUrls = mockLink.mock.calls.map(
      (c: unknown[]) => (c[4] as { url: string }).url
    );
    expect(
      linkedUrls.some(
        (u: string) =>
          u.includes('handout_decide') && u.includes('promptanatomy.app')
      )
    ).toBe(true);
    expect(
      linkedUrls.some((u: string) => u.includes('promptanatomy.pro'))
    ).toBe(false);
  });
