...
Failas didelis (keli tūkstančiai eilučių). **Naujausia istorija** prasideda skiltyje **[Unreleased]**; toliau eina **pažymėti leidimai** nuo naujausių (**[1.6.3]**, **[1.6.2]**, **[1.6.1]**, **[1.6.0]**, **[1.5.0]**, **[1.4.9]** …). **Failo pabaigoje** paliktas ankstesnis **2.x** ir **1.0.0** žymėjimas – tai istorinis sluoksnis iš ankstesnės kūrimo fazės (prieš dabartinę **1.x** produkcijos liniją; žr. poskyrį _Ankstesnis versijų žymėjimas (2.x ir 1.0.0)_).
...
- **Moduliai 10–12:** Agentų kelias; corporate cut `build:corporate12` (`*-m1-m12.json`); EN overlay `modules-en-m10-m12.json`; P2 artefaktai – `m1012` atmintinė ir tier 4 sertifikatas. Prod bundle **M1–12**; **12 live per Supabase**.
- **Moduliai 13–15:** Turinio inžinerijos kelias; corporate cut `build:corporate15` (`*-m1-m15.json`); EN overlay `modules-en-m13-m15.json`; P2 artefaktai – `m1315` atmintinė ir tier 5 sertifikatas.
...
- **M11 Path Test chrome + EN q6 (2026-08-16):** `WarmUpQuizSlide` paskutinis CTA kvietė `common.finish`, kurio nebuvo — LT rodė raktą `finish`. Pridėtas `common.finish` (Baigti / Finish); savitikros done-state ir `warmUpQuestionInfo` per `contentSlides` (ne „įskaita neįskaitoma“ / ne „testas neįskaitomas“). EN `m11-q6` scenarioContext dublikatas kirptas `build-en-m10-m12.mjs` + rebuild. Shared visiems `warm-up-quiz`. RTL `WarmUpQuizSlide` 3. Ne `generate:core-data` / ne 1.6.3.
...
- **M11 / M12 owner walk close (2026-08-16):** **M11 walked, no RAW** · **M12 walked, no RAW**. Intake + TODO §1.3 walk eilutės uždarytos.

- **Sertifikatų kompetencijos (2026-08-16):** 5 tier `programName` – teminės _Išmoko…_ eilutės (ne _Geba_; T2 kontekstas, T4 eiga/įrankiai/kontrolė, T5 + tekstas). `completionLine` / footer neliesti. LT + EN.

- **Žmogaus kontrolė be „varto“ (2026-08-16):** gyvuose M10–12 tekstuose `žmogaus vartas` / `human gate` → **žmogaus kontrolė** / **human control** (žodynas HITL). Agentų kelio atmintinė: Paleidiklis / Veiksmas / Įrankis / Žmogaus kontrolė / Audito žurnalas. LT + EN build + core profiliai. Neliečia kokybės / paleidimo / CI vartų.

- **Sertifikatų veidas opcija B (2026-08-16):** visų 5 tier PDF copy – oficialus kelio vardas + 1 kompetencijos eilutė; nuimti moduliai / ≥ 70 % / projektas nuo veido. Bendras footer be „promptų struktūros mokymas“. Schema `completionLine` ≤70 / `programName` ≤55. LT + EN (`certificateContent*.json`, ne `build-en-*`).

- **M11 Path Test item quality (2026-08-16):** `111` q8 raktas = žmogaus patvirtinimas prieš siuntimą (`correct: 3`); išimtys lieka [0] kaip tikėtinas klaidingas. q6 = nuosekli grandinė be RFP (`correct: 1`). q1/q3/q7 distractoriai plausible-but-wrong (M10, ne absurdas). q4 / 10.6 neliesti. LT + `build:modules-en-m10-m12` + `generate:core-data`. SOT §7.2 sulygiuotas (q5 @ 10.64).

- **Home hero CTA zona (2026-08-16):** returning-user apačia = progresas (`N iš T modulių baigta`, be nav `%`) + centruota grupė: gold `Tęsti` ir text-button `Pakartoti M{{n}} · 5 min.` (`home:recallCta`, 44 px, `px-3`). Ne `RetrievalDueCard`, ne katalogo `nextStepRecallCta`.

- **M10 pritaikymo katalogas 12→4 (2026-08-16):** savininko batch. Keturi procesai (pardavimai / personalas / finansai / aptarnavimas) pirmą kartą mokomi `10.15` „Kur pritaikyti“ su etiketėmis Rizika / Rodiklis; `10.8` tik recap (ne „12 pritaikymo pavyzdžių“); hero stat = 1 eigos aprašymas. 10.1 nebevadina santraukos neprivaloma. Likę 8 – SOT ref, ne gyvas UI. LT + EN build. Be naujos skaidrės / Pattern / diagramos.

- **M10 10.15 hierarchy stem (2026-08-16):** belongs-to kotas 2.5 `flow` (ne amber × 0.72 siūlas); „Paleidiklio tipas“ kairėje nuo ašies; T-lentyna virš tipų, be process antgalio. Amber tik pasirinktam chip. Be `modules.json`.

- **M12 120.5 restore fork-in-pipe (2026-08-16):** grąžinta HEAD šakė vamzdyje (A virš / B po, tas pats x) + in-place scale (VB 1024×400, h 72, linear gap ≥32); veiksmažodžiai off-shaft su fonu, vienas takte. 2 eilių stalas / unknot koridoriai atšaukti. Title-only ir Trumpai palikti. Be `generate:core-data`.

- **M10 10.35 tree readability v02 (2026-08-16):** kriterijai `edgeLabel` 12 px virš juostos (ne ant koto); dim tik unselected drop’ams (`TREE_DIM` 0.72); lapai be group-ghost ir be `#ffffff`; Workato solid + „orientyras“ (ne dashed); vienas trunk+bus, selected drop paskutinis; mobile `scroll` @ 800. Layout SOT `m10ToolDecisionTreeLayout.ts`. Be `modules.json`.

- **M10 10.15 process-config-hierarchy etalonas (2026-08-16):** Kit sibling (ne W8): `hierarchyDropGeometry` – horizontalu + antgalis = srautas, vertikalu be antgalio = priklauso. Tipų juosta gyva tik Paleidiklio takte (lokalus orphan kituose). Amber = pasirinktas tipas (default Pranešimas). Paaiškinimas takte 0 += **Pasirinktas tipas** + HTML radiogroup (ne ChoiceControl). Dedicated Block; factory neliečiamas. Shell 3 taktai. Be `modules.json`.

- **Owner status sync (2026-08-16):** **12 live per Supabase**; M7–18 = corporate grant iki 2027-01 (ne produkto skola). Pin **v1.6.2** / `c35a1f5`. `v1.4.9` nebe current pin. MON-2/3 `[x]`; MON-8 superseded; CAV-B1 `[x]`. Optional SPA chrome fingerprint ≠ cutover blokas. `MON_P0_EXECUTION_PLAN` superseded. Ne 1.6.3 / ne re-pin.
...
_M10–12 content freeze · katalogo UX · SCHEME-CENTRAL W1 · Docs Lean · CATALOG-HOME. Corporate12 pin **v1.6.2**._
...
- **ESLint 8 (EOL) → 9 flat config (2026-08-11):** `.eslintrc.cjs` → [`eslint.config.js`](eslint.config.js); `@typescript-eslint/*` 6 → `typescript-eslint` 8 meta paketas; `eslint-plugin-react-hooks` 4 → 7; `react-refresh` 0.4 → 0.5; nauji `@eslint/js` + `globals`. Advisory **12 → 6** (`minimatch` high uždarytas). **Ne 10, o 9:** `eslint-plugin-react@7.37.5` yra naujausia ir jos peer baigiasi `^9.7`.
  - **Migracija ≠ politikos keitimas.** `react-hooks` v7 `recommended` turi **16** taisyklių (v4 turėjo 2) – įjungus būtų vienu ypu atsiradusios 14 React Compiler taisyklių, tad projekto visą laiką taikytos dvi (`rules-of-hooks` error, `exhaustive-deps` warn) surašytos tiesiogiai. Sprendimas dėl likusių = `TODO.md` §1.7 **TOOL-5**.
  - **Du realūs radiniai:** `typescript-eslint@8` pakeitė `no-unused-vars` `caughtErrors` numatytąjį į `all` → `catch (_)` pradėjo lūžti; pridėtas `caughtErrorsIgnorePattern: '^_'` (ta pati projekto konvencija kaip `args`/`vars`). `react-refresh` 0.5 „konstanta" laiko tik primityvus, tad 15 `slides/shared/*Diagram.tsx` geometrijos objektų (`*_GEOMETRY`, testų SOT) pradėjo warninti – taisyklė ten išjungta _scoped_ (paveikia tik HMR ergonomiką, o co-location yra SCHEME_AGENT konvencija).
  - **Patikrinta, kad migracija neprarado padengimo:** 87 aktyvios taisyklės; `react/jsx-key`, `rules-of-hooks`, `no-unused-vars`, `no-explicit-any` įjungtos; `react/prop-types`, `react-in-jsx-scope` išjungtos kaip anksčiau; `react-refresh` override tikrai scoped (ne diagramose vis dar `warn`). Vartai: lint · typecheck · **161/982**.

- **Vite 5 → 7 + Vitest 1.6 → 4 (2026-08-11):** `vite` `^5.0.8` → `^7.3.6`, `vitest` / `@vitest/coverage-v8` / `@vitest/ui` → `^4.1.10`. Advisory **6 → 0** (`vite` high, `esbuild` moderate, `vitest` critical). **Ne Vite 8:** `vite@8` pereina į `rolldown` + `lightningcss`; `vite@7` jau turi pataisytą `esbuild ^0.27`, išlaiko Rollup ir mūsų `manualChunks` (`vendor` / `icons` / `helmet`) + `rollup-plugin-visualizer`. `@vitejs/plugin-react`, `jsdom` ir Testing Library sąmoningai nekeisti – `plugin-react@4.7.0` jau palaiko Vite 7, o `vitest@4` nereikalauja `jsdom` major.
  - **Vitest 4 suderinamumas:** `vitest.config.ts` pervestas į ESM-safe `fileURLToPath(new URL(...))`; pašalintas perteklinis `**/../**` exclude. `jsPDF` testų mock'ai pakeisti iš arrow factory į constructor-safe `function`, nes produkto kodas daro `new jsPDF(...)`. Lėti repo-wide audit testai ir `ToolsPage` gavo scoped timeout'us Windows/Vitest 4 režimui. `ModulesPage` prefetch tapo non-critical `Promise.all(...).catch(...)`, kad dynamic import rejection po test environment teardown nebūtų unhandled.
  - **Vartai:** lint · typecheck · `test:run` **161/982** · `validate:schema` · core MVP + production 9 + corporate12 + corporate15 + full build'ai · dev smoke (`modules.json` no-store) · `npm audit` **0 vulnerabilities**.
...
- **Toolchain advisory closeout (2026-08-11):** TOOL-2 pakėlė Vite 7 + Vitest 4 ir `npm audit` dabar rodo **0 vulnerabilities**. Vite 8/Rolldown sąmoningai atidėtas kaip atskiras epikas, nes tai jau bundlerio migracija, ne advisory uždarymo būtinybė.
...
- **Kalbos konvencijų SOT + repo-wide vartai (2026-08-11):** LT kreipinys **`tu`** ir **American English** = standartas (GOLDEN **§6c**). LT: 71 pataisa – `Jūs` forma ir barbarizmai (`įtakoti` ×5, `pilnai` ×5, `sekantis`, `-ki` imperatyvas) `modules.json` / `glossary.json` / `promptLibrary.json` / `m6HandoutContent.json` / `lt.json` **ir** komponentų hardcoded fallback'uose (`m10TeamReadinessContent`, `ContentSlides`, `TestPracticeSlides`, `ConfidenceSelector`, `questionPool`). EN: BrE→AmE visuose EN paviršiuose (overlays, glossary, quiz, tools, handouts, journey, `en.json`, komponentai) **+ durable** `build-en-*.mjs` / overrides, kad rebuild negrąžintų BrE. Nauji vartai [`audit-en-spelling.mjs`](scripts/audit-en-spelling.mjs) + [`audit-lt-address.mjs`](scripts/audit-lt-address.mjs) (16 duomenų + 393 source failai) `audit:release-preflight`; `audit:m79`/`m46` LT pusė praplėsta per `lt-address-rules`. **Kodėl praslydo:** `audit:en-language-m*` skaito tik savo 2 failus, LT pusėje – 5 literal šablonai; ASCII `\b` nemato `į…`. Regresija: `languageConventionGates.test.ts`. `audit:release-preflight` green, **161/982** tests.
- **M10–M12 deep audit remediation (2026-08-11):** Applied Batch A–F from [`M10_M12_DEEP_AUDIT_2026-08.md`](docs/development/M10_M12_DEEP_AUDIT_2026-08.md): false enlarge copy removed; `124` made optional and rewritten; M10 language/jargon cleaned; dependency inversions fixed; M11 coverage retargeted to 3A + minimal spec; M10 content gaps filled (MCP/connectors, prompt injection, cost/model choice, structured JSON handoff, AI Act); tools catalog + M12 tools synced; `m12_three_labs` converted to Shell; M10 picker duplication and linear-process renderer debt reduced. `audit:release-preflight` green; re-verified **161/982** after the language gates landed (M10 lab copy `.ts` liko `Jūs` forma – žr. eilutę aukščiau).
- **Pre-launch audit remediation (2026-08-04):** EN M13 titles `13.33`/`13.35`/`13.8` — strip `(optional)` from title map + rebuild (`audit:slide-titles`); `m18_launch_gates` in `PREMIUM_DIAGRAM_IMAGE_KEYS`; `audit:m1618` in `audit:release-preflight`; docs metrics **160/966** + stale open TE/Should/M16-PLAIN sync. `audit:release-preflight` green.
...
- **M13P-TRIM copyable fit (2026-08-04):** −30% pass — trim 13.4 image→video chain + 13.6 EN MASTER (collapsible); kiss 13.35 MASTER (merge light+colour); trim 13.1 A/E/C to Micro ≤4 L; keep 13.2/13.3. EN heading-index twins in [`m13-en-plain-overrides.mjs`](scripts/lib/m13-en-plain-overrides.mjs); `audit:m1315` green. Contract [`M13_PROMPT_MATURITY.md`](docs/development/M13_PROMPT_MATURITY.md); TODO §1.2j. Ne plain reopen / ne Lab 13.325 / ne `generate:core-data`.
- **M13 learner plain epic exit (2026-08-04):** `M13-PLAIN-B1`–`B3` + `EN` + `QA` — intro 130 outcomes/whyBenefit (veiksmo kalba); LT titles 13.32/13.52/13.56; section-break nextSteps; 13.11 Trumpai prieš diagramą; 13.9 summary + 13.8 glossary (CPI/C2PA/I2V/audio-first/reference lock) + 13.101 Trumpai; soft gloss 13.3–13.6/13.35/13.12. EN: kill identical outcomes stub; durable [`m13-en-plain-overrides.mjs`](scripts/lib/m13-en-plain-overrides.mjs) + rebuild guards; `audit:m1315` green. Plan [`M13_MATURITY_PLAN.md`](docs/development/M13_MATURITY_PLAN.md); TODO §1.2j. Ne TE / ne `generate:core-data`.
- **M17–M18 learner plain epic exit (2026-08-04):** `M17-PLAIN` — 172 results lukštas ≈ M11; bank bridges (PACKET/VSR/greita patikra); durable [`m17-en-plain-overrides.mjs`](scripts/lib/m17-en-plain-overrides.mjs). `M18-PLAIN` — PACKET/Soft DoD/smoke first-use LT tiltai; Soft DoD earned 18.23; Alias meta strip; durable [`m18-en-lt-map.json`](scripts/lib/m18-en-lt-map.json) + [`m18-en-plain-overrides.mjs`](scripts/lib/m18-en-plain-overrides.mjs) (apply prieš DI→AI walk); `audit-en-language-m16-18` body gate **modules 16–18**. Plans [`M17_MATURITY_PLAN.md`](docs/development/M17_MATURITY_PLAN.md) · [`M18_MATURITY_PLAN.md`](docs/development/M18_MATURITY_PLAN.md); TODO §1.2k–l. Optional M18 dens C deferred. Ne corporate18 / ne `generate:core-data`.
- **M16-PLAIN-C (2026-08-04):** delete echo `16.9` into `16.85` (checklist absorb); sharpen 16.7 LT zones vs 16.18 Must/Should/Won’t+NNL; M16 **22** sk.; footer renumber LT/EN.
- **M16 learner plain epic exit (2026-08-04):** `M16-PLAIN-EN` + `B1`–`B4` — LT plain (dienos prioritetų etalonai, brief 1–11, be Soft DoD/PACKET/MCP/smoke/Delivery/Alias M18); diagram chrome `Kelio žingsniai` / `Brief brandinimo žingsniai`; full M16 EN durable overrides (`scripts/lib/m16-en-plain-overrides.mjs` + `build:modules-en-m16-m18`); `audit-en-language-m16-18` M16 body diacritics gate. Intro 160/16.2 freeze paliktas. Plan [`M16_MATURITY_PLAN.md`](docs/development/M16_MATURITY_PLAN.md); TODO §1.2i.

### Added

- **M16 learner maturity plan (2026-08-04):** [`M16_MATURITY_PLAN.md`](docs/development/M16_MATURITY_PLAN.md) — skaidrė-po-skaidrės plain/EN/praktika/verslo nauda; rubrika P/N/Pr/V; sprintai `M16-PLAIN-0`…`C`; **vykdymas ✅** (EN+B1–B4; C optional deferred). **§0.1 freeze:** nesipjauna su action-intro polish (vibe hero + micro-CTA + EN 160/16.2). Authoring+TE done ≠ learner praeinamas (plain dabar ✅).
- **Caveats Closure / M1618 path handout (2026-08-04):** `m1618HandoutContent` LT/EN + `m1618HandoutPdf` path-funnel (Brief/VSR · PACKET · Soft DoD · starter); `completionArtifacts` earn-on M18; schema + unit/content/materials tests; `HANDOUT_MATURITY` + TODO §1.6 CAV-\* tracking; corporate18 intake parked; marketing handoff pin/analytics notes synced.

### Fixed

- **M16 action-intro clarity + CTA dedupe (2026-08-04):** slide `160` – drop misleading `1+1+1`, tu-form whyBenefit, vibe coding plain gloss + Cursor in `heroSubText`, CTA micro-win vs full D1; `16.2` triad `1 funkcija`; `ActionIntroSlide` non-reveal CTA only in hero (GOLDEN §4.2); EN hand-tune in `build-en-m16-m18` + seed sync. No tools mug on intro / no reveal / no core generate.
- **M7–9 EN language polish (2026-08-04):** Must/Should grammar + US spelling on EN overlay (`META block`, articles, `run it in AI`, truncated RULE, etc.); handout EN UK→US; glossary-en Kiss-Marry `Kill`→`Drop` + injection `behavior`/`Defense`. Patch `patch-m79-en-language-polish.mjs` + `generate:core-data`. LT SOT neliečia; **ne** `build:modules-en-m7-m9`.
- **M7–9 LT copy QA (2026-08-04):** rašyba / gramatika / kabutės / `ne`+liepiamoji learner copy (`modules.json` + `modules-journey-m7.json` + `generate:core-data`); SOT mirror `turinio_pletra_moduliai_7_8_9.md` scraping placeholder. Pvz. `Individūalų`→`Individualų`, `Paversi`→`Paversti`, `nespėliok`/`neleisk`, `ruoiesi`, `ĮKLIJUOK_…`, M9 `starte` / `įtakojantys` / `grupės` / užbaigtas `(pvz. Q3 kritimas)`. EN overlay / Geštalto nelieta.
- **M1–M6 EN proofread (2026-08-04):** LT spill + decimal commas (M4 tools/workflows/stats); grammar/parallelism; M3 Scenario 2/3 EN templates realigned to LT SOT (sales / SmartFlow); BrE + `artefact` consistency; glossary Meta/Brief + quiz stems. Files: `modules-en.json`, `modules-en-m4-m6.json`, `glossary-en.json`, `quiz-en.json` (no LT SOT / no `generate:core-data`).
- **M1–M6 LT copy (2026-08-04):** rašyba / gramatika / tu–Jūs / kabutės learner copy (`modules.json` + `generate:core-data`); satelitai `questionPool` / `questionPoolSelector` / `m6HandoutContent` / `lt.json` quiz chrome. Pvz. `empatingas`→`empatiškas`, `Atsiprašomas`→`Atsiprašantis`, `Sujunkite`→`Sujunk`, `nepatikrų`→`nepatikrintų`, `visa DI`→`visas DI`, quiz `Kokią … reikšmę`.
- **Pre-launch P0 (2026-08-01):** M1 footers renumbered after UJ-MUST eval slide `12.5` (LT `modules.json` + EN `modules-en.json` + `generate:core-data`); `injectOwnWorkContext` uses split/join instead of `replaceAll` (ES2020); `moduleTransfer` test fixtures include `subtitle`. `audit:release-preflight` green (**154/940**).

### Added

- **DS Waves A–E (2026-08-03):** `StatusPanel` (`success`/`warning`/`outcome` × hero/compact) + adoption in path-test results / practice covers / quest+journey confirm / summaries; `ctaButtonClassName` for `<a>` upsells; CTAButton/PathStep/ActionIntro/ModuleComplete CTA consolidation; `focusRingClasses`/`touchTargetClasses`/`surfaceGlass.overlay` chrome adoption (focus-visible policy); CTAButton self-contained Tailwind (hero gold); removed dead `.btn-*` / `.card` / `.badge*` from `index.css`; Schema3/4/ContextFlow/InstructGpt/LlmAutoregressive hex → `DIAGRAM_TOKENS` (+ `LLM_AUTOREGRESSIVE_ROLE_COLORS`); `DESIGN_SYSTEM.md` baseline sync (hex≤202 / total≤272).
- **M16 stack map table 16.25 (2026-08-01):** content-block + LENTELIU 4×4 (chat DI · Cursor · GitHub · host) between 16.2 and 16.3; M16 budget 23; EN hand-tuned + footer rebuild in `build-en-m16-m18`; overlay `table:m16:16.25:s1`; no new tools rows / no Feature Doc.
- **M16–18 glossary pack + tools docs (2026-08-01):** SOT §5.4 žodynas; LT/EN glossary — VSR fix (Vibe→Skeleton→Refinement ≠ vertical slice) + Brief (MVP), Triage, Vertikalus pjūvis / Vertical slice, PROJECT_RULES.md, Smoke, Diff ritualas; wire 16.85 / 18.125 `itemGlossaryTerms` + 18.16 / 18.23 unlocks; Cursor tools copy (katalogas M16 / build M18); `DATA_AGENT_TOOLS.md` moduleId 1–18; `patch-m1618-glossary-fill.mjs` + EN `glossaryTermMap` in `build-en-m16-m18.mjs`. Ne core / ne nauji tools rows.
- **Horizon D / TE-M1618-C2 .env contrast (2026-08-01):** 18.17 upgraded from static Blogas|Geras table to `toolChoiceBar.variant: manipulation-contrast` (M7/67 brother) — ChoiceControl Git/Promptas/Commit, null until pick, linked Geras copyables with PLACEHOLDER-only fixtures; EN hand-tuned + durable override in `build-en-m16-m18.mjs`; overlay `embed:toolChoiceBar:m18:18.17:s1` (removed obsolete `table:m18:18.17:s1`); `M16_M18_PROMPT_MATURITY` 18.17 Klaidos/Lab row; C3–C5 won’t-now; C1 parked (sales reassessment). No Feature Doc. `audit:teaching-elements:strict` green.
- **Horizon D / M1618 ritmas & journey (2026-08-01):** `M1618-R1`…`R6` — GOLDEN §3.8 breath without new Feature Doc: M16 `16.85` section-break (D1→D2) + `16.205` warm-up + `16.8` path-step kortelė; `16.15` lab-primary (score table collapsible); M18 `18.55` mid-A warm-up + `18.125` section-break (A→B) + `18.16`/`18.23` Soft DoD path (1/2–2/2); M16=22 / M18=28 slides; EN ritmas polish; overlay table key `16.15:s2`; `audit:teaching-elements:strict` + footer M16–18 LT/EN green. Scripts: `patch-m1618-ritmas.mjs`, `patch-m18-wu-1855.mjs`, `patch-en-m1618-ritmas.mjs`. Could C1–C5 + D3 remain open/Deferred.
- **Docs/EN sync B (2026-08-01):** M17 `module.transfer` LT+EN; EN M16/M18 transfer + own-work chrome (no LT leak); soft `audit:m1618` (`audit:en-coverage-m16-18` + transfer-chrome language + `audit:module-transfer` ≤18); glossary LT/EN — Cursor, VSR, BUILD PACKET, Vibe debt, Soft DoD (`moduleId` 16/18; ne core profiles).
- **Horizon D / TE-M1618 Should polish (2026-08-01):** `TE-M1618-S1` `lab:m16_direction_picker` on 16.15 — `M16DirectionPickerLabBlock` (ChoiceControl A/B/C, null until pick, score mirror, copyable why; brand-only); `TE-M1618-S2` `diagram:m18_launch_gates` on 18.19 — tollgate-barriers metaphor (≠ delivery corridor / ≠ diff magnifier); M18 dens soft pass (18.9/13/15/17/18/101 tables + plain Trumpai); M16–18 hygiene (strip authoring meta from learner body); M17 bank — VSR `ordering` + success-criterion MCQ (≤2 M18-forward). Overlay + `LAB_IMAGE_KEYS` + `audit:teaching-elements:strict` green. Open Could C1–C5; D3 Deferred.
- **Horizon D / TE-M1618 Must (2026-08-01):** M16–18 teaching elements MoSCoW Must — lentelės (16.7/14/15, 18.14/16 + S3 18.1/18.4); embeds (`recognitionExercise` 16.6/18.201, `briefCheck`+`preCopy` 16.21, `preCopy` 18.6–18.8); 5 signature schemos S4-INDIV (`m16_delivery_gates`, `m16_vsr_maturity`, `m16_user_cycle` shared 16.16/18.3, `m18_packet_stack`, `m18_diff_ritual`); satellite [`DIAGRAMU_M16_M18_REGISTRY.md`](docs/development/DIAGRAMU_M16_M18_REGISTRY.md) + overlay; M18 `ownWorkTemplate` ant santraukos; EN via `build:modules-en-m16-m18`; `audit:teaching-elements:strict` green.
- **Horizon D / M16–18 Kodo inžinerija authoring (2026-08-01):** `M1618-D0`…`F8` — kelias gyvas full SOT: eilė [`MODULIO_16_SKAIDRIU_EILES.md`](docs/MODULIO_16_SKAIDRIU_EILES.md) (M16 ~20 / M17 Path Test / M18 ~26); LT modules 16–18 + EN `modules-en-m16-m18` (`build:modules-en-m16-m18`); Cursor `tools.json`+EN; ModulesPage cyan track; DEV `getMaxAccessibleModuleId` → 18; M17 shell test. Content draft [`M16_M18_CONTENT_DRAFT.md`](docs/development/intake/M16_M18_CONTENT_DRAFT.md). Wave D3 corporate18 / magic-link tier 18 lieka Deferred. `generate:core-data` N/A.
- **Horizon E / UJ-MUST Transfer & Retention (2026-07-31):** GOLDEN §3.4f Transfer Contract (`abilityBefore`/`abilityAfter`/`firstAction24h`, module `transfer` fallback) + §3.4g Eval-as-habit; `resolveModuleTransfer` + CompleteScreen mirror; own-work slots on M3/M6/M9/M12/M15; M1 evaluator slide **12.5**; Progress `retrievalSchedule` + spaced recall (Home/Modules due card → QuizPage retrieval mode); eval deep-links. Intake `UJ_MUST_TRANSFER_RETENTION_INTAKE.md`. Scripts: `patch-uj-must-transfer*.mjs`, `audit-module-transfer.mjs`.
...
- **Horizon C corporate15 production cut (2026-07-30):** `npm run build:corporate15` (`VITE_MAX_BUILD_MODULE=15`) + Architecture A profilis `*-m1-m15.json` (`generate:core-data`); vite/vitest alias ladder 6/9/12/15/full; Vaizdo/I2V **live** corporate15 slice. Magic-link tier **15** (`MAGIC_LINK_TIERS`, `api/verify-access`); `ACCESS_TIERS` +15 @ €249 provisional. CI step corporate15. RELEASE_QA §6b. Tickets `M1315-C0`…`C4`. Intake [`M13_M15_CORPORATE_PRODUCTION_INTAKE.md`](docs/development/intake/M13_M15_CORPORATE_PRODUCTION_INTAKE.md).
- **Skyrių startai + recovery (tier-based):** sequence bypass įėjimams **M1 / M4 / M7 / M10** pagal `maxAccessible` (6/9/12); be `completedModules` seed. ModulesPage: dismissible recovery kortelė (tuščia eiga + tier ≥ 6) + nuolatinė „Skyrių startai“ juosta; util `chapterStarts.ts`. Support: `MARKETING_HANDOFF_CHECKLIST` Lost access.
- **M13–15 journey/UX gap closure (M1315-J\*):** 13.1 Patikra+Daryk; 13.3/13.4 density collapse; skaidrė **13.325** consistency lab (`m13_consistency_lab`, `M13ConsistencyLockLabBlock`, Feature Doc `M13_CONSISTENCY_LOCK_LAB.md`); `M13_PROMPT_MATURITY.md`; M15 Greitas summary escape + 48h transfer ant 150.26. Intake `M13_M15_JOURNEY_UX_INTAKE_2026-07.md`.
- **M13–15 Content-track visual exception (GOLDEN §6b):** scoped Turinio kelio spalvos be platform retheme. Token SOT `contentTrackTokens.ts` + tone maps `contentTrackDiagramTones.ts`; softRose SVG bg wash; per-step `DiagramTone` (brand/slate/amber/emerald) ant M13 process/funnel/stack + M15 practice_loop frame; 13.325 Drift Before/After strip + rose soft lab shell; Vaizdo/I2V palette swatches (`contentTrackSwatches`); path Choice `optionTone: rose` kai `moduleAccent === 'rose'`. Docs: GOLDEN §6b / §3.1b, DIAGRAM_KIT, SCHEME §2.2b, `DIAGRAMU_M13_M15_REGISTRY` „Content tones“. Intake `M13_CONTENT_TRACK_VISUAL_INTAKE_2026-07.md`.
...
- **M1315-DENS soft pass (2026-07-31):** 13.3 / 13.4 content density — Trumpai trimmed (13.4); brand + tools collapsed on 13.3 (`toolsCollapsible`); Stage copyables kept; real EN section twins via surgical overlay patch + `build-en-m13-m15` overrides (`slide13_3` / `slide13_4`). No Density CI. Scripts: `patch-m1315-dens-13-3-13-4.mjs`, `patch-m1315-dens-en-13-3-13-4.mjs`.
...
- **Design-tokens baseline sync §6b (2026-07-29):** `audit:design-tokens` BASELINE **250→272** / hex **180→202** after Content-track `contentTrackTokens` inventory (gate green; not a silence of new scatter hex).
- **M13–15 EN LT leftovers (2026-07-29):** direct patch `modules-en-m13-m15.json` — 13.31/13.36/13.37/13.4/13.47/13.51 shortTitles + tldr/patikra/nextSteps; 150.26 checkpoint chrome; 158 `Project summary`. `audit:m1315` green. Do **not** full-regenerate overlay via `build:modules-en-m13-m15`.
...
- **Docs sync A (2026-08-01):** authoring ceiling = **M1–18** / DEV unlock 18 / magic-link ≤15; open TE = Could only; test SOT **154/940**; dead archive refs retargeted (UJ sequence → `USER_JOURNEY_AGENT`; mobile → audits report; EN → `CONTENT_MODULIU_ATPAZINIMAS`); CODEBASE/INDEX/QUICK_REF/LEAN/DEPLOYMENT/README/package description sync.
- **Product next: M7–9 skaitomumo ROI (2026-07-31):** fokusas → `M79-S*` (ne Horizon D quiet). Sprendimai: **gylis lieka**; **formalus Density DoD / CI – ne** (soft CONTENT checklist); M13 13.3/13.4 dens → later closed as `M1315-DENS` done. Planas [`M79_READABILITY_EXECUTION_PLAN.md`](docs/development/intake/M79_READABILITY_EXECUTION_PLAN.md); TODO §1.2d + ROADMAP v4.4.
...
- **Horizon B-V verify/lock (2026-07-30):** re-ran corporate12 build, tier-12 tests, `audit:m1012`, release-preflight tail; EN slide-title optional_parens scrub (13.33/13.35/13.8); `.tmp-*` gitignore.
- **M13 13.1 A/E/C funnel fills (§6b):** brand depth ladder stage fills → per-step `DiagramTone` (brand / amber / emerald) + softRose bg; wow silhouette/motifs/ring lieka.
- **M13 13.1 A/E/C funnel wow (P1–P4):** brand depth ladder, active inset ring, continuous silhouette (`gapY=0` + hairlines + outline), inline motifs (eye/pointer/target), reduced-motion transitions.
- **M13 13.1 Micro A/E/C tikslų brief:** copyable po collapsible (ne image prompt); Daryk/Patikra sync; `M13_PROMPT_MATURITY` + SOT §2; EN via `build:modules-en-m13-m15`.
- **M13 Consistency Drift Lab (13.325):** ref checklist ×4 + drift/fresh ChoiceControl ×5 + Simptomas|Fix kontrastas; Copy tik lab’e. 13.32 liesinta (be dublikatinio copyable; brand sheet collapsible). Feature Doc + M13P + EN overlay sync.
- **M13 13.1 A/E/C funnel etalon:** juostose tik etapas (be sub overflow); trapezoid `path` siluetas (`funnelStageTrapezoids`); paaiškinimas Shell; W3 docs + `lmsFunnelStackPolish`.
- **M13 13.12 plain-first (2026-07-28):** Medijos grandinė – learner copy ir diagramos etiketės be bare `pipeline` / EN santrumpų sienos; checklist + step bodies su plain gloss (I2V, C2PA); chrome „grandinė“ / „chain“; SOT §2.1 + EN twin sync.
- **M13 image tools ROI (2026-07-28):** Section-breaks **13.15 / 13.36 / 13.56** – `recap` + `nextSteps` temų žemėlapis, be brandų dump’o; **13.3** – 6× `content.tools` kortelės (exact `tools.json` names) po Patikra; LT + EN; SOT §3.2 sync.
- **M13 13.1 A/E/C polish:** GOLDEN ciklas (Trumpai → schema → Daryk → collapsible guide → Patikra); LT juostų etiketės (Atpažįstamumas / Įsitraukimas / Konversija); EN 13.1 sekcijos + `slide13_1Sections` build override; glossary A/E/C terminai.
- **Horizon A closeout CORP-M1…M3 (2026-07-28):** M7–9 backlog formal close (done); mid-path handout / skills-QR won’t-now; pedagogikos Should deferred-with-date (revisit po MON / Horizon C). Day 0: DS 0.3.2 soft close + `audit:design-tokens` BASELINE → **250**. ROADMAP v4.2 · TODO §1.3 closed → archive.
- **DS 0.3.2 rails (2026-07-28):** `typographyClasses` (GOLDEN §1) — content-block body `md:text-base`, ModuleView H1 `md:`, footer `text-xs`, TemplateBlock `code`; raw `btn-*` → `CTAButton`; `.card` → `<Card />`; new `<Badge />` + call-site migracija; `DIAGRAM_TOKENS.colors.inactiveSoftDark` (CustomGpt/M7/DiPrezentacijos). Docs: `GOLDEN_STANDARD` §1, `DESIGN_SYSTEM` 0.3.2. CSS `.card`/`.btn-*`/`.badge*` lieka (0.3.3+).
- **LMS Wave 4b residual typography (2026-07-28):** M10 residual (`workflow_spec`, `incident_playbook`, `tool_decision_tree`, LearningLoop NodeBox, DepthRoles mini, orch error badge) + M15 `practice_loop` SVG size/weight → `DIAGRAM_TOKENS.typography` roles (no raw `800` / title `14`; tight leaves → `stepLabel.compact`). Docs: `LMS_DIAGRAM_POLISH_10_2.md` Wave 4b, `DIAGRAM_KIT_STANDARD` residual. Gate: `lmsCaptionTokenPolish` Wave 4b source-scan.

---

## [1.5.0] – 2026-07-28

_Horizon B corporate12 + magic-link tier 12 + M13 I2V builder; preflight closeout (design tokens + m1–m12 schema); **133/841**._

### Added

- **Horizon B corporate12 production cut (2026-07-28):** `npm run build:corporate12` (`VITE_MAX_BUILD_MODULE=12`) + Architecture A profilis `*-m1-m12.json` (`generate:core-data`); vite/vitest alias ladder 6/9/12/full; M15 Vaizdo/I2V stub’ai corporate slice. Magic-link tier **12** (`MAGIC_LINK_TIERS`, `api/verify-access`); gate/magicLink/accessTier testai. CI step corporate12. RELEASE_QA §6a. Tickets `M1012-P0`…`P4`.
- **M13 gen meter + I2V builder (2026-07-28):** 13.37 quality/readiness meter, A/E/C + proporcijos, 4 preset’ai (`VaizdoGeneratoriusSlide`, `vaizdoGenQuality`); nauja skaidrė **13.47** `i2v-generatorius` (`I2vGeneratoriusSlide`, Feature Doc `M13_I2V_CLIP_BUILDER.md`, TE overlay). Intake `M13_GEN_METER_I2V_INTAKE_2026-07.md`.
- **Footer Skaitiniai / Deep reads (2026-07-27):** globali nuoroda į `promptanatomy.blog` po Ekosistema (`utm_source=training&utm_medium=footer&utm_campaign=app_footer`); `buildFooterDeepenUrl`; i18n LT/EN. Primary nav nekeistas.
- **M4–M6 brandos polish (2026-07-27):** GOLDEN §3.2 chrome (Trumpai → Daryk → Patikra) ant silpniausių M4/M5/M6 content-block; `preCopyCheckBlock` M4/59–60 ir M6/68; M4/62 RAG magistralė – `InteractiveDiagramShell` chrome + brand-active tokens (maturity 2→3, Shell=Ne); M5/515 tabs → collapsible kortelės. LT+EN (`modules-en-m4-m6`) + `generate:core-data`. TE overlay: `preCopyCheckBlock` m4:59/60, m6:68; TCB m4:53 s2→s3.
- **P2 #2 T2 test flows (2026-07-28):** `p2.t2.flows.test.tsx` – quiz≥70→progress persist; resume slide-pos clamp; mid-quiz LT→EN chrome.

### Changed

- **ROADMAP v4.1 Horizon B exit (2026-07-28):** M10–12 corporate production cut repo-done; open fokusas → Horizon A (CORP-M\*) + MON cutover; C/D deferred. Default `build:production` lieka M1–9 iki marketing pin.
- **ROADMAP v4 production ladder (2026-07-28):** forward plan Horizon A (M1–9 micro P2) → **B** M10–12 corporate cut (P1 next) → C M13–15 prod → D M16–18; TODO seed `M1012-P*` / `CORP-M*`; intake [`M10_M12_CORPORATE_PRODUCTION_INTAKE.md`](docs/development/intake/M10_M12_CORPORATE_PRODUCTION_INTAKE.md). Nebe „learning empty → M16 OR MON“.
- **P2 #3 `modules.ts` S-R4 split (2026-07-28):** barrel `src/types/modules.ts` + `types/modules/{questions,slides,module,shared}.ts`; public imports unchanged; typecheck green.
- **M13–15 authoring brandumas closeout M1315-2/DIAG (2026-07-28):** Playwright smoke `smoke-diag1-m1315.mjs` **25/25** @375 LT/EN light/dark (C1–C6 + diagrams). DEV `getMaxAccessibleModuleId` → **15** (+ `VALID_MAX_MODULE_IDS`). M15 practice-loop CTA be curriculum ID (`m15PracticeLoopContent`).
- **M13–15 Top-5 batch M1315-W1 (2026-07-28):** density/cycle polish `13.35` / `13.34` / `13.101` / `13.33` / `13.5` (Trumpai/Patikra/collapse); EN stub rebuild + copyables. Ranking [`M13_M15_SLIDE_RANKING_AUDIT.md`](docs/development/M13_M15_SLIDE_RANKING_AUDIT.md) §6. Scripts `patch-m1315-w1-top5.mjs`, `patch-m1315-w1-en-copyables.mjs`.
- **M13–15 chrome brandumas M1315-1 (2026-07-28):** learner copy be curriculum ID (title-only cross-ref); M15 path copy be `150.5`/`151–154`; shortTitle gaps 13.11/13.37. Script `patch-m1315-i2-chrome.mjs`. Gates: footer-numbers · `audit:m1315`.

### Fixed

- **Preflight design-token regression (2026-07-28):** `text-[11px]` → `text-xs` in `I2vGeneratoriusSlide` / `VaizdoGeneratoriusSlide` — `audit:design-tokens:gate` green (arbitraryClass ≤ 59).
- **Corporate12 schema gate (2026-07-28):** `validate-schema.mjs` validates `modules-m1-m12.json`, `glossary-m1-m12.json`, `tools-m1-m12.json` + `tools-en-m1-m12.json` (id/moduleId ≤ 12).
- **M13–15 footer numbers + title WARN (M1315-F/T, 2026-07-28):** recalculated LT/EN `Toliau – skaidrė N` / `Next – slide N` for M13–15; renamed `13.12` / `143` titles (pipeline → grandinė / media chain). Script `patch-m1315-i1-footers-titles.mjs`. Gates: footer-numbers LT/EN · slide-titles · `audit:m1315` · schema.
- **M5 handout footer overlap (2026-07-27):** full M5 body at `regular` density overran A4 by ~55 mm — QC bullets + thresholds collided with copyright footer. M5 now uses `compact`; `HANDOUT_CONTENT_BOTTOM` + `buildM5HandoutPdf` fit tests (LT/EN). Smoke `generate-m5-m6-handout-pdfs` calls the real builder.

### Docs

- **Horizon B docs exit (2026-07-28):** `DEPLOYMENT.md` corporate12 eilutė; `MARKETING_HANDOFF_CHECKLIST` tier 12 cutover; ROADMAP v4.1 B done; TODO §1.2 closed → archive `M1012-P*`; RELEASE_QA §6a.
- **Docs Lean TODO closeout (2026-07-28):** Done §1.0h–§1.3 (CQ/PDF/M1012/M1315/M13GEN–I2V/P2) → archive [`TODO_DONE_SPRINTS_2026-07-28.md`](docs/archive/development/TODO_DONE_SPRINTS_2026-07-28.md); gyvas `TODO.md` §1 = tik MON + Deferred; ROADMAP Open gaps slim.
- **P2 #GP GitHub Pages gate policy (2026-07-28):** DEPLOYMENT Gate policy lentelė; GITHUB_SETUP; README; deploy.yml comment; `accessTier` MVP+tier15→6; `mvp.gating` getModule(7) null @ tier 6.
- **P2 §3 RAG kelio nuoroda (2026-07-28):** SOT M7–9 §1.1 „Papildomas skaitymas“ (M4/`61` + M7/`71.35`); sk. 71 title-only pointer LT+EN; `DUOMENU_ANALIZES_GERIAUSIOS_PRAKTIKOS` sync; `generate:core-data`.
- **M13–15 authoring brandumas epic I0 (2026-07-28):** TODO M1315-F/T/1/R/W1/2/DIAG open; ROADMAP gap; intake [`M13_M15_TOBULINIMO_INTAKE_2026-07.md`](docs/development/intake/M13_M15_TOBULINIMO_INTAKE_2026-07.md). Production release lieka Deferred.
- **Unreleased docs sync (2026-07-28):** ECOSYSTEM_MAP footer Skaitiniai/`utm_medium=footer`; RELEASE_QA spot check; PDF_MAKETO + PDF_DOWNLOAD_TESTING + **PDF_GENERATION_AGENT_MEMORY** (P2 #16); GOLDEN preCopy M4/59–60/M6/68; DIAGRAMU RAG P3 → done; TE overlay `updatedAt`; CODEBASE/INDEX/QUICK_REF/LEAN/DOCS_SYNC.
- **PDF-FIT-1 handout visual closeout (2026-07-27):** owner checked all handout PDFs (M1/M4/M5/M6/M79/M1012/M1315) after M5 compact fit — PASS; gate closed in TODO/ROADMAP/`TEST_REPORT`.
- **TE registry (2026-07-27):** `diagram:rag_duomenu_ruosimas` maturity 3; nauji `preCopyCheckBlock` embed’ai M4/M6.
- **Pre-launch WIP closeout (2026-07-28):** CODEBASE_WHAT_IS_DONE Horizon B done + 133/841; TEST_REPORT closeout note.

---

## [1.4.9] – 2026-07-27

_Learning QA closeout + PC-4 polish: CQ-PORTAL / PDF-1…6 / DIAG-1 / M1012-2 owner smokes, practice closer, PracticeIntro type gap; preflight **130/825** before tag._

### Added

- **PC-4 practice closer polish (2026-07-27):** M3 live `N/6` portfolio chip (intro + ModuleView); M6 soft-preselect ChoiceControl + „Tęsti į projektą“ CTA; M15 `Privaloma` badge (`recommended && !optional`) + intro copy be curriculum ID; M1 sandbox „treniruoklis“ banner; M3 darymo CTA. i18n LT/EN. Tests: `PracticeIntroSlide.pathChoice`. Docs: TODO/ROADMAP/PRACTICE_CLOSER/PEDAGOGINES.

### Changed

- **Audit remediacija kodas (2026-07-26):** shared `diagramPathGeom.shortenToTip` (orch/loop/M12 rewire); `DIAGRAM_AMBER_INK_SOFT` + `opacity.orphanMap` / `orphanLearningLoop`; M12 diagram single SVG path (compact/desktop layout props); PromptFilter EDA strip decorative (`aria-hidden`, ChoiceControl-only); PDF util try/log + `pdfLink` warn be `doc.link`; one-shot `patch-m10*` / `patch-m1012-w3b` → `scripts/archive/applied-2026-07/`; PracticeQuestIntro type guard + `isM7JourneyChoiceId`.
- **M7–9 mobile polish (2026-07-26):** EDA strip + tool chips `min-h-[44px]`; M9 quest intro Start before outcome chips + tighter spacing; M9/90 duration `~45–90 min` LT+EN; comparison/solution tables max-sm right-edge scroll fade.

- **M12 120.25 Stage maturity A+B+C (2026-07-26):** `toolChoiceBar` 3A juostos (`autoSelect: false` + whenHint) + linked Stage copyable ×3 + `preCopyCheckBlock` (Augment human chip) + Patikra ×2. LT `modules.json` + `build-en-m10-m12`. TE: `embed:toolChoiceBar:m12:120.25:s2`, `embed:preCopyCheckBlock:m12:120.25`.
- **M11/M12 Wave3-B Top-5 batch (2026-07-26):** `120.25` Micro Copy + Daryk; `125` Trumpai/Daryk/Patikra + 2/3 collapsible; EN orphans `126`/`127` deleted from `build-en-m10-m12`; `123` title-only incident (**Testavimas ir saugumas**) + 10.64 carry; `112` failedMessage „3A juostos (gylis ir rolės)“; `121` Stage template ≈ Minimalus eigos aprašymas. LT `modules.json` + EN rebuild. Ranking audit §7 done.
- **M10 Top-5 density/cycle batch W3a (2026-07-26):** `10.36` orientacija collapsible + Daryk vienas režimas (Patikra = preCopy); `10.65` micro-cycle Daryk/Copy/Patikra + 8 blokų collapsed; `10.37` slim (eiga → Trumpai/Daugiau); cluster eilė = live + `10.481` epilogas; `10.48`/`10.35` Trumpai/Daryk tighten. LT `modules.json` + EN overlay/`build-en-m10-m12` + SOT/eilė. Ranking audit §6 done.
- **M10 10.36 Kur paleisti – plain + embed (2026-07-26):** title/subtitle plain LT (`Kur paleisti programą ar agentą`); 4 sluoksnių lentelė; PaaS orientacija; `toolChoiceBar` režimams (`autoSelect: false`) + linked copyable ×3 (sutrumpintas šablonas); `preCopyCheckBlock` n8n vs Railway; be naujos diagramos. EN: `build-en-m10-m12.mjs` + `m10-ux-batch-en-overlay`. SOT §3c1.
- **M10 10.35 įrankių picker + medis namuose (2026-07-26):** `m10_tool_decision_tree` perkeltas iš 10.4 į 10.35; `toolChoiceBar` (Zapier/Make/n8n/Power Automate, `autoSelect: false`) + linked copyable ×4 su „kodėl ne“; 10.4 = tik DI platformos (Browse/Tools). TE: overlay slideId + `embed:toolChoiceBar:m10:10.35:s2`. EN: `build-en-m10-m12.mjs`. SOT §3c/§4.2.
- **M10 10.15 workflow sąvokos + schema (2026-07-26):** branduolys **Trigger → Condition → Action** (Shell 3); **Webhook** = Trigger tipas (chip strip), ne 4-asis žingsnis. Worked example, SOT copy (klaidos žingsnis), Patikra įvykis vs veiksmas. LT `modules.json` + `turinio_pletra` §3a + EN `build-en-m10-m12.mjs` / `m10-ux-batch-en-overlay` (10.151 recap). Schema: `M10TriggerFlowDiagram` + layout.
- **M10 10.49 heading be meta (2026-07-26):** sekcijos antraštė `Mokymosi ciklas` / EN `Learning loop` (be `(diagrama)` / `(diagram)`); instrukcija lieka body. LT `modules.json` + `m10-ux-batch-en-overlay.json` + EN rebuild.
- **M10 Wave 2 content brandumas (2026-07-26):** Orkestravimas (`10.481`/`10.482`/`10.49`); įrankiai/promptai (`10.4` medis first, `10.5` be workflow-spec spill, `10.6` „Nerašyk“); workflow (`10.15` Kur pritaikyti; `10.35`/`10.36`/`10.37` be docs/§); MUST/optional (`10.64` Trigger; `10.65` Trumpai + collapsible testai, be medis echo); santrauka be „taksonomija“. EN: `build-en-m10-m12.mjs` + `m10-ux-batch-en-overlay.json`. Intake #7–11 → done Wave2.
- **M10 Wave 1 content/schema brandumas (2026-07-26):** `10.1` Trumpai žemėlapis; `10.2` ciklo dedupe + taikymo copyable; `10.25` „Kur pritaikyti“ verslo taisyklės; `10.3` sisteminio anatomija + `[ROLĖ]`/`[RIBOS]` šablonas; `10.45` lab polish (flat SVG, caption air, decisionRule) + chrome; `10.48` `toolChoiceBar` embed (5× whenHint + linked copy). EN: `build-en-m10-m12.mjs` + `m10-ux-batch-en-overlay.json`. TE: `embed:toolChoiceBar:m10:10.48:s1`. Intake: `docs/development/intake/M10_M12_TOBULINIMO_INTAKE_2026-07.md`.

### Fixed

- **PracticeIntro type gap (2026-07-27):** `PracticeIntroContent.minScenariosToComplete` added to `modules.ts` — unblocked `typecheck` / release-preflight after PC-4 portfolio chip test.
- **M5 handout CTA at score 0 (2026-07-27):** `TestResultsSlide` M5 branch no longer requires `rawScore > 0` — failed/empty attempts still show „Parsisiųsti Modulio 5 atmintinę (PDF)“ (PDF-1 / §5d). Test: `TestResultsSlide.m5Handout.test.tsx`.
- **Handout/certificate PDF links + maturity (2026-07-26):** `pdfLink.drawClickableUrl` (underline + visible URL + `doc.link` hitbox); Decide CTA → hub (kol `.pro` nestabilus); sertifikatų author labels sukeisti; M6 LT tu-forma; path-fit primary CTA (M1012 orchestrator, M1315 ecosystem map); M79/M1012 `starterPrompt` + `addPromptBlock` / page numbers; SOT [`HANDOUT_MATURITY.md`](docs/development/HANDOUT_MATURITY.md); Annots smoke `handoutPdfLinks.annots.test.ts`.
- **M12 120.25 three labs PPT gestalt (2026-07-26):** solid 3 juostos + vienodos amber human kortelės → soft row + tone accent bar + chip flow; human gate = filled chip (Approve / Escalation); Automatize be gate chip. Caption LT/EN be „modal / View full size“. `M12ThreeLabsDiagram` + layout/content.
- **M9 practice-quest-intro domain re-select (2026-07-26):** po `moduleJourneyFocus[9]` confirm nebegalima pakeisti srities — `useEffect` sync’ino `selectedId` iš saved focus kiekvieną click. Hydrate tik iš `savedM9`/`softChoice` (be `selectedId` deps). Test: change domain after confirm.
- **M10 10.482 full-map + step-focus (2026-07-26):** visi mazgai + topologija + Kartoti U visada; žingsnis = focus ring / path emphasis / staged verbai (ne hide/appear). Staged-hide SUPERSEDED. Tests: `lmsMultiAgentPolish`.
- **M10 10.482 router→orch vertical + label AABB (2026-07-26):** `router.cx === orch.cx` (no diagonal); `parenka srautą` always left of shaft; `skaito / įrašo` biased to State; tests pill∩pill + cx. Root cause: pad-bump without intersection gate.
- **M10 10.482 staged reveal polish (2026-07-26):** _(superseded by full-map)_ hide !live nodes… Tests: `lmsMultiAgentPolish`.
- **M10 10.482 label clearance + roles (2026-07-26):** edge pills off boxes (`patvirtina` above row, `perduoda` off fan-in trunk, `parenka srautą` pad, agents band right of research drop, Kartoti lower-third); boxes = Tyrėjas / Rašytojas / Tikrintojas (EN Researcher / Writer / Checker). Tests: `lmsMultiAgentPolish`.
- **M10 10.482 orthogonal handoff + center (2026-07-26):** Vertintojas under Tikrintojas (no hypotenuse); compact elbow path; desktop X center; `perduoda` above bus; Kartoti above retry U horizontal. Tests: `lmsMultiAgentPolish`.
- **M10 10.49 learning loop routes (2026-07-26):** dual-panel + update bus (gutter magistrale), Atnaujinimas pilnai mokymosi 2×2 cikle, orthogonal fan, staged edges (`shouldPaintLearningLoopEdge`), 3 stroke tipai (flow/learn/update), learn/update tipai = rankinis polygon, verb chips. Helper `learningLoopUpdateBus.ts`. Testai: `lmsLearningLoopPolish`.
- **M10 10.49 learning loop residual P1 (2026-07-26):** `active`/`live`/`orphan` mazgų būsenos (local orphan 0.5 ≠ LMS inactive 0.88); gold ring tik fokusui; learn panel cycle inset ring nuo 2/4; ghost learn 1–2/4 sumažintas. Testai: `lmsLearningLoopPolish` state matrix.
- **M10 10.482 step-3 assign clash (2026-07-26):** fan-out assign pill off-shaft kairėn (ne ant trunk); LT/EN `paskiria agentus` / `assigns agents`; +20 px agents cascade + viewBox H 490. Testai: `lmsMultiAgentPolish`.

### Docs

- **Pre-launch deep audit (2026-07-27):** GO learning (P0 closed) · ship process → tag **v1.4.9** · NO-GO monetization; preflight **130/825** po type fix; `TEST_REPORT` verdict matrix.
- **CQ-PORTAL 48h closeout (2026-07-27):** M4 sk. 53.5 @375 — Phase A `scripts/smoke-cq-portal.mjs` (LT light/dark + EN, `fails: []`); Phase B owner-proxy Q1–Q10 panel (4 LT + 1 EN) all thresholds PASS; facilitator [`CQ_PORTAL_48H_FACILITATOR.md`](docs/development/CQ_PORTAL_48H_FACILITATOR.md). TODO/ROADMAP/TEST_REPORT; open learning P0 = none.
- **PDF-1…6 owner smoke closeout (2026-07-27):** §5d M5/M6 PDF + M4/56 · M6/64 · @390 · entry points PASS; helper `scripts/smoke-pdf-1-6.mjs`. TODO PDF-1…6 `[x]`; ROADMAP PDF gap done; `TEST_REPORT` section; lieka CQ-PORTAL (later ✅).
- **DIAG-1 + M1012-2 owner smoke closeout (2026-07-27):** Playwright @375×667 light/dark — DIAG-1 (M7/71·73·731·74, M9/93, M8/80) + M1012-2 C1–C6 (+C5b) LT/EN hotspots; no `docs/` / bare HITL / curriculum-ID chrome; no product code fix. Helper `scripts/smoke-diag1-m1012.mjs`. TODO/ROADMAP/TEST_REPORT gates closed; lieka CQ-PORTAL (PDF later ✅).
- **Audit remediacija docs BLOCK (2026-07-26):** TE inventory rebaseline **156** (embed 33 / table 30; was 149 @ 1.4.8); DIAGRAM_KIT + LMS W5 checklist → depth/roles lab (`m10DepthRoles*`, ne `m10TaxonomyLayout`); M9 MUST live `90→93.1→93.2→93→92` (PRACTICE_CLOSER / turinio / backlog / TEST_REPORT); TODO Learning QA – CQ-M79 closed, CQ-PORTAL open P0.
- **TE overlay 10.65 table index (2026-07-26):** `table:m10:10.65:s4` → `s7` (po W3a section reorder) – `audit:teaching-elements --strict` green.
- **CQ-M79-1/2 browser smoke closeout (2026-07-26):** savininkas @375px — S1–S7 + E1–E6 PASS; `TEST_REPORT` Browser ✅; TODO CQ-M79-1/2 `[x]`; ROADMAP M7–9 gap done; `07_08_09_backlog` §12–14; lieka Portal 48h / PDF (M1012-2 / DIAG-1 later ✅ 2026-07-27). Be JSON/UI.
- **M11/M12 Wave3 ranking audit (2026-07-26):** `docs/development/M11_M12_SLIDE_RANKING_AUDIT.md` – 16 skaidrių 5 ašių balai; Path Test pass + practice gaps; Top 8 / Top 5 frozen (120.25, 125, 123, 112, 121); EN orphans 126/127. TODO `M1012-W3R`. Be JSON/UI.
- **M10 slide ranking audit (2026-07-26):** `docs/development/M10_SLIDE_RANKING_AUDIT.md` – 30 skaidrių registry + 5 ašių balai (UI/UX/Journey/Maturity/TE); Top 8 worst-first; next batch Top 5 frozen. TODO `M1012-R`. Be JSON/UI.
- **M16–18 parking lot §8 (2026-07-26):** `turinio_pletra_moduliai_16_17_18.md` – freeze, un-defer gates (M79+Portal+product call), F1–F8 checklist; TODO/ROADMAP Deferred; be eilės/JSON.
- **M16–18 market SHOULD patch (2026-07-26):** `turinio_pletra_moduliai_16_17_18.md` – `PROJECT_RULES.md`, plan→approve→code, diff ritualas, 3 spąstai (vibe debt lite), auth/mokėjimai = Won’t pirmam MVP; M17 bank + copyable eskizai; framing „vibe coding su disciplina“. JSON / skaidrių eilė vis dar Deferred.
- **M16–18 agentų / rules sync (2026-07-26):** `.cursorrules`, `AGENTS.md`, QUICK_REF, LEAN §3, CONTENT/CURRICULUM/DATA/ORCHESTRATOR specs + skills/lessons, `CONTENT_MODULIU_ATPAZINIMAS` §6 – SOT visibility; vibe ≠ „niekur“; JSON/TE Deferred. Be `modules.json`.
- **M16–18 turinio SOT konsolidacija (2026-07-26):** `docs/turinio_pletra_moduliai_16_17_18.md` – curriculum SOT (M16=D1+D2 planavimas, M17 testas, M18=D3–D5 packet→build→deploy, Cursor-first); RAW intake → `docs/archive/development/intake/VIBE_CODING_INTAKE_2026-07.md`. INDEX / TODO §1.5 / ROADMAP atnaujinti. Be JSON.
...
- **M12 120.5 W7 layout brother polish I0–I5 (2026-07-26):** `m12_multi_agent_schema` – Diagram wired to `m12MultiAgentSchemaLayout` SOT; caption air + sub≥11; full-map orphan dim + focus ring; orthogonal fan-out/fan-in; staged edge verbs LT/EN; feedback U; Shell `stepOfLabel`. Unlock W7 brother = layout polish (ne tokens-only). Tests: `lmsMultiAgentPolish` M12 suite + geometry. Docs: LMS polish / registry / DIAGRAM_KIT / scheme lessons.
- **M12 120.5 / 120.55 schema sibling polish (2026-07-26):** Kept `m12_multi_agent_schema` (practice sibling of M10 orch; no new diagram). LT Trumpai chain + Koordinatorius + bridge to **Agentų orkestravimo simuliacija**; EN curated full GOLDEN sections + handoff copyable (no stub bodies); `120.55` Checkpoint EN restored; Output label HITL → human approval. SOT §8.2a sibling note. `build:modules-en-m10-m12`. Vartai: `validate:schema` · `audit:m1012` · footer M10–12 LT/EN.
...
- **tools.json hygiene (2026-07-26):** Veo → moduleId 13 + `Video generavimas`; Zapier → `Verslo automatizavimas` / EN `Business automation`; ElevenLabs → shared name + `Garsas`/`Audio`; ChatGPT/DALL·E/GPT-Image URL → `chatgpt.com`; 81 LT/EN tools; `generate:core-data`.
- **Branduolio pasitikrinimas (Plan A, 2026-07-26):** Nav „Apklausa“ → „Pasitikrink“ / EN „Ready check“; formuojantis branduolio check (ar verta eiti į M4+), ne final exam. QuizPage intro + formative results; soft CTA po M3 (ModuleComplete + ModulesPage). Tier 2 ≥70 % lieka antrinis perk’as. `modules.json` / `quiz-en.json` metadata + `generate:core-data`. GOLDEN §3.7 + CERT SOT terminologija.
- **M7 sk. 72 Strateginis pamatas copy polish (2026-07-26):** „įsisavinsi“ → „Ką čia išmoksi (1–2 min)“ (EN: _What you'll learn here_); tezė 1 + „Be sprendimo – tai tik ataskaita“ (Patikra #1 atrama); 3️⃣ antraštė sutrumpinta, body su „4 tezės“; Patikra remediation → „grįžk prie 4 tezių“; collapsible „Nori giliau? Deming (5 principai)“; EN Patikra sulyginta (2 bullets + remediation). LT + EN m7–9 + string-map + `generate:core-data`.
- **LT skaitvardžių derinimas „modulis“ (2026-07-25):** `ltPlural.ts` – one/few/other (1 modulis, 2–9 moduliai, 10–20 modulių, 21…); Home progresas, duration badge, ModulesPage / lock / SEO; EN singular/plural. Testai: `ltPlural.test.ts`.
- **M4 promptų branda M4P-0…5 (2026-07-25):** Fit-for-purpose (ne „ilgesnis = brandesnis“): Micro/Stage/Flagship/6-block išimtis 54.5. Trim 49/55/43/66.6; Stage upgrade 60/64 Quick win/65/59 mini; Micro Formatas cue 63×4 + 48; chrome Trumpai/Daryk 55. LT + EN m4–m6 + `generate:core-data`. Script: `patch-m4p-prompt-maturity.mjs`. Kontraktas: `M4_PROMPT_MATURITY.md`.
- **M7/M9 8 žingsnių workflow desktop 2×4 (2026-07-25):** `M9DataWorkflowDiagram` – 1×8 juosta → 2×4 (`→` eilutėse, `↓` 4→5); tipografija be uppercase/tracking, numeris atskirtas, `min-h` title; trumpi box labeliai LT/EN (`m9DataWorkflowContent`). Mobile stack nepakitęs.
- **M7 sk. 97 Deming teorijos atkūrimas (2026-07-25):** matoma teorija prieš Copy – kas Deming + 4 principai; promptas su Deming logika; Patikra (principas + DI output); collapsible PDCA/LT/citata (`terms`); be meta-nav į sk. 72. LT + EN m7–9 + `generate:core-data`.
- **M4/61 + M7/71.35 DI paieškos įrankiai (2026-07-25):** optional atmintinė → sprendimo įrankis: `toolChoiceBar` + „Kada ką?“ lentelė (`solutionMatrixStyle`) + tipinė eiga lentelė; GOLDEN ciklas (Trumpai → Daryk → Perplexity starteris → Patikra); `toolsCollapsible`; pašalinti collapsible „Patarimai“. LT + EN m4–m6 / m7–m9 + `generate:core-data`.
- **M7 sk. 67.8 Haliucinacijos UX (2026-07-25):** plain-text sieną pakeitė 3 lentelės (FAKTAI vs SPĖJIMAI `comparisonStyle`, Kodėl, 4 kontrolės lygiai); sujungti 4 patarimai + „ne tik promptas“; `preCopyCheckBlock` prieš pirmą copyable; 5 taisyklės + anti-šablonas hero. `ContentSlides`: be `linkedRowIndex` preCopy renderinamas prieš pirmą `copyable` (ne skaidrės viršuje). LT + EN m7–9 + `generate:core-data`. Testas: `ContentBlockSlide.preCopyPlacement`.
...
- **Turinio ambicijos flip (2026-07-26):** Šis repo = turinio/pedagogikos/UI OS. `TODO.md` / `ROADMAP.md`: P0 = mokymosi kokybė (M79 smoke, §4.6 #6–9, Portal 48h); P1 = PDF + M10–12 chrome; MON → §1.4 out of scope / marketing handoff. `USER_JOURNEY_AGENT` + skill + `AGENT_ORCHESTRATOR`: konversija → mokymosi baigtis/transferas. `PEDAGOGINES_IZVALGOS_ROADMAP` sync su `CODEBASE_WHAT_IS_DONE`. QUICK_REF / LEAN / `AGENTS.md` / orchestrator skill.
- **DATA_AGENT tools M1–15 (2026-07-26):** `DATA_AGENT_TOOLS.md` – šaltiniai M1–15, LT+EN procedūra, `audit:tools`; `DATA_AGENT_DUOMENYS_ATNAUJINIMAS` tools eilutės; data-agent `SKILL.md` + `lessons.md`.
- **Tools agents/skills sync (2026-07-26):** `AGENTS.md` mišri eilutė + universal; orchestrator/content/coding/qa skills + lessons; glossary-tools `.mdc` (EN parity + `audit:tools`); `CONTENT_AGENT.md` handoff; `RELEASE_QA_CHECKLIST` ToolsPage smoke.
- **Branduolio pasitikrinimas docs/agents sync (2026-07-26):** `AGENTS.md` pipeline; CONTENT/DATA/UI_UX/USER_JOURNEY; skills + lessons; README nav; RELEASE_QA / TESTING_CHECKLIST; TEST_REPORT; GOLDEN §3.7 + CERT SOT (jau Unreleased Changed).
- **Žodyno agent/docs sync (2026-07-26):** `AGENTS.md` universal; `CODEBASE_WHAT_IS_DONE` žodynėlio eilutė; `DATA_AGENT` §1/§5 + skill; `CONTENT_AGENT` §3.1.2; glossary order `.mdc`; `TEST_REPORT`; lessons data/qa/coding (+ ankstesnės content/ui-ux).
- **M7–9 glossary sync (2026-07-26):** `turinio_pletra_moduliai_7_8_9` §11 JSON pastaba + §11.1/§11.2; §8.2 unlockedBy = atlygis (GOLDEN §3.4d); content `lessons.md` MASTER≠Master.
- **Meta sync prieš push (2026-07-25):** `CODEBASE_WHAT_IS_DONE` / `DOCS_SYNC_CHECKLIST` / `DOCUMENTATION_INDEX` / TODO / ROADMAP / CHANGELOG santrauka → **117/745** (1.4.7 + Unreleased); be versijos bump.
- **M4 prompt maturity sync (2026-07-25):** `M4_PROMPT_MATURITY.md`; `AGENTS.md` mišri eilutė; `CONTENT_AGENT` §3.4 fit-for-purpose; `DATA_AGENT_DUOMENYS_ATNAUJINIMAS` M4–6 EN; TE registry tree; content/data/orchestrator `SKILL.md` + lessons (content/data/qa/orchestrator).
- **M7 sk. 71 path map (2026-07-25):** TE overlay + `DIAGRAMU_M7_M12_REGISTRY` + `TEACHING_ELEMENTS_REGISTRY` (`m7_macro_path_map`, linear-process, Shell Ne); `07_08_09_backlog` M79-46; `TEST_REPORT`.
- **M7/M9 8-step 2×4 sync (2026-07-25):** `DIAGRAM_KIT_STANDARD` (HTML sibling + checklist §6), `SCHEME_AGENT` M7/M9 eilutės, `CONTENT_AGENT` box-label taisyklė, `DIAGRAMU_M7_M12_REGISTRY` + TE overlay notes, `DIAGRAMU_M1_M9_AUDITAS` (ne vertical-flow), `07_08_09_backlog` M79-23, `TEST_REPORT`; scheme/coding/content/ui-ux/qa `lessons.md` + scheme/coding `SKILL.md`.
- **M7 sk. 97 sync (2026-07-25):** `turinio_pletra_moduliai_7_8_9` §7.4 + §8 7.24, `MODULIO_7_SKAIDRIU_EILES`, `07_08_09_backlog` #4–5; CONTENT `lessons.md`.
- **M4/61 + M7/71.35 sync (2026-07-25):** `LENTELIU_STANDARTAS`, `TEACHING_ELEMENTS_REGISTRY` + overlay (2× toolChoiceBar, 4× table), `turinio_pletra_moduliai_4_5_6` 4.2a-academic struktūra.
- **M7 sk. 90 prompt-tool (2026-07-25):** `GOLDEN_STANDARD` §3.8.1 (`prompt-tool` eilutė), TE overlay maturity 2→3 + `PromptFilterToolSurface`, `MODULIO_7_SKAIDRIU_EILES` 7.17, `TEACHING_ELEMENTS_REGISTRY` embed pastaba; `AGENTS.md` mišri eilutė; CONTENT/DATA/UI_UX agent SOT + skills/lessons; `TEST_REPORT` vartai.
- **M7 sk. 200 sync (2026-07-25):** `SKAIDRIU_TIPU_ANALIZE` §3.7, `DATA_AGENT_DUOMENYS_ATNAUJINIMAS` (`RESEARCH_PROMPT` + i18n), `MODULIO_7_SKAIDRIU_EILES`, `turinio_pletra_moduliai_7_8_9`, TE overlay notes (`hallucination-dashboard`), `TEST_REPORT`.
- **M7 67.8 sync (2026-07-25):** `MODULIO_7_SKAIDRIU_EILES`, `turinio_pletra_moduliai_7_8_9`, `LENTELIU_STANDARTAS`, `TEACHING_ELEMENTS_REGISTRY` + overlay (embed + 3 tables), `GOLDEN_STANDARD` §3.8.1 (`preCopyCheckBlock` vieta + etalonai), `TEST_REPORT`; agent SOT `CONTENT_AGENT` §3.6 + `DATA_AGENT_DUOMENYS_ATNAUJINIMAS` M7–9; lessons content/data/ui-ux/qa/coding/orchestrator.

---

## [1.4.7] – 2026-07-24

_Post-1.4.6 sprintas: TE registry, Path Test Shell, practice closer, LMS/diagram polish, Docs Lean; preflight gate fixes prieš tag._

### Fixed

- **M6 accent biudžetas 62/63/64 (2026-07-24):** mid sekcijos (`Pavyzdys…` / `PERŽIŪRĖTI` / `TIP` + EN) → `brand`; palikta Trumpai+Patikra accent (GOLDEN §3.2). LT + EN m4–m6 + `generate:core-data`. `audit:accent-budget` ✅.
- **M10 orchestrator typecheck (2026-07-24):** `getOrchestratorEdgeLabelAnchor` – `viewBoxW: number` (priima compact 420 | desktop 760).
- **Preflight hardening (2026-07-24):** `audit:release-preflight` įtraukia `typecheck` prieš `test:run`.
- **EN M10–12 slide titles (2026-07-24):** `audit:slide-titles` – pašalinta `(optional)` iš 10.65/10.7 title; 120.5 subtitle HITL → human approval (parity su LT); sync `build-en-m10-m12.mjs`.
- **M5 47.2 markdown lentelė (2026-07-24):** body pipes → `section.table` (comparisonStyle) LT+EN; `audit:markdown-tables` ✅.

### Added

- **Docs Lean Pass DL-0…4 (2026-07-24):** §1c Docs Lean rules; TODO/ROADMAP = open only + `TODO_DONE_SPRINTS_2026-07.md`; `.cursorrules`/`AGENTS` open P0/P1; LEAN ≤25 + QUICK_REF be legacy/audit-as-SOT; archive moves (AUDIT_2026-06, SLIDE_UX plan, portal UI audit/retrospective, DS v0.2 plan, LlmArch B3, Satori plan, UX Banga EN sync, PLAN_AGENTAI) + stubs; `DIAGRAMU_M1_M9_AUDITAS` demoted (TE = inventory); sot_index + orchestrator/scheme skills; filled QA/UJ/CR `SKILL.md`; INDEX/DOCS_SYNC/lessons sync.
- **Teaching Elements Registry TE-0…5 (2026-07-24):** Master SOT `TEACHING_ELEMENTS_REGISTRY.md` + overlay `teaching-elements-overlay.json` (**135** elementai; 38 image / 4 lab / 5 off-renderer / 2 orphaned / 22 embed / **22** table / 44 tipai; +`table:m5:47.2:s2`). Brandą 0–4 + scorecard. Drift gate: `npm run audit:teaching-elements` (`--strict` į `audit:release-preflight`). Feature Doc §1b → master overlay prieš JSON. Satelitų sync: M7–12 UTF-8 + live slide ID (73/89/92); W5 dual-taxonomy superseded; SLIDE_UX baseline **283**; sot_index / LEAN / DOC indexes. Orphaned Schema3/ContextFlow – tik registruoti. TODO §1.0g.
- **M5 Apply+Gate Wave A+B (2026-07-24):** Vienas sprinto kelias (be A/B mygtuko). Dedupe: 6 blokų promptas tik 47.5; įrankio medis (Gamma / Copilot / Canva + collapsible kiti). Wave B: **47.2** Storyline, **510.5** QC lab, **516** Rescue bonus po results; warm-up 4; testas 5; trukmė 25–30 min. `level: test` + hybrid_copy (Apply+Gate). Soft proof 510. Handout LT/EN. UI: `ModuleView` + `ContentSlides` bonus 516. Docs: `MODULIO_5_SKAIDRIU_EILES.md`, SOT §3, GOLDEN M5 išimtis, `CONTENT_MODULIU_ATPAZINIMAS`. Script: `patch-m5-ab-waves.mjs`. Core: `generate:core-data`. Gate: `audit:m46` ✅ (kartu EN Check/Patikra M6 62–64 – merge diacritics).
- **M7 promptų branda M7P-0…5 (2026-07-24):** Kontraktas F/S/R/L (`M7_PROMPT_MATURITY.md`) – ne force-META; Type S = Duomenys/Padaryk/Formatas. MASTER 74 + path-step 71.1–71.5 Type F su `Nežinau`; 73/89/90/731 Type S; journey sibling parity **42×6** (buvo 31×6); branduolio chrome Trumpai/Daryk; mid F + viz 103/106/861 Patikra/Nežinau. LT/EN + `modules-journey(-en)-m7.json` + `generate:core-data`. Registry/audit sync. Script: `patch-m7p-prompt-maturity.mjs`.
- **Path Test Shell Sprint 1 (M2/M8/M11/M14) (2026-07-24):** GOLDEN §3.4a1 Path Test Contract (kelio vartai intro → warm-up → graded → results → bonus; M2/M8 išimtys). M2 `questionPool.ts` Jūs→tu. M11 EN parity (jau) + `audit:m1012`. M14 lukštas kaip M11: `140.5` warm-up (3) + `143` pipeline bonus; scope bubbles (pipeline/audio-first/licencijos/C2PA/brand/workflow); remediation chips M8 (+732/86/733) ir M14. Bonus GOLDEN: M2 `51`/`52`, M8 `801`/`802` (+ EN m7–9). SOT sync: `turinio_pletra` M2=15; M7–9 §9 MVP gate; M13–15 §8=12Q + shell; `CONTENT_MODULIU_ATPAZINIMAS`; `MODULIO_13` eilė. EN: `build:modules-en-m13-m15`. Core: `generate:core-data`. Testai: M14 shell contract. Agent docs: `AGENTS.md`, `CONTENT_AGENT.md` §4.1, `DATA_AGENT_DUOMENYS_ATNAUJINIMAS`, `SCHEME_AGENT` test chrome; skills content/data/orchestrator + lessons (content/data/curriculum/coding/orchestrator/qa).
- **M10 10.45 depth/roles hybrid 8/10 (2026-07-24):** `m10_agent_taxonomy` → Pattern `interactive-control-lab` (Shell=Ne): mini schema (4 pill + L2 role strip) + ChoiceControl lab be scenarijaus + Copy lab viduje. W5 dual-taxonomy 8-step Shell superseded. Pašalinta 3A siena ir JSON copy wall. Failai: `M10DepthRolesLabBlock`, `M10DepthRolesMiniDiagram`, `m10DepthRoles*`. SOT §3b2, registry, GOLDEN §3.1b consumer, EN build sync.
- **M10 10.45 mini role-strip polish (2026-07-24):** edge verbs (`deleguoja`/`pateikia`/`grąžina`) 10px gutter’yje lipo po box’ais („ghost layer“); mini strip = tik shaft + tip, gap≥32, connectors under boxes.
- **M10 10.45 process tip SOT (2026-07-24):** hand-rolled marker → `getProcessArrowMarkerGeom()` (`processTipLen` + `refX=0`); horizontal + router vertical end inset via `processArrowEndInset` (ne legacy `markerLen` 6).
- **Docs/skills sync 10.45 (2026-07-24):** `AGENTS.md` hybrid route; scheme/coding/ui-ux/content/data/orchestrator `SKILL.md`; `SCHEME_AGENT.md` §2.2 vs §2.2c; lessons already had pitfall lines (ghost verbs, process tip, EN build).
- **Process arrow tip DS promote + migracija (2026-07-24):** kanonas `DIAGRAM_TOKENS.arrow.processTipLen` (10) + `processArrowMarker.ts` (`getProcessArrowMarkerGeom`, `refX=0`); `arrow.markerLen` lieka **legacy 6** (nekeisti). W1 cycle (AgentWorkflow / DataStory / LearningLoop: tip + U be start circle + flat AW fills) → etalonų rewire ant tokeno → Waves A–E (M13 linear + Turinio, M10 Trigger/Incident/WorkflowSpec/ToolTree/Taxonomy, M7 Bi/ThreeAgents, M12 ThreeLabs + layout inset, M15 PracticeLoop, Schema3/4, ContextFlow) + WorkflowComparison. Testai: `processArrowMarker`, `w1CycleArrowTip`, dual-taxonomy/M15 inset → processTipLen. Išimtys: HTML `StrukturuotasProcesas` thin connector; ContextEngineering tip 8 (ne 10). Docs: DIAGRAM_KIT, SCHEME §3.2/§3.7, DIAGRAMU_GERIAUSIOS, LMS polish tip eilutės, scheme lessons.
- **M10 10.2 AgentWorkflow caption air (2026-07-24):** I3c crop paliko title/edge lipimą; micro-breath kaip RL: `rowY` 40→**62**, `titleY` 20→**22**, feedback trough +28→**+36**; H lieka 248 (oras perskirstytas į viršų). Test: caption↔edge ≥18. Scheme lesson.
- **M10 10.482 multi-agent-flow Type Etalon W7 (2026-07-24):** `m10_agent_orchestrator` – LMS 1A floors (inactive ≥0.88, stroke.feedback, tip≥10 `refX=0`, Title Case band, `stepOfLabel`); `orchestratorRetryPath.ts`; role bands; staged verb edge labels; M12 `m12_multi_agent_schema` tip/dim tokens brother (own layout). Tests: `lmsMultiAgentPolish`. Docs: DIAGRAM_KIT, LMS polish W7, registry, SCHEME skill/lessons.
- **M10 10.482 W7 micro P1 (2026-07-24):** retry U mount only steps 4–5; single assign verb (`paskiria vykdymo agentus`); state softActive (hub keeps gold); agents band y=198; LT `Maršrutizatorius`; amber gate dark ink.
- **M10 10.482 W7 P2 junction layer (2026-07-24):** EdgePill off-shaft; orthogonal fan-out trunk/bus/drops + soft agents lane; `shouldPaintEdge` early cull; amberSoft+dark ink; Būsena shrink; title pad; state verb `skaito / įrašo`.
- **M10 10.482 W7 P3 process graph (2026-07-24):** Fan-in agents→Vertintojas→Rezultatas; true horizontals (state↔orch, eval↔output); Kartoti label clear of shaft; Tyrimas→Įrankiai dashed; lane header; annotation (not button) labels; local data stroke 2; state ⇄ markers. Tests: `lmsMultiAgentPolish`.
- **M10 10.482 W7 v06 error/retry hierarchy (2026-07-24):** Step 4 cull `eval-output` + dim non-active; Tikrinimas amber+rose stroke/`!` (ne rose fill); soft-rose fan-in 2.5; retry U tarp Įrankių ir Vertintojo; `Kartoti`/`Retry`; agentsBand start-align; NodeBox text air. Tests: `lmsMultiAgentPolish`.
- **M10 10.482 content RFP sync (2026-07-24):** Content-only – LT Maršrutizatorius (ne Router); scenarijus RFP santrauka+citatos (ne ES DI Aktas); Būsena `atmintis / changelog`; 6 step body istorija; EN overlay atkurtas. Schema/layout neliessta.
- **M10 10.482 W7 v06.1 micro chrome (2026-07-24):** Orphan node dim (`isOrchestratorNodeLive`); router/eval/state text-fit widths; `kviečia` +4 clear; viewBox H 448 + HITL +16; assign annotation opacity 0.85. Tests: `lmsMultiAgentPolish`.

### Changed

- **M11 EN overlay parity (2026-07-24):** EN Module 11 matches LT semantics – warm-up 110.5 (agent vs prompt / Augment 15% / router≠orchestrator); m11-q1 sources→10.3; m11-q8 exception review→10.26; m11-q9 learning loop→10.49; 112 META/INPUT/OUTPUT reflection; 113 bonus Trumpai→Daryk→copyable 3-role pack→Patikra; catalog description with ≥70% gate. SOT: `build-en-m10-m12.mjs` (+ regenerate). Gate: `audit:m1012` ✅.
- **Practice closer P0–P1 (PC-1…PC-3, 2026-07-24):** M9 soft MUST vs hub (`recommendedSlideIds` + amber/slate badge’ai, primary footers, `MODULIO_9_SKAIDRIU_EILES`); M3 `firstActionCTA`; M6 soft 1-iš-2 + **summary 69** + Patikra/footers; M15 LT/EN footers; eilės `MODULIO_3/6_SKAIDRIU_EILES`; `CERTIFICATE_CONTENT_SOT.md` stub. EN: m7–m9, m4–m6; M15 EN footers patched (full `build:modules-en-m13-m15` not used – preserves M13 EN). UI: `PracticeScenarioHubSlide`. PC-4.\* backlog. Plan locked earlier same day (`PRACTICE_CLOSER_PLAN.md`, TODO §1.0e).
- **M10–12 chrome consistency (2026-07-24):** GOLDEN §3.6/§4.2 – perskaičiuoti LT/EN footeriai (`Toliau – skaidrė N` / `Next – slide N`), `shortTitle` ≤55; M10/M11/M12 `firstActionCTA` be curriculum ID; 10.1 „rasi“; 10.1/10.2 Trumpai schema; PathStep „atrakinsi“; EN build `syncEnFootersFromLt`; M12 i18n required-path be slide ID. Docs: `AGENTS.md` map, CONTENT_AGENT §3.3/§3.1.2, DATA_AGENT_DUOMENYS_ATNAUJINIMAS footer/M10–12; skills content/data (+ lessons content/data/coding/ui-ux/orchestrator). Vartai: `audit-footer-numbers` LT/EN · `audit-footer-length` · `audit:accent-budget:m1012` · `audit:m1012` · `validate:schema` · `test:run`.
- **Home hero copy (2026-07-24):** Eyebrow → brand šūkis (LT `NUSTOK KALBĖTI. PRADĖK KURTI` / EN `STOP TALKING. START BUILDING`); gold pill `~45 min` → dinaminė apimtis `{{count}} modulių` / `modules`; bonus be pasenusio laiko; trust #1 → „Praktinės užduotys“ / „Hands-on practice“ (be modulių skaičiaus dublio).
- **Pre-launch auditas I0–I4 (2026-07-24):** Typecheck/ModulesPage grid tipai; M14 12 klausimų contract; `llm_arch` aria-pressed assert; HITL `lt_en_phrase` allowlist; DS gate pastaba RELEASE_QA; Feature Doc Contract; I4 – M6 accent demote + orchestrator `viewBoxW` + typecheck preflight. Meta sync **1.4.7** / **111/720**. Vartai: `typecheck` ✅ · `audit:m1012`/`m1315` ✅ · `audit:release-preflight` ✅ · builds default/MVP/M1–9 ✅.
- **M8 kelionės UX (2026-07-24):** Sk. 80 copy pagal SOT (be jargon CTA); intro scope chip'ai antrinio stiliaus + „grįši į testą“; pass CTA = bonusai + „Į Modulį 9“; fail retry → test-section; „Peržiūrėti M7/M10/M13“ išsaugo remediation source. Remediation return banner M8→M7, M11→M10, M14→M13 (ne tik M2→M1). Baigtas badge testuose → score / pakartotinė peržiūra. Geštaltas → Vizualizacija. LT+EN + m1-m9 sync.
- **M7 sk. 67.7 micro polish (2026-07-24):** Play/Pause short + aria be „autoplay“; clickHint tik `title`; bridge viena matoma eilutė; „Šablone:“ tik ground; reduced-motion hint.
- **M7 sk. 67.7 V3 content hooks (2026-07-24):** 5 etapų body hook į 67.8 anti-haliucinacinį šabloną; UI bridge eilutė; shortTitle „5 etapų patikimumas“ / EN „5-stage reliability“; CoVe → „Savęs patikra (CoVe)“ / „Self-check (CoVe)“ (≠ 5 etapų map). Be CopyButton ant 67.7.
- **M7 sk. 67.5 saugumas (2026-07-24):** GOLDEN §3.2 praktika – Trumpai → Daryk → kliento laiško scenarijus → jailbreak signalas → copyable gynybos šablonas → OWASP collapsible → Patikra (2). Žodynas: naujas „Promptų injekcija (Prompt injection)“ / EN „Prompt injection“; atnaujintas „Jailbreak“ (saugumas ≠ manipuliacija) + EN; `unlockedBy` 67.5. SOT `turinio_pletra_moduliai_7_8_9.md`. LT+EN + `generate:core-data`.
- **M5/M6 schema LMS 1A (2026-07-24):** M5 `di_prezentacijos_workflow` → `InteractiveDiagramShell` + `EnlargeableDiagram` + W2 linear polish (palette, flat fills, tip≥10/`refX=0`, column helpers). Shared `custom_gpt_process` (M4/46 + M6/66): no glow, flat/palette captions, ProcessStepper chrome slim (Enlargeable, no Card/pulse/scale); practice checklist lieka. Docs: `DIAGRAMU_M1_M9_AUDITAS`, SCHEME §3.6.
- **M5/M6 schema micro polish (2026-07-24):** M5 Title Case + verb step subs; Custom GPT SVG subtitle → HTML `diagramClickHint`, no ALL CAPS box titles, `stroke.flowStrong`, layout Y tighten; dark smoke tests DiPrez + ProcessStepper.

### Added

- **M4 54 promptų režimo lab (2026-07-24):** `m4_prompt_mode_simulator` (`interactive-control-lab`, Shell: Ne) – metodinis vs agentinis pagal gyvi duomenys × metodika; 3×4; match/mismatch; klaidos atvejis; kopijuojama taisyklė → RAG / M6. Failai: `M4PromptModeSimulatorBlock`, `m4PromptModeModel/Content/LabTokens`. SOT §4.1-prompts; registry `DIAGRAMU_M1_M9_AUDITAS`; GOLDEN §3.1b/c; LT+EN + `generate:core-data`.
- **M8 results CTA test (2026-07-24):** `TestPracticeSlides.m8Results.test.tsx` – fail retry / remediation source / pass dual CTA.
...
- **M10 10.26 lab visual W1.1 polish (2026-07-23):** Risk strip (3 chips, be empty celės); rekomendacija tik po režimo; 2 trade meteriai (be error-timing dublio); scenario description be stake chip dublio; SCHEME/registry sync.
- **M10 10.26 lab visual W1 (2026-07-23):** GOLDEN §3.1c lab color exception; risk tokens + 2×2 matrix + mode timing strip + trade meters; ChoiceControl optional `optionTone`; SCHEME §2.2c + registry.
- **M10 HITL terminas (2026-07-23):** `glossary.json` + `glossary-en.json` + M10 žodynėlis 10.7 – **Žmogaus kontrolė (Human-in-the-loop / HITL)** / EN **Human-in-the-loop (HITL)**; 10.26 Trumpai bilingvinis paminėjimas; SOT §3b1 + `PAPRASTOS_KALBOS_GAIRES` HITL eilutė. Antraštėse HITL vis dar draudžiama.
- **M10 10.45 dual-taxonomy etalon W5 (2026-07-23):** `m10_agent_taxonomy` – Pattern `dual-taxonomy`, Shell=Taip, 8-step decision spine (L0–L3 → roles); L2 owns hub + bridge; LMS strokes/edge verbs; slim GOLDEN body; 10.451 differentiated (I/O only). Layout `m10TaxonomyLayout.ts`; tests `lmsDualTaxonomyPolish` + localization 8 nav.
- **M10 10.26 žmogaus kontrolės simuliatorius (2026-07-23):** Nauja MUST skaidrė po 10.25 (3A), prieš 10.3 – `content-block` + `m10_human_control_simulator` (ne SVG shell). Keturi režimai × 3 scenarijai, rekomendacija/neatitikimas, klaidos atvejo outcome, kopijuojama kontrolės taisyklė → 10.64 / 10.66 / M11 `m11-q8` / M12 `122`. Modelis: `m10HumanControlModel.ts` + LT/EN `m10HumanControlContent.ts`. Registry: `interactive-control-lab` eilutė `DIAGRAMU_M7_M12_REGISTRY.md`.
- **M13–15 schemas DiagramKit + P0–P2 (2026-07-22):** I0–I4: `m13_media_pipeline` (13.12, 6 žingsniai); `turinio_workflow` → `diagramRenderers` + shell; `m15_practice_loop` quick/full + shell; AEC/prompt/thirds token polish. W1–W5: 13.12 „Kaip skaityti schemą“ + kryžminės nuorodos 13.12↔13.11; A/E/C + prompt stack → shell; M15 focus (viena aktyvi eilė + CTA 150.5/151); `m13_consistency_lock` (13.32); `m13_postprod_steps` (13.52); `m13BusinessWorkflowContent` + `m15PracticeLoopLayout` (+ geometry test). Registry: `docs/development/DIAGRAMU_M13_M15_REGISTRY.md`.

### Fixed

- **M4 sk. 56 LlmArch micro polish (2026-07-24):** Suvienodintos Neaktyvu juostos (Tools+DB); absent be hint overlap; `role=img` tik srautui; return etiketė prie path (`getReturnPathLabelPoint`); RAG tool soft-live; sutrauktas Bazinio spacer; nuimtas subtitle UPPERCASE.
- **M4 sk. 48 RLHF santrumpa (2026-07-23):** 1️⃣ Trumpai – `RLHF (Reinforcement Learning from Human Feedback)` vietoj LT parafrazės skliaustuose; paralelė su RL; SOT `turinio_pletra` *with*→*from*.
- **LLM feedback tip/label micro (2026-07-23):** Tip→Įvestis gap 8→**3** (nebe floating); etiketė „Pridedama…“ `troughY+12` **po** linija (ne `troughY−10` virš – liejosi su N eilės blokais). SCHEME §3.7.4; tests + lessons.
- **LLM feedback inter-row U (2026-07-23):** Autoregresinė schema – įstrižas Q + tip ant rėmo skaitė kaip atvirkštinę jungtį. Fix: `feedbackInterRowPath` (žemyn→kairėn→žemyn), ROW_N1 +8 gutter; tip/label micro – žr. aukščiau; tests + scheme lesson.
- **LLM feedback tip outside block (2026-07-23):** Autoregresinė schema – feedback trikampis buvo piešiamas _bloko viduje_ muted flow 1.2 (vizualiai dingsta). Etalonas RlProcess/AgentWorkflow: tip už krašto, accentDark, stroke.feedback 3.5, tip 12×8. Layout FEEDBACK tipY/baseY; tests + scheme lesson.
- **Diagram forward arrows / bare triangles (2026-07-23):** After LMS `stroke.flow` 3.5, SVG default `markerUnits=strokeWidth` scaled tips past the shaft (GAP≈24) — forward connectors looked like floating triangles. Fix: `DIAGRAM_TOKENS.arrow.markerUnits=userSpaceOnUse` on process markers (`LlmAutoregressiveDiagram` P0 + DiagramKit/legacy marker sweep). Docs: SCHEME §3.2, DIAGRAM_KIT, GERIAUSIOS_PRAKTIKOS §6.1; tests `llmAutoregressiveLayout.test.ts`.

### Changed

- **M7 DataPrep LMS 1A parity (2026-07-24):** `m7_data_prep_workflow` – flat/soft fills, BOX_H 58, desktop 600×440 / `max-w-3xl`, local tip 10 + `refX=0`, verb subtekstai, shell `stepOfLabel` + clickHint + Enlargeable `top-right` viduje shell; `viewBoxH` 448. Tests: `lmsLinearPolish`, `DiagramLocalization`. Docs: LMS W2 DataPrep parity, registry, scheme lesson (Media/Consistency/Postprod/Turinio – galimas tas pats 1A gap).
- **M7 DA pipeline LMS 1A+ micro (2026-07-24):** Inactive soft ≠ frame (light `#e6eef7` / dark `#334155`); `EnlargeableDiagram` viduje shell (mygtukas ant SVG); trumpi desc; matomas clickHint; SVG caption „Analizės eiga“ / „Analysis path“; explanation titles be `N.`; `viewFullSize` → „Išskleisti schemą“ / „Expand diagram“; startY 44 / viewBoxH 520.
- **M7 DA pipeline LMS 1A polish (2026-07-24):** Type Etalon W2 `m7_da_pipeline` – flat step fills (be gradientų), BOX_H 58, desktop 600×440 / `max-w-3xl`, local arrow tip 10 + `refX=0` (global `markerLen` neliečiamas), verb subtekstai (Modeliavimas/Modeling), shell `stepOfLabel` + enlarge `top-right`, hero explanation padding; skaidrės 73 body atskiria schemą vs promptus 3a–3e. Tests: `lmsLinearPolish`, `verticalFlowGeometry`, `DiagramLocalization`.
- **M4 sk. 56 LlmArch Type Etalon W6 (2026-07-24):** `comparison-mode-architecture` – režimai Bazinis/RAG/Agentinis; brand-blue active (tab+card+badge); mode-absent dashed + „Neaktyvu“; LLM be CTA glow; `llmArchReturnPath` + tests; `llm_arch` premium slim chrome; slide heading „DI sistemos veikimo režimai“. Docs: LMS W6, DIAGRAM_KIT, M1–M9 auditas, SCHEME lessons.
- **M4 RL process LMS polish (2026-07-24):** `RlProcessDiagram` – flat brand/amber fills (be gradientų), be glow/scale; feedback U be start circle; EN Agent „AI system“. **Visual weight:** BOX 180, GAP 32, tip 10; active = stroke 2.5; nodeTextGap 30; **caption air:** ROW_Y 62 / viewBox H 236 (nebe „RL proceso struktūra“ ant „atlikimas“).
- **M4 sk. 45 ContextEngineeringPipeline placeholder B (2026-07-23):** Prompt režime 3/5 = dashed outline slotai (`+ kontekstas` / `+ įrankiai`), ne ghost fill; Context režime – pilni emerald + badge; stub rodyklės į slotus Prompt režime nerodomos (bypass lieka). Docs/agents sync: `DIAGRAM_KIT_STANDARD` comparison exception, `SCHEME_AGENT` + scheme/ui-ux lessons, `DIAGRAMU_M1_M9_AUDITAS` P2/inventory, SOT `turinio_pletra_moduliai_4_5_6` UI pastaba.
- **M4 sk. 45 ContextEngineeringPipeline B+1 (2026-07-23):** Geometrija VB 580 / BOX 250 (annotation lane ≥140 px, L-path ≠ label X); token emerald AA (be pastel + `brightness` wash); badge pill; edge 12/500 su 2 eilučių LT/EN anotacijomis; `max-w-xl`; guardrail `contextEngineeringPipelineLayout.test.ts`.
- **M4 struktūruotas procesas HTML polish + DS tipas (2026-07-23):** `StrukturuotasProcesasDiagram` – be outer border/shadow (Shell chrome); inactive ≥ `DIAGRAM_TOKENS.opacity.inactive`; connector brand/flow + stroke.inactive (HTML išimtis vs SVG 3.5). `DIAGRAM_KIT_STANDARD` – etalon `linear-process (HTML horizontal cards)`; `DIAGRAMU_M1_M9_AUDITAS` M4/43 sync.
- **M10 10.45 P6 label air (2026-07-23):** viewBox 500, COL_GAP 40, Spec/Eval ±120; staged edge pills by shell step; hand anchors (clean space > midpoint); grąžina inside frame. LMS W5 + scheme lesson.
- **M10 10.45 P5 framing (2026-07-23):** UI be „taksonomija“/„taxonomy“ – SVG „Agentų gylis ir rolės“; section „Gylis ir komandos rolės“; Trumpai/3A/Daryk polish; M100 outcome sync; SOT §3b2.
- **M10 10.45 dual-taxonomy P4 visual grammar (2026-07-23):** Ghost hard (`HUB_GHOST` 0.2, be edges/pills kai `!isHubLive`); rolių paletė ≠ L0–L3 (bendras slate + Coord violet); edge KISS – Spec→Coord `pateikia`/`delivers`, vienas dashed `grąžina` (be dual Eval shafts). LMS W5 + scheme lessons.
- **M10 10.26 docs sync W1.1 (2026-07-23):** SOT §3b1, SCHEME §2.2c (be „risk matrix“), GOLDEN §3.1c, indexes, CONTENT_AGENT – risk strip + lab CopyButton artefaktas; be senų empty-matrix nuorodų.
- **M10 10.45 dual-taxonomy P3 state truth (2026-07-23):** Hub live tik L2/rolė; ghost caption „Komanda – pasirink L2“; be SVG subtitle; quiet inactive fills; simetrinis feedback U; intentional LT hyphenation; daugiau oro hub'e.
- **M10 10.45 dual-taxonomy visual polish (2026-07-23):** Header zona; kolonų 38/62; L2→Koordinatorius rodyklė + pill „Pasirinktas lygis“; hierarchinis hub; pill edge labels; outer feedback; tylūs neaktyvūs lygiai; vienodi r=48; plonesni mazgų kontūrai (flow shafts LMS ≥3.5).
- **M10 10.45 content recreate (2026-07-23):** Pašalintos L0–L3 / rolių brand sienos (→ shell explanations); Trumpai → schema → 3A tipas → Daryk (su L?) → Copy → Patikra; SOT §3b2 sync.
- **M10 10.25 3A pedagogika A+B (2026-07-23):** Shell explanations = Kiek/Kodėl/Pvz. (`**%**` + klaidos / HITL / siaura autonomija); vertikali 3 eilučių legenda (● TITLE pct + sub); Trumpai = sprendimų portfelis; SOT §3b „Kodėl“ sinchronas. Layout `viewBoxH` ~232.
- **M10 10.26 docs/agents sync (2026-07-23):** SCHEME §2.2c `interactive-control-lab`; GOLDEN §3.1b + ChoiceControl consumers; agent skills/lessons; Feature Doc Contract (`DOCS_MAINTENANCE`); meta indexes + `sot_index`.
- **M10 10.25 3A legend + title budget (2026-07-23):** Po juosta – tolygi legenda (● title pct), ne centro-labeliai ant 15%/5%; juostoje tik %; pašalintas AUGMENT/AUTONOMIZE overlap ir AUTOMATIZE ×4 dublis.
- **M10 10.25 3A horizontal 100% bar (2026-07-23):** Vertical square+bars → viena stacked juosta 80/15/5; be SVG title dublio; `max-w-3xl`; `showEnlargeControl={false}`; `stepOfLabel`. Layout SOT remodel `m10ThreeAStrategyLayout.ts`.
- **M10 10.25 3A pedagoginis rewrite (2026-07-23):** Pašalinta „Trys juostos“ dublis; diagrama = skaitymo gidas; pridėtas vienas užpildytas pavyzdys prieš Daryk; Patikra + „būtinas žmogus“ + remediacija į diagramą/pavyzdį. SOT §3b + LT/EN.
- **M10 10.25 3A strategija diagrama (2026-07-23):** Proporcinės juostos 80∶15∶5 + side labels (be teksto juostose); `InteractiveDiagramShell density=hero` + HitArea; layout SOT `m10ThreeAStrategyLayout.ts`; explanations LT/EN. Pašalinta overlapping subtitle / static comparison išimtis.
- **LMS diagram polish I0–I7 (2026-07-23):** Pilot M10.2 `agent_workflow_diagram` – `agentWorkflowLayout.ts`, didesni mazgai (`BOX_W=180`), `max-w-4xl`, edge labels 11px + connector, feedback R=16, inactive 0.7; `InteractiveDiagramShell density="hero"`; copy 5 žingsnių (su Aplinka) LT+EN+SOT; slide chrome slim (`isPremiumDiagram` + transparent panel); AppNav overall % slepiamas modulio vaizde. Roll-out `density=hero`: LearningLoop, DataStoryCycle, RlProcess, M15 practice loop, TriggerFlow, M9 workflow, Orchestrator, M12 multi-agent. Brief: `docs/development/LMS_DIAGRAM_POLISH_10_2.md`.
- **M10.2 I3b visual (2026-07-23):** Agentų ciklas – kompaktiškesnis SVG (1100×300, BOX 188×88, GAP 32), `max-w-5xl`; forward iš bloko krašto stroke 3.5 (matomas kotas); feedback U `ACCENT_DARK` 3.5 + tip tik į Agent; inactive 0.88 (be grey wash).
- **M10.2 I3c micro polish (2026-07-23):** SVG H 248 / compact 540; hero status „Žingsnis N iš T“ + title pill; `space-y-2` + lengvesnis inactive nav; edge labels 12/500; explanation `border-l-2`; premium diagram section heading `text-2xl sm:text-3xl font-semibold`.
- **M10.2 etalon center (2026-07-23):** desktop eilė horizontaliai centre – `START_X = (VIEWBOX_W − rowWidth) / 2` (16|16 maržos); geometry guard `agentWorkflowDesktopMarginsEqual`.
- **LMS W1 cycle brothers (2026-07-23):** DataStory / LearningLoop / RlProcess / M12 – `DIAGRAM_TOKENS.lmsCycle` (inactive 0.88, stroke 3.5, title 17/700); feedback tip `accentDark` kur U/return; topologijos unikalumas išsaugotas (ne AgentWorkflow BOX clone).
- **DS DiagramKit promote A→B→C (2026-07-23):** canonical LMS floors į `opacity`/`stroke`/`typography`; `diagramLayoutMath`; process Blocks `density=hero`; linear etalon `m7_da_pipeline`; funnel/stack breathing; `lmsCycle` deprecated alias.
- **Type Etalon W1 cycle-feedback (2026-07-23):** `cycleFeedbackGeometry` (`horizontalRowBoxes`, `feedbackUPath`); AgentWorkflow/DataStory/RlProcess wired; LearningLoop/M12 → canonical tokens; checklist LMS brief + DIAGRAM_KIT; next = linear Wave 2.
...
- **Type Etalon W3 funnel/stack (2026-07-23):** `funnelStackGeometry` (widths/rects); AEC funnel + prompt stack wired; rule_of_thirds titleWeight; type-etalon waves closed (W1–W3).
- **LMS Wave 4 residual caption tokens (2026-07-23):** SVG diagram titles → `typography.titleWeight` (non-spine); LMS diagram polish track complete.
- **M10.2 edge labels B + title caption (2026-07-23):** Forward etiketės → veiksmažodžiai (užduoda/assigns, sudaro planą/plans, kviečia/calls, grąžina/returns); SVG title caption 17px / weight 700 (ne H1 konkurentas).
- **Module chrome: breadcrumb vs Ankstesnė (2026-07-23):** Pašalinta „Atgal į modulius“ / „Atgal“ konkurencija – escape = `ModuleBreadcrumb` („Moduliai / …“), player Prev = `prevShort` („Ankstesnė“); mobile nebesutrumpina escape į „Atgal“. `ModuleCompleteScreen` secondary = „Moduliai“. GOLDEN §8.6; LT/EN keys `modulesParent`, `prevShort`, `breadcrumbAria`.
- **Path Choice P0–P2 (2026-07-23):** M15 practice-intro Greitas/Pilnas ↔ Fast track (skip 151–154); M10/M13 action-intro `howToUseModule.short/full`; M6 projekto tipas ChoiceControl (navigate 61/67, ne Fast track). GOLDEN §3.1b consumers; PEDAGOGINE §2.8 M6.
- **ChoiceControl DS (2026-07-23):** Naujas `ChoiceControl` primityvas (radiogroup); M4 Trumpas/Ilgas + M7 journey kortelės; Fast track label → „Trumpas kelias“ / „Short path“; GOLDEN §3.1b + UI_UX §4.2.
- **M4 4.1 Trumpas/Ilgas kelio mygtukai (2026-07-23):** Vietoj `howToUseModule` SOT kodų (`4.2-open`, `4.3a`) – du mygtukai ↔ Fast track; outcomes sutrumpinti iki 4 naudos punktų; optional +59 / +65. LT/EN + SOT §1.3–1.4 / PEDAGOGINE §2.8.
- **Docs sync – ModulesPage katalogas UX (2026-07-23):** GOLDEN §8.4 (track / subsection / materials pozicija); UI_UX_AGENT §3.8; skills/lessons (ui-ux, coding, qa, user-journey); PDF_DOWNLOAD_TESTING + TESTING/RELEASE_QA checklist; CODEBASE_WHAT_IS_DONE.
- **ModulesPage „Mano medžiaga“ po prieinamo katalogo (2026-07-23):** Atmintinės/sertifikatai rodomi grid’e iškart po paskutinio `maxAccessible` modulio, prieš tier-locked track’us ir coming-soon.
- **ModulesPage bazės sub-juosta M4–M6 (2026-07-23):** Po M1–M3 track’e „Bazė“ – tylesnė subsection juosta „Kontekstas ir projektas (M4–M6)“ (ne antras track); atnaujintas `trackBaseSubtitle` LT/EN.
- **Docs sync – M13–15 schemų agentai (2026-07-22):** `SCHEME_AGENT.md` §2 / §2.2b + M13/M15 inventorius; `CONTENT_AGENT.md` / `DATA_AGENT_DUOMENYS_ATNAUJINIMAS.md` registry pointeriai; `DOCS_SYNC_CHECKLIST.md` 11b; local skills/lessons (scheme/content/data/coding/curriculum).
- **ModulesPage katalogas – plain language + polish + DS chrome (2026-07-22):** Modulių kortelių LT/EN `description` / `subtitle` / `businessExamples` (M1–15) – paprasta nauda be SOT jargon (`pipeline`, `MASTER`, `provenance`, `RAG` ir pan.); learn balsas `Išmoksi` / `Learn to`, testai = ką tikrina + `≥70%` (be „Trumpas testas“ dublio); anglicizmų valymas (darbo eiga, užduoties aprašas, pagrindinis vaizdas). Faktai: M7 be M9 bleed, M12 EN sync, M12/M14 trukmė. Track antraštės pagal accent (brand/sky/fuchsia/rose); coming-soon Agentai + Turinys. **DS chrome:** kortelių title `h3`; CTA dekoratyvus `span` (`min-h-[44px]`, vienas tab stop); ikonos well + chips = `module.accent` (`accentIconWellClasses` / `accentChipClasses`); progress = level gradient; `duration` po subtitle; „rekomenduojama“ – iki 1 incomplete unlocked per track; locked overlay kontrastas; GOLDEN §6 M9/M15 `identityIcon` → `Rocket`. `comingSoonModules.ts`, `moduleIdentity` track/coming-soon/accent maps; `gate.smoke` 6 preview. `generate:core-data`. Docs/skills: `.cursor/rules/module-description-criteria.mdc`, `CONTENT_AGENT.md` §3.0, `DATA_AGENT_DUOMENYS_ATNAUJINIMAS.md` §3, CONTENT/DATA/UI_UX `lessons.md`, `CONTENT_MODULIU_ATPAZINIMAS.md` §6.
- **M13–15 2026 best-practice curriculum refresh (2026-07-22):** Generatyvinės medijos pipeline (**13.12**), character/product consistency (**13.32**), video 3–5 s + CPI + 2026 tool matrix (**13.4/13.5**), post-production (**13.52**), audio-first VO/SFX/music (**13.56/13.6/13.7**), C2PA/SynthID/disclosure (**13.101**), M14 **12** klausimų, M15 optional montažas (**154**). SOT `turinio_pletra_moduliai_13_14_15.md`; eilė `MODULIO_13_SKAIDRIU_EILES.md`; atpažinimas §6; LT `modules.json` + EN `modules-en-m13-m15.json`; `tools.json` / `tools-en.json` (FLUX, GPT-Image, Seedance, Kling, Veo 3.1, ElevenMusic, Imagen); `m15PracticeLoopContent` garsas/montažas. Agent lessons + `CONTENT_AGENT.md` / `CURRICULUM_AGENT.md` / `DATA_AGENT_DUOMENYS_ATNAUJINIMAS.md` sync. P2 lieka: hybrid B-roll, Zapier automation, ComfyUI; optional `m1315HandoutContent` body sync.
- **M13–15 P0+P1 brandumo sprintas (2026-07-21):** Audito silpnos zonos – 13.37 JSON `tldr`/`patikra` + `VaizdoGeneratoriusSlide` props/Banner + a11y; 13.101 matoma „Teisės ir rizikos (privaloma)“; 13.6 TOP-5 įrankiai + subtitle be „demokratizacijos“; 13.7 muzikos licencijos checklist; 13.35 draft tonas + collapsible workflow; M14 q2 → brand/proporcijų scenario (scenario share 50 %); M15 MUST 150.5 (SOT sinchronas), 150.25 GOLDEN ciklas, 150.26 + „Eiti į santrauką“, 151–153 optional. Kodas: `ModuleView` M15 summary jump, `PathStepSlide` CTA. Docs: `turinio_pletra_moduliai_13_14_15.md`, `MODULIO_13_SKAIDRIU_EILES.md`. EN overlay sync; `audit:m1315` / `validate:schema` OK.
...
- **M10 sk. 10.37 GitHub kaip kodo šaltinis (2026-07-20):** optional content-block po 10.36 – GitHub saugo / PaaS paleidžia; lentelė GitHub/Vercel/Railway/Render; GOLDEN; be git tutorialio. LT+EN. Tiltai 10.36/10.1/10.151/10.7. Glossary + `tools.json` GitHub (Kodo saugykla). `slidePhaseConfig` → Integracijos. SOT §3c2; AUTOMATIZAVIMO §21b.
- **M10 sk. 10.36 Paleidimas / PaaS (2026-07-20):** lean content-block po 10.35 – darbo eiga vs RPA vs DI agentas vs Railway tipo hostingas; `section.table` lentelės (režimai + PaaS); GOLDEN ciklas; hybrid collapsible. LT+EN. Tiltai: 10.35, 10.151, 10.1, 10.7, lab 121. `slidePhaseConfig` → Integracijos. `tools.json` / `tools-en.json`: Railway, Render, Fly.io. Glossary: Hostingas, Always-on, RPA. SOT §3c1; `AUTOMATIZAVIMO_IRANKIAI_VERSLUI.md` §21b; `MODULIO_10_SKAIDRIU_EILES.md`.
- **`audit:slide-titles` (2026-07-20):** `scripts/audit-slide-titles.mjs` + `npm run audit:slide-titles` – FAIL ant `Skyrius:` / `(neprivaloma)` / bilietas / HITL|Spec|CopyButton title|sub / LT `workflow` antraštėse; WARN title >55 be `shortTitle`. Įtraukta į `audit:release-preflight` ir RELEASE_QA.
- **M10–12 DS audit iteracijos I1–I5 (2026-07-20):** accent biudžetas (Daryk→brand); M10 `slidePhaseConfig` fazės (`3A ir šablonas` + orphan ID); ToolDecisionTree → `InteractiveDiagramShell`; `DIAGRAM_ROLE_COLORS` / `getDiagramActiveStroke()` + M10/M12 SVG token migracija; `SlideWorkspace` M11–12; ModuleComplete `resolveModuleAccent` juosta; terminai workflow→darbo eiga antraštėse; EN `10.2`/`125` shape sync; `10.5` GOLDEN ciklas; `scripts/audit-accent-budget.mjs` + `npm run audit:accent-budget[:m1012]` (release-preflight).
- **M10–12 pedagoginė analizė + max ROI turinys (2026-07-20):** `docs/PEDAGOGINE_ANALIZE_MODULIAI_10_11_12.md` (S/P/N/R scorecard, silpniausios skaidrės, backlog). Indeksai: `DOCUMENTATION_INDEX.md`, `LEAN_INDEX.md`.
- **M10 sk. 10.64 Minimalus darbo eigos aprašymas (2026-07-20):** privaloma content-block po 10.36/10.37 – 1 psl. schema + 3 bandymai + kada tvirtina žmogus; prieš optional 10.65. Susieta su 10.51, 10.66, 120 intro, 121–123. LT+EN. SOT §3d0; `MODULIO_10_SKAIDRIU_EILES.md`.

### Changed

- **DS max-ROI compliance (2026-07-20):** GOLDEN §3.2 accent biudžetas – max 2× `accent` (Trumpai+Patikra); CTA/Daryk/`brand`/`terms` remap M4 (44, 53, 54, 54.5, 60, 64), M5 47.5, M6 65, M7 (67.3–68, 77.5), M8 801–802, M9 93, M13 13.11. LT `modules.json` + EN `modules-en-m4-m6` / `m7-m9` / `m13-m15`; `generate:core-data`. `SlideWorkspace` – visi M1–15 `content-block` / `evaluator-prompt-block` (nebėra modulio Set filtro). `M10OrchestratorDiagram` SVG fonas → `<linearGradient>` + `<rect>` (inlineStyle 13→12; `audit:design-tokens:gate` PASS). `audit:accent-budget` skaito visus EN overlay; `audit:release-preflight` → pilnas `audit:accent-budget` (ne tik `:m1012`). Docs: `DESIGN_SYSTEM.md` perrašytas kaip dabartinis SOT indeksas (sluoksniai, accent biudžetas, gate baseline, versijų istorija; be „TBD v0.3“ stub); `DESIGN_SYSTEM_V0_2.md` pažymėtas ARCHYVAS; `RELEASE_QA_CHECKLIST.md`, `TODO.md` DS-max-ROI ✅. EN M7 sk. 891 `shortTitle` (be LT diacritics leak).
- **Skaidrių antraščių kokybė M1–M15 (2026-07-20):** sisteminis pervadinimas – be `Skyrius:` (ir `section-break` `content.title`), `(neprivaloma)`, „bilietas“ / „Prieš M12“ antraštėse; `workflow` → **darbo eiga** LT titles; 10.64 → **Minimalus darbo eigos aprašymas** (body/Copy LT+EN be „entry ticket“); CopyButton pašalintas iš subtitle; `shortTitle` ilgoms antraštėms. EN overlay sync + `generate:core-data`. `PAPRASTOS_KALBOS_GAIRES.md` §2a; eilės `MODULIO_4/7/10_SKAIDRIU_EILES.md`; SOT §3d0.
- **M10 10.36/10.37 polish + footeriai (2026-07-20):** LT kalba (paleidimo platformos, įrankiai be kodo, darbo eiga pirmame paminėjime); SOT §3c1 Daryk→brand; footeriai 10.36→24 / 10.37→25 (Minimalus aprašymas). Glossary „GitHub (repo)“ suderintas.
- **M10–12 DS consistency (2026-07-20):** GOLDEN §3.2 accent taisyklė dokumentuota; `PAPRASTOS_KALBOS_GAIRES.md` – workflow/router poros; `DESIGN_SYSTEM.md` SlideWorkspace (vėliau išplėsta į visus modulius – žr. DS max-ROI).
- **M10–12 pedagoginis max ROI (2026-07-20):** GOLDEN veiksmo ciklas silpnoms skaidrėms – 10.25 (3A priskyrimas), 10.15 (Copy+Patikra), 10.35 (sprendimo matrica), 120.25 (Trumpai/Daryk/Patikra), 124 (golden test + įrankio pėdsakas). LT `modules.json` + EN `modules-en-m10-m12.json` + SOT.
- **M10 seka 10.3 (2026-07-20):** sisteminio prompto skaidrė perkelta po 10.25, prieš 10.45 (taksonomiją); Trumpai tiltas į 10.45. `relatedSlideId: 10.3` (M11) nepakitęs.
- **M12 sk. 120.5 (2026-07-20):** Trumpai + Daryk + Copy perdavimo taisyklė + Patikra (ne tik schema prose).
- **M10–12 diagram audit W0–W3 (2026-07-20):** Registry + RELEASE_QA §5b1 shell truth; code scorecard `AUDITO_ATASKAITA_MODULIAI_10_11_12.md`. P1 polish: AgentWorkflow `reflow`; taxonomy marker inset; 3A/ThreeLabs/incident dark tones; trigger/workflow/incident → `InteractiveDiagramShell`; tool tree `DiagramStepNav` → vėliau full shell (I2); learning-loop edges from layout SOT + compact `update`. **M10-DIA-02** closed (4 makro contract); **M10-DIA-03** evaluator→coordinator dashed feedback. Tests: `DiagramLocalization` + `m10m12LayoutGeometry`.
...
- **Agentų pamokos:** po 2026-07-30 konsolidacijos laikomos `docs/development/lessons/`; anti-PPT įrašai perkelti į atitinkamus `<agent>.md` failus.
...
- **Orchestrator:** doc-loading triggers + Special case Ecosystem/spin-off; [`orchestrator` lessons](docs/development/lessons/orchestrator.md).
